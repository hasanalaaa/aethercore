#!/usr/bin/env python3
"""Build the complete Phase 34 archive twice and write its external hash."""
...
OUT = ROOT.parent
NAME = "AetherCore-Phase34-Master-Delivery"
EXCLUDED_DIRS = {
    "target",
    "node_modules",
    ".git",
...
def build(destination: pathlib.Path) -> None:
    if destination.exists():
        destination.unlink()
    staging = pathlib.Path("/tmp/p34_archive_stage")
...
if not identical:
    raise SystemExit(1)
(OUT / "PHASE34_FINAL_SHA256.txt").write_text(
    f"{hashes[0]}  {NAME}.zip\n", encoding="utf-8"
)
