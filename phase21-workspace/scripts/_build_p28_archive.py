#!/usr/bin/env python3
"""GH gate: deterministic Phase 28 master archive built twice with identical SHA-256."""
import hashlib
import os
import shutil
import subprocess
...
def build(dest: Path):
    if dest.exists():
        dest.unlink()
    staging = Path("/tmp/p28_archive_stage")
    if staging.exists():
    ...
    gzip = subprocess.Popen(["gzip", "-n", "-9"], stdin=tar.stdout,
                            stdout=open(dest, "wb"))
    tar.stdout.close()
    gzip.communicate()
...
if identical:
    (OUT / "PHASE28_FINAL_SHA256.txt").write_text(f"{hashes[0]}  {NAME}.zip\n")
