#!/usr/bin/env python3
"""Phase 34 binary-safe verification: delta hashes or complete scoped tree."""
...
SELF = f"{SELF_PREFIX}PHASE_34_EXPECTED_FULL_SHA256.json"
...
def later_patch(rel: str) -> bool:
    ...
    return bool(match and int(match.group(1)) > 34)
...
def patch_mode(root: pathlib.Path, here: pathlib.Path) -> dict[str, object]:
    ...
    return {
        "schema": "aethercore.phase34.binary-safe-verify.v1",
        "mode": "patch",
        "root": str(root),
        "checked": checked,
...
def full_tree_mode(root: pathlib.Path, here: pathlib.Path) -> dict[str, object]:
    entries = json.loads(
        (here / "PHASE_34_EXPECTED_FULL_SHA256.json").read_text(encoding="utf-8")
    ).get("files", {})
    problems: list[str] = []
    checked = 0
    seen: set[str] = set()
