...
#[serde(rename_all="camelCase")]
struct UiKernelEvent { sequence:u64, emitted_unix_ms:i64, kind:String, plan_id:String, payload:serde_json::Value }
#[derive(Clone, Serialize)]
#[serde(rename_all="camelCase")]
struct UiSessionState { connected:bool, session_id:String, service_version:String, current_sequence:u64, replay_floor_sequence:u64, replay_complete:bool }
#[derive(Clone, Serialize)]
#[serde(rename_all="camelCase")]
struct UiStreamReset { reason:String, current_sequence:u64, replay_floor_sequence:u64, message_key:String }
#[derive(Clone, Serialize)]
#[serde(rename_all="camelCase")]
struct SupportExportResult { path:String, sha256:String, verification_fingerprint_sha256:String }

#[cfg(windows)]
fn session_slot() -> &'static Mutex<Option<Arc<aethercore_ipc::SessionClient>>> { SESSION.get_or_init(||Mutex::new(None)) }
#[cfg(windows)]
fn connect_lock() -> &'static Mutex<()> { CONNECT_LOCK.get_or_init(||Mutex::new(())) }

#[cfg(windows)]
fn normalize_event(event:v1::EventEnvelope)->UiKernelEvent {
    use v1::event_envelope::Payload;
    let (kind,payload)=match event.payload {
        Some(Payload::ServiceSnapshot(v))=>("serviceSnapshot",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::Plan(v))=>("plan",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::DriverHubSnapshot(v))=>("driverHubSnapshot",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::DriverInstallStatus(v))=>("driverInstallStatus",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::RecoveryHistory(v))=>("recoveryHistory",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::RepairAssessment(v))=>("repairAssessment",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::SystemRepairStatus(v))=>("systemRepairStatus",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::CleanupSnapshot(v))=>("cleanupSnapshot",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::CleanupStatus(v))=>("cleanupStatus",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::StartupSnapshot(v))=>("startupSnapshot",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::StartupStatus(v))=>("startupStatus",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::StartupHistory(v))=>("startupHistory",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::DiagnosticsSnapshot(v))=>("diagnosticsSnapshot",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::DiagnosticsHistory(v))=>("diagnosticsHistory",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::ConsentIntent(v))=>("consentIntent",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::MutationLease(v))=>("mutationLease",serde_json::to_value(v).unwrap_or_default()),
        Some(Payload::ProgressTelemetry(v))=>("progressTelemetry",serde_json::to_value(v).unwrap_or_default()),
...
fn emit_session_state(connected:bool, client:Option<&aethercore_ipc::SessionClient>) {
    if let Some(app)=APP_HANDLE.get(){
        let state=UiSessionState{connected,session_id:client.map(|c|c.hello.session_id.clone()).unwrap_or_default(),service_version:client.map(|c|c.hello.service_version.clone()).unwrap_or_default(),current_sequence:client.map(|c|c.hello.current_sequence).unwrap_or_else(||LAST_EVENT_SEQUENCE.load(Ordering::Acquire)),replay_floor_sequence:client.map(|c|c.hello.replay_floor_sequence).unwrap_or_default(),replay_complete:client.map(|c|c.hello.replay_complete).unwrap_or(false)};
...
fn connect_persistent_session()->anyhow::Result<Arc<aethercore_ipc::SessionClient>> {
    ...
    let on_event:Arc<dyn Fn(v1::EventEnvelope)+Send+Sync>=Arc::new(|event|{
        ...
        if let Some(app)=APP_HANDLE.get(){let _=app.emit("aethercore://kernel-event",normalize_event(event));}
    });
    let on_stream_reset:Arc<dyn Fn(v1::StreamReset)+Send+Sync>=Arc::new(|reset|{
        ...
        if let Some(app)=APP_HANDLE.get(){
            ...
            let _=app.emit("aethercore://stream-reset",UiStreamReset{reason,current_sequence:reset.current_sequence,replay_floor_sequence:reset.replay_floor_sequence,message_key:reset.message_key});
        }
    });
    let on_disconnect:Arc<dyn Fn()+Send+Sync>=Arc::new(||{
        ...
        let current_is_live=session_slot().lock().ok().and_then(|slot|slot.as_ref().map(|c|c.is_alive())).unwrap_or(false);
        if !current_is_live { emit_session_state(false,None); }
    });
    let client=aethercore_ipc::SessionClient::connect("aethercore-desktop",env!("CARGO_PKG_VERSION"),replay,on_event,on_stream_reset,on_disconnect)?;
...
fn active_session()->anyhow::Result<Arc<aethercore_ipc::SessionClient>> {
    if let Some(existing)=session_slot().lock().map_err(|_|anyhow::anyhow!("session lock poisoned"))?.as_ref().filter(|c|c.is_alive()).cloned(){return Ok(existing)}
    let _connect_guard=connect_lock().lock().map_err(|_|anyhow::anyhow!("session connect lock poisoned"))?;
    ...
    *session_slot().lock().map_err(|_|anyhow::anyhow!("session lock poisoned"))?=Some(client.clone());
    Ok(client)
}
...
fn invalidate_session_if_current(failed: &Arc<aethercore_ipc::SessionClient>) {
    if let Ok(mut slot) = session_slot().lock() {
        if slot.as_ref().is_some_and(|current| Arc::ptr_eq(current, failed)) {
            *slot = None;
        }
    }
}
...
fn request(payload: request::Payload) -> anyhow::Result<v1::Response> {
    ...
    let resp={
        ...
        for _ in 0..2 {
            ...
            match client.request(req.clone(),Duration::from_secs(15)).map_err(anyhow::Error::from) {
                Ok(v)=>{response=Some(v);break},
                Err(e)=>{
                    last_error=Some(e);
        ...
        response.ok_or_else(||last_error.unwrap_or_else(||anyhow::anyhow!("service session unavailable")))?
    };
    #[cfg(not(windows))]
    let resp=aethercore_ipc::connect(&req)?;
    if let Some(error)=resp.error.as_ref(){if error.code!=v1::ErrorCode::Unspecified as i32{anyhow::bail!(error.message_key.clone());}}
    if resp.status_code != 0 { anyhow::bail!(resp.error_message.clone()); }
    Ok(resp)
}
    ...
    #[cfg(windows)]
    {
        ...
        let client = tauri::async_runtime::spawn_blocking(|| {
            ...
            let hydrate = Request {
                ...
                payload: Some(request::Payload::HydrateSession(v1::HydrateSessionRequest {})),
            };
...
async fn start_driver_scan() -> Result<v1::DriverHubSnapshot, String> {
    tauri::async_runtime::spawn_blocking(|| {
        let resp = request(request::Payload::StartDriverScan(v1::StartDriverScanRequest {}))
            .map_err(|e| e.to_string())?;
        extract_driver_hub(resp)
    })
    .await
...
#[command]
async fn set_driver_candidate_policy(scan_id: String, inventory_epoch: u64, candidate_id: String, policy: String) -> Result<v1::DriverHubSnapshot, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let resp = request(request::Payload::SetDriverCandidatePolicy(v1::SetDriverCandidatePolicyRequest {
            scan_id, inventory_epoch, candidate_id, policy,
        })).map_err(|e| e.to_string())?;
        extract_driver_hub(resp)
...
async fn create_driver_install_plan(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let resp = request(request::Payload::CreateDriverInstallPlan(
            v1::CreateDriverInstallPlanRequest { scan_id, inventory_epoch, candidate_ids },
        )).map_err(|e| e.to_string())?;
        extract_plan(resp)
    }).await.map_err(|e| e.to_string())?
}
...
#[command]
async fn get_driver_install_status(plan_id: String) -> Result<Option<v1::DriverInstallStatus>, String> {
    tauri::async_runtime::spawn_blocking(move || {
...
async fn create_system_repair_plan(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let resp = request(request::Payload::CreateSystemRepairPlan(
            v1::CreateSystemRepairPlanRequest { assessment_id, run_disk_scan },
        )).map_err(|e| e.to_string())?;
        extract_plan(resp)
    }).await.map_err(|e| e.to_string())?
}
...
#[command]
async fn get_system_repair_status(plan_id: String) -> Result<Option<v1::SystemRepairStatus>, String> {
    tauri::async_runtime::spawn_blocking(move || {
...
async fn create_cleanup_plan(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let resp = request(request::Payload::CreateCleanupPlan(
            v1::CreateCleanupPlanRequest { scan_id, inventory_epoch, candidate_ids },
        )).map_err(|e| e.to_string())?;
        extract_plan(resp)
    }).await.map_err(|e| e.to_string())?
}
...
async fn start_cleanup(plan_id: String) -> Result<v1::CleanupStatus, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let resp = request(request::Payload::StartCleanup(
            v1::StartCleanupRequest { plan_id },
        )).map_err(|e| e.to_string())?;
...
fn extract_system_repair_status(resp: v1::Response) -> Result<Option<v1::SystemRepairStatus>, String> {
    match resp.payload {
...
async fn start_startup_scan() -> Result<v1::StartupSnapshot, String> {
    tauri::async_runtime::spawn_blocking(|| {
        let resp = request(request::Payload::StartStartupScan(v1::StartStartupScanRequest {})).map_err(|e| e.to_string())?;
...
async fn get_startup_snapshot() -> Result<v1::StartupSnapshot, String> {
    tauri::async_runtime::spawn_blocking(|| {
        let resp = request(request::Payload::GetStartupSnapshot(v1::GetStartupSnapshotRequest {})).map_err(|e| e.to_string())?;
        extract_startup_snapshot(resp)
    }).await.map_err(|e| e.to_string())?
}

#[command]
async fn create_startup_plan(scan_id:String, inventory_epoch:u64, decisions:Vec<v1::StartupDecisionInput>, confirm_service_changes:bool) -> Result<v1::PlanSnapshot,String> {
    tauri::async_runtime::spawn_blocking(move || {
        let resp=request(request::Payload::CreateStartupPlan(v1::CreateStartupPlanRequest{scan_id,inventory_epoch,decisions,confirm_service_changes})).map_err(|e|e.to_string())?;extract_plan(resp)
    }).await.map_err(|e|e.to_string())?
}

#[command]
async fn create_startup_restore_plan(change_id:String, confirm_service_changes:bool) -> Result<v1::PlanSnapshot,String> {
    tauri::async_runtime::spawn_blocking(move || {
        let resp=request(request::Payload::CreateStartupRestorePlan(v1::CreateStartupRestorePlanRequest{change_id,confirm_service_changes})).map_err(|e|e.to_string())?;extract_plan(resp)
    }).await.map_err(|e|e.to_string())?
}
...
async fn start_startup_changes(plan_id:String) -> Result<v1::StartupStatus,String> {
    tauri::async_runtime::spawn_blocking(move || {
        let resp=request(request::Payload::StartStartupChanges(v1::StartStartupChangesRequest{plan_id})).map_err(|e|e.to_string())?;extract_startup_status(resp)?.ok_or("startup status missing".into())
    }).await.map_err(|e|e.to_string())?
}
...
async fn get_startup_status(plan_id:String) -> Result<Option<v1::StartupStatus>,String> {
    tauri::async_runtime::spawn_blocking(move || {
        let resp=request(request::Payload::GetStartupStatus(v1::GetStartupStatusRequest{plan_id})).map_err(|e|e.to_string())?;extract_startup_status(resp)
    }).await.map_err(|e|e.to_string())?
}
...
async fn get_startup_history(limit:u32) -> Result<Vec<v1::StartupHistoryEntryInfo>,String> {
    tauri::async_runtime::spawn_blocking(move || {
        let resp=request(request::Payload::GetStartupHistory(v1::GetStartupHistoryRequest{limit})).map_err(|e|e.to_string())?;match resp.payload{Some(response::Payload::StartupHistory(p))=>Ok(p.entries),_=>Err("unexpected startup history response".into())}
    }).await.map_err(|e|e.to_string())?
}
...
async fn start_diagnostics_scan() -> Result<v1::DiagnosticsSnapshot,String> {
    tauri::async_runtime::spawn_blocking(||{
        let resp=request(request::Payload::StartDiagnosticsScan(v1::StartDiagnosticsScanRequest{})).map_err(|e|e.to_string())?;
...
async fn get_diagnostics_snapshot() -> Result<v1::DiagnosticsSnapshot,String> {
    tauri::async_runtime::spawn_blocking(||{
        let resp=request(request::Payload::GetDiagnosticsSnapshot(v1::GetDiagnosticsSnapshotRequest{})).map_err(|e|e.to_string())?;
        extract_diagnostics_snapshot(resp)
    }).await.map_err(|e|e.to_string())?
}

#[command]
async fn get_diagnostics_history(limit:u32) -> Result<Vec<v1::DiagnosticHistoryEntryInfo>,String> {
    tauri::async_runtime::spawn_blocking(move||{
        let resp=request(request::Payload::GetDiagnosticsHistory(v1::GetDiagnosticsHistoryRequest{limit})).map_err(|e|e.to_string())?;
        match resp.payload{Some(response::Payload::DiagnosticsHistory(p))=>Ok(p.entries),_=>Err("unexpected diagnostics history response".into())}
    }).await.map_err(|e|e.to_string())?
}

fn extract_diagnostics_snapshot(resp:v1::Response)->Result<v1::DiagnosticsSnapshot,String>{
    match resp.payload{Some(response::Payload::DiagnosticsSnapshot(p))=>p.snapshot.ok_or("missing diagnostics snapshot".into()),_=>Err("unexpected diagnostics response".into())}
}
...
async fn start_deep_scan()->Result<v1::DeepScanSnapshot,String>{
    tauri::async_runtime::spawn_blocking(||{
        let resp=request(request::Payload::StartDeepScan(v1::StartDeepScanRequest{})).map_err(|e|e.to_string())?;
...
async fn cancel_deep_scan(scan_id:String)->Result<v1::DeepScanSnapshot,String>{
    tauri::async_runtime::spawn_blocking(move||{
        let resp=request(request::Payload::CancelDeepScan(v1::CancelDeepScanRequest{scan_id})).map_err(|e|e.to_string())?;
        extract_deep_scan_snapshot(resp)
    }).await.map_err(|e|e.to_string())?
}
...
async fn get_deep_scan_snapshot()->Result<v1::DeepScanSnapshot,String>{
    tauri::async_runtime::spawn_blocking(||{
        let resp=request(request::Payload::GetDeepScanSnapshot(v1::GetDeepScanSnapshotRequest{})).map_err(|e|e.to_string())?;
        extract_deep_scan_snapshot(resp)
    }).await.map_err(|e|e.to_string())?
}
...
async fn get_deep_scan_history(limit:u32)->Result<Vec<v1::DeepScanHistoryEntry>,String>{
    tauri::async_runtime::spawn_blocking(move||{
        let resp=request(request::Payload::GetDeepScanHistory(v1::GetDeepScanHistoryRequest{limit})).map_err(|e|e.to_string())?;
        match resp.payload{Some(response::Payload::DeepScanHistory(v))=>Ok(v.entries),_=>Err("unexpected deep scan history response".into())}
    }).await.map_err(|e|e.to_string())?
}

#[command]
async fn seal_remediation_plan(scan_id:String,selected_action_ids:Vec<String>)->Result<v1::PcRemediationPlan,String>{
    tauri::async_runtime::spawn_blocking(move||{
        let resp=request(request::Payload::SealRemediationPlan(v1::SealRemediationPlanRequest{scan_id,selected_action_ids})).map_err(|e|e.to_string())?;
        match resp.payload{Some(response::Payload::PcRemediationPlan(v))=>v.plan.ok_or("missing remediation plan".into()),_=>Err("unexpected remediation plan response".into())}
    }).await.map_err(|e|e.to_string())?
}

fn extract_deep_scan_snapshot(resp:v1::Response)->Result<v1::DeepScanSnapshot,String>{
    match resp.payload{Some(response::Payload::DeepScanSnapshot(v))=>v.snapshot.ok_or("missing deep scan snapshot".into()),_=>Err("unexpected deep scan response".into())}
}
...
async fn get_update_snapshot()->Result<v1::UpdateSnapshot,String>{
    tauri::async_runtime::spawn_blocking(||{let response=request(request::Payload::GetUpdateSnapshot(v1::GetUpdateSnapshotRequest{})).map_err(|e|e.to_string())?;extract_update_snapshot(response)}).await.map_err(|e|e.to_string())?
}
...
fn check_for_updates_blocking(channel:i32)->anyhow::Result<v1::UpdateSnapshot>{
    ...
    let response=request(request::Payload::GetUpdateCheckDescriptor(v1::GetUpdateCheckDescriptorRequest{channel}))?;
    ...
    if descriptor.channel!=channel || descriptor.max_manifest_bytes==0 || descriptor.max_signature_bytes==0{
        anyhow::bail!("update.error.trust");
    }
    let transport=HttpsTransport::new().map_err(update_transport_error)?;
    let manifest=transport.get_bytes(&descriptor.manifest_url,descriptor.max_manifest_bytes as usize).map_err(update_transport_error)?;
    let signature=transport.get_bytes(&descriptor.signature_url,descriptor.max_signature_bytes as usize).map_err(update_transport_error)?;
    let response=request(request::Payload::SubmitUpdateManifest(v1::SubmitUpdateManifestRequest{channel,manifest_bytes:manifest,signature_bytes:signature}))?;
...
fn stage_update_blocking(release_id:&str)->anyhow::Result<v1::UpdateSnapshot>{
    ...
    let response=request(request::Payload::BeginUpdateStageUpload(v1::BeginUpdateStageUploadRequest{release_id:release_id.into()}))?;
    let descriptor=match response.payload{
        Some(response::Payload::UpdateStageUploadDescriptor(v))=>v,
        _=>anyhow::bail!("update.error.unavailable"),
    };
    if descriptor.upload_id.is_empty() || descriptor.package_url.is_empty() || descriptor.expected_size==0 || descriptor.expected_size>MAX_UPDATE_PACKAGE_BYTES || descriptor.expected_sha256.len()!=64 || !descriptor.expected_sha256.bytes().all(|b|b.is_ascii_hexdigit()) || descriptor.max_chunk_bytes==0 || descriptor.max_chunk_bytes as usize>MAX_STAGE_CHUNK_BYTES as usize{
    ...
    let result=(||->anyhow::Result<v1::UpdateSnapshot>{
        ...
        let transport=HttpsTransport::new().map_err(update_transport_error)?;
        let actual_hash=transport.download_to_file(
        ...
        if !actual_hash.eq_ignore_ascii_case(&descriptor.expected_sha256){anyhow::bail!("update.error.integrity")}
        ...
        loop{
            ...
            if read==0{break}
            let response=request(request::Payload::WriteUpdateStageChunk(v1::WriteUpdateStageChunkRequest{upload_id:descriptor.upload_id.clone(),offset,data:buffer[..read].to_vec()}))?;
            let _=extract_update_snapshot(response).map_err(anyhow::Error::msg)?;
            offset=offset.checked_add(read as u64).ok_or_else(||anyhow::anyhow!("update.error.integrity"))?;
        }
        if offset!=descriptor.expected_size{anyhow::bail!("update.error.integrity")}
        drop(file);
        let response=request(request::Payload::FinalizeUpdateStageUpload(v1::FinalizeUpdateStageUploadRequest{upload_id:descriptor.upload_id.clone()}))?;
        let snapshot=extract_update_snapshot(response).map_err(anyhow::Error::msg)?;
        cleanup.remove_now();
        Ok(snapshot)
    })();
    if result.is_err(){let _=cancel_update_upload(&descriptor.upload_id);}
    result
}

fn cancel_update_upload(upload_id:&str)->anyhow::Result<()> {
    if upload_id.is_empty(){return Ok(())}
    let _=request(request::Payload::CancelUpdateStageUpload(v1::CancelUpdateStageUploadRequest{upload_id:upload_id.into()}))?;
    Ok(())
}

fn cancel_update_install_intent(intent_id:&str)->anyhow::Result<()> {
    if intent_id.is_empty(){return Ok(())}
    let _=request(request::Payload::CancelUpdateInstallIntent(v1::CancelUpdateInstallIntentRequest{intent_id:intent_id.into()}))?;
    Ok(())
}

fn user_update_cache_dir()->anyhow::Result<std::path::PathBuf>{
    let root=std::env::var_os("LOCALAPPDATA").map(std::path::PathBuf::from).ok_or_else(||anyhow::anyhow!("update.error.internal"))?;
    Ok(root.join("AetherCore").join("UpdateCache"))
}

struct LocalTempFile{path:std::path::PathBuf,removed:bool}
impl LocalTempFile{
    fn new(path:std::path::PathBuf)->Self{Self{path,removed:false}}
    fn remove_now(&mut self){match std::fs::remove_file(&self.path){Ok(())=>self.removed=true,Err(e)if e.kind()==std::io::ErrorKind::NotFound=>self.removed=true,Err(_)=>{}}}
}
impl Drop for LocalTempFile{fn drop(&mut self){if !self.removed{let _=std::fs::remove_file(&self.path);}}}
...
fn update_transport_error(error:aethercore_update_download::DownloadError)->anyhow::Error{
    ...
    let key=match error{
        DownloadError::Network(_) | DownloadError::HttpStatus(_) | DownloadError::Cancelled=>"update.error.network",
        DownloadError::InvalidHttpsUrl | DownloadError::ResponseTooLarge | DownloadError::SizeMismatch=>"update.error.integrity",
...
async fn install_staged_update(release_id:String,locale:String)->Result<(),String>{
    tauri::async_runtime::spawn_blocking(move||install_update_blocking(&release_id,&locale)).await.map_err(|e|e.to_string())?.map_err(|e|e.to_string())
}
#[command]
async fn create_support_bundle_preview(include_hardware:bool,include_crash_metadata:bool,include_operation_history:bool,include_scheduler_activity:bool)->Result<v1::SupportBundlePreview,String>{
    tauri::async_runtime::spawn_blocking(move||{let response=request(request::Payload::CreateSupportBundlePreview(v1::CreateSupportBundlePreviewRequest{include_hardware,include_crash_metadata,include_operation_history,include_scheduler_activity})).map_err(|e|e.to_string())?;match response.payload{Some(response::Payload::SupportBundlePreview(v))=>v.preview.ok_or("missing support preview".into()),_=>Err("unexpected support preview response".into())}}).await.map_err(|e|e.to_string())?
}
#[command]
async fn export_support_bundle(preview_id:String)->Result<SupportExportResult,String>{
    tauri::async_runtime::spawn_blocking(move||export_support_bundle_blocking(&preview_id)).await.map_err(|e|e.to_string())?.map_err(|e|e.to_string())
}
fn extract_update_snapshot(response:v1::Response)->Result<v1::UpdateSnapshot,String>{match response.payload{Some(response::Payload::UpdateSnapshot(v))=>v.snapshot.ok_or("missing update snapshot".into()),_=>Err("unexpected update response".into())}}

#[cfg(windows)]
fn install_update_blocking(release_id:&str,locale:&str)->anyhow::Result<()>{
    use aethercore_windows_foundation::OwnedHandle;
    use windows::{Win32::{System::Threading::{GetExitCodeProcess,INFINITE,WaitForSingleObject},UI::{Shell::{SEE_MASK_NOCLOSEPROCESS,SHELLEXECUTEINFOW,ShellExecuteExW},WindowsAndMessaging::SW_SHOWNORMAL}},core::PCWSTR};
    let response=request(request::Payload::BeginUpdateInstallIntent(v1::BeginUpdateInstallIntentRequest{release_id:release_id.into()}))?;
    let intent_id=match response.payload{Some(response::Payload::UpdateInstallIntent(v))=>v.intent_id,_=>anyhow::bail!("update.error.stateConflict")};
    let broker=locate_update_broker()?;let locale=if locale.eq_ignore_ascii_case("ar"){"ar"}else{"en"};let params=format!("--intent-id {intent_id} --locale {locale}");
    let verb=wide("runas");let file=wide_os(broker.as_os_str());let parameters=wide(&params);let mut info=SHELLEXECUTEINFOW{cbSize:std::mem::size_of::<SHELLEXECUTEINFOW>() as u32,fMask:SEE_MASK_NOCLOSEPROCESS,lpVerb:PCWSTR(verb.as_ptr()),lpFile:PCWSTR(file.as_ptr()),lpParameters:PCWSTR(parameters.as_ptr()),nShow:SW_SHOWNORMAL.0,..Default::default()};
    let broker_result=(||->anyhow::Result<()> { unsafe{ShellExecuteExW(&mut info)?;if info.hProcess.is_invalid(){anyhow::bail!("update.error.brokerRejected")}let process=OwnedHandle::new(info.hProcess);WaitForSingleObject(process.get(),INFINITE);let mut exit=1u32;GetExitCodeProcess(process.get(),&mut exit)?;if exit==2{anyhow::bail!("update.error.cancelled")}if exit!=0{anyhow::bail!("update.error.installFailed")}} Ok(()) })();
    if broker_result.is_err(){let _=cancel_update_install_intent(&intent_id);}
    broker_result
}
#[cfg(not(windows))] fn install_update_blocking(_: &str,_:&str)->anyhow::Result<()>{anyhow::bail!("Windows only")}
#[cfg(windows)]
fn locate_update_broker()->anyhow::Result<std::path::PathBuf>{let desktop=std::env::current_exe()?;let sibling=desktop.parent().ok_or_else(||anyhow::anyhow!("desktop executable has no parent"))?.join("aethercore-update-broker.exe");if std::env::var_os("AETHERCORE_DATA_DIR").is_some()&&sibling.is_file(){return Ok(sibling)}if let Some(program_files)=std::env::var_os("ProgramFiles"){let installed=std::path::PathBuf::from(program_files).join("AetherCore").join("aethercore-update-broker.exe");if installed.is_file(){return Ok(installed)}}if sibling.is_file(){return Ok(sibling)}anyhow::bail!("update.error.brokerPath")}

fn export_support_bundle_blocking(preview_id:&str)->anyhow::Result<SupportExportResult>{
    use sha2::{Digest,Sha256};use std::io::Write;
    let response=request(request::Payload::PrepareSupportBundle(v1::PrepareSupportBundleRequest{preview_id:preview_id.into()}))?;
    let ready=match response.payload{Some(response::Payload::SupportBundleReady(v))=>v.bundle.ok_or_else(||anyhow::anyhow!("support.error.unavailable"))?,_=>anyhow::bail!("support.error.unavailable")};
    if !ready.file_name.starts_with("AetherCore-Support-")||!ready.file_name.ends_with(".aetherdiag")||ready.file_name.contains('/')||ready.file_name.contains('\\'){anyhow::bail!("support.error.invalid")}
    if ready.public_key_hex.len()!=64 || !ready.public_key_hex.bytes().all(|b|b.is_ascii_hexdigit()) || ready.public_key_fingerprint_sha256.len()!=64 || !ready.public_key_fingerprint_sha256.bytes().all(|b|b.is_ascii_hexdigit()){anyhow::bail!("support.error.integrity")}
    let public_key=decode_hex_32(&ready.public_key_hex).ok_or_else(||anyhow::anyhow!("support.error.integrity"))?;
    let computed_fingerprint=hex_lower(Sha256::digest(public_key).as_slice());
    if !computed_fingerprint.eq_ignore_ascii_case(&ready.public_key_fingerprint_sha256){anyhow::bail!("support.error.integrity")}
    let root=std::env::var_os("USERPROFILE").map(std::path::PathBuf::from).ok_or_else(||anyhow::anyhow!("support.error.destination"))?;
    let downloads=root.join("Downloads");std::fs::create_dir_all(&downloads)?;
    ...
    let result=(||->anyhow::Result<SupportExportResult>{
        let mut hash=Sha256::new();let mut offset=0u64;
        loop{
            let response=request(request::Payload::ReadSupportBundleChunk(v1::ReadSupportBundleChunkRequest{bundle_id:ready.bundle_id.clone(),offset,max_bytes:256*1024}))?;
            let chunk=match response.payload{Some(response::Payload::SupportBundleChunk(v))=>v,_=>anyhow::bail!("support.error.unavailable")};
            if chunk.offset!=offset||chunk.total_size!=ready.size_bytes{anyhow::bail!("support.error.integrity")}
            file.write_all(&chunk.data)?;hash.update(&chunk.data);offset=offset.checked_add(chunk.data.len() as u64).ok_or_else(||anyhow::anyhow!("support.error.integrity"))?;
            if chunk.eof{break}
        }
        file.sync_all()?;drop(file);
        let actual_sha256=hex_lower(hash.finalize().as_slice());
        if offset!=ready.size_bytes||!actual_sha256.eq_ignore_ascii_case(&ready.sha256){anyhow::bail!("support.error.integrity")}
        let archive_bytes=std::fs::read(&temporary)?;
        aethercore_support_bundle::verify_archive(&archive_bytes,&ready.public_key_fingerprint_sha256).map_err(|_|anyhow::anyhow!("support.error.integrity"))?;
        std::fs::rename(&temporary,&path)?;
        let _=request(request::Payload::MarkSupportBundleExported(v1::MarkSupportBundleExportedRequest{bundle_id:ready.bundle_id.clone()}));
        let _=request(request::Payload::DiscardSupportBundle(v1::DiscardSupportBundleRequest{bundle_id:ready.bundle_id.clone()}));
        Ok(SupportExportResult{path:path.to_string_lossy().into_owned(),sha256:actual_sha256,verification_fingerprint_sha256:ready.public_key_fingerprint_sha256.clone()})
    })();
    if result.is_err(){let _=std::fs::remove_file(&temporary);let _=request(request::Payload::DiscardSupportBundle(v1::DiscardSupportBundleRequest{bundle_id:ready.bundle_id.clone()}));}
    result
}

fn non_overwriting_support_path(downloads:&std::path::Path,file_name:&str)->anyhow::Result<std::path::PathBuf>{
    let direct=downloads.join(file_name);if !direct.exists(){return Ok(direct)}
    let stem=file_name.strip_suffix(".aetherdiag").ok_or_else(||anyhow::anyhow!("support.error.invalid"))?;
    for index in 1..=999u16{let candidate=downloads.join(format!("{stem}-{index}.aetherdiag"));if !candidate.exists(){return Ok(candidate)}}
    anyhow::bail!("support.error.destination")
}

fn decode_hex_32(value:&str)->Option<[u8;32]>{
    if value.len()!=64{return None}let mut out=[0u8;32];let bytes=value.as_bytes();
    for index in 0..32{let hi=hex_nibble(bytes[index*2])?;let lo=hex_nibble(bytes[index*2+1])?;out[index]=(hi<<4)|lo;}Some(out)
}
...
fn hex_lower(bytes:&[u8])->String{const HEX:&[u8;16]=b"0123456789abcdef";let mut out=String::with_capacity(bytes.len()*2);for b in bytes{out.push(HEX[(b>>4) as usize] as char);out.push(HEX[(b&0xf) as usize] as char);}out}

fn extract_startup_snapshot(resp:v1::Response)->Result<v1::StartupSnapshot,String>{match resp.payload{Some(response::Payload::StartupSnapshot(p))=>p.snapshot.ok_or("missing startup snapshot".into()),_=>Err("unexpected startup snapshot response".into())}}
fn extract_startup_status(resp:v1::Response)->Result<Option<v1::StartupStatus>,String>{match resp.payload{Some(response::Payload::StartupStatus(p))=>Ok(p.status),_=>Err("unexpected startup status response".into())}}
...
fn open_gpu_vendor_support_blocking(vendor: &str) -> anyhow::Result<()> {
    ...
    let vendor = GpuVendor::parse(vendor).ok_or_else(|| anyhow::anyhow!("unsupported GPU vendor"))?;
...
fn authorize_blocking(plan_id: &str, locale: &str) -> anyhow::Result<()> {
    ...
    let locale = if locale.eq_ignore_ascii_case("ar") { "ar" } else { "en" };
    ...
    let result = tauri::Builder::default()
        .setup(|app| {
            #[cfg(windows)] {
                ...
                std::thread::Builder::new().name("aether-desktop-ipc-reconnect".into()).spawn(|| loop {
                    let desired=SESSION_DESIRED.load(Ordering::Acquire);
                    let needs_connect=session_slot().lock().map(|slot|slot.as_ref().is_none_or(|c|!c.is_alive())).unwrap_or(true);
                    if desired && needs_connect { let _=active_session(); }
