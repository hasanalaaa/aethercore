...
fn run_windows() -> Result<()> {
    ...
    if args.len() != 5 { bail!("unexpected consent broker arguments"); }
    ...
    let response = call(request::Payload::ApproveConsentIntent(
        v1::ApproveConsentIntentRequest { intent_id: intent_id.clone() },
    ))?;
...
fn fetch_intent(intent_id: &str) -> Result<v1::ConsentIntentResponse> {
    let response = call(request::Payload::GetConsentIntent(v1::GetConsentIntentRequest {
        intent_id: intent_id.to_owned(),
    }))?;
    match response.payload {
        Some(response::Payload::ConsentIntent(intent)) if intent.intent_id == intent_id => Ok(intent),
...
fn call(payload: request::Payload) -> Result<v1::Response> {
    ...
    if response.status_code != 0 {
        bail!("consent request rejected: {}", response.error_message);
    }
    Ok(response)
}
...
fn confirm_intent(intent: &v1::ConsentIntentResponse, locale: &str) -> Result<bool> {
    ...
    let flags = if arabic { base_flags | MB_RIGHT | MB_RTLREADING } else { base_flags };
...
fn localized_operation_ar(title: &str) -> String {
    if title.starts_with("Install ") { return "تثبيت تعريفات Windows التي تمت مراجعتها".into(); }
    if title == "Repair Windows integrity" { return "إصلاح تكامل Windows وفق الخطة التي تمت مراجعتها".into(); }
    if title.starts_with("Clean ") { return "تنظيف العناصر التي تمت مراجعتها ضمن قائمة السماح".into(); }
    if title.starts_with("Restore ") { return "استعادة تغييرات بدء التشغيل التي تمت مراجعتها".into(); }
    if title.starts_with("Optimize ") { return "تطبيق تغييرات بدء التشغيل التي تمت مراجعتها".into(); }
...
fn localized_risk_ar(risk_code: i32, fallback: &str) -> &'static str {
    match v1::RiskLevel::try_from(risk_code).ok() {
        ...
        _ if fallback.eq_ignore_ascii_case("low") || fallback.eq_ignore_ascii_case("green") => "منخفض",
        _ if fallback.eq_ignore_ascii_case("high") || fallback.eq_ignore_ascii_case("red") => "مرتفع",
...
fn format_number_ar(value: u64) -> String {
    ...
    value.to_string().chars().map(|c| c.to_digit(10).map(|d| DIGITS[d as usize]).unwrap_or(c)).collect()
}
