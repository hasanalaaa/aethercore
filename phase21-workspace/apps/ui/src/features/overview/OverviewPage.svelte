...
<section class="hero-card">
  ...
  <div class="hero-metrics"><div><span>{t('overview.protocol',locale)}</span><strong class="technical-isolate" dir="ltr">v7</strong></div><div><span>{t('overview.journal',locale)}</span><strong>{snapshot.journalEventCount}</strong></div><div><span>{t('overview.driverScan',locale)}</span><strong>{hub.state === 'Ready' ? hub.summary.deviceCount : '—'}</strong></div></div>
</section>
...
<section class="grid">
  ...
  <article class="panel driver-preview">
    ...
    <div class="preview-number">{hub.state === 'Ready' ? hub.summary.selectableUpdateCount : '—'}</div>
