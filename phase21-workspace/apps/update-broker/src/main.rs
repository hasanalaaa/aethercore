...
fn main() -> Result<()> {
    #[cfg(not(windows))]
    bail!("AetherCore update broker supports Windows only");
    #[cfg(windows)]
    return run_windows();
}
...
fn run_windows() -> Result<()> {
    ...
    if args.len() != 5 {
        bail!("unexpected update broker arguments")
    }
    ...
    let release = ticket
        .release
        .as_ref()
        .context("update ticket release missing")?;
    if release.release_id
        != intent
            .release
            .as_ref()
            .context("update intent release missing")?
            .release_id
    {
        bail!("update release identity changed")
    }
    let path = std::path::PathBuf::from(&ticket.staged_path);
    aethercore_update_engine::verify_file_hash_size(
        &path,
    ...
    let exit_code = match Command::new(&path).status() {
        Ok(status) => status.code().unwrap_or(-1),
        Err(_) => -1,
    };
    complete_with_retry(&ticket.ticket_id, exit_code)?;
    if !matches!(exit_code, 0 | 3010) {
        bail!("verified AetherCore installer failed with exit code {exit_code}")
    }
    Ok(())
}
...
fn fetch_intent(intent_id: &str) -> Result<v1::UpdateInstallIntentResponse> {
    let response = call(request::Payload::GetUpdateInstallIntent(
        v1::GetUpdateInstallIntentRequest {
            intent_id: intent_id.into(),
        },
    ))?;
    match response.payload {
        Some(response::Payload::UpdateInstallIntent(v)) if v.intent_id == intent_id => Ok(v),
        _ => bail!("unexpected update intent response"),
    }
}
#[cfg(windows)]
fn claim(intent_id: &str) -> Result<v1::UpdateExecutionTicketResponse> {
    let response = call(request::Payload::ClaimUpdateInstall(
        v1::ClaimUpdateInstallRequest {
            intent_id: intent_id.into(),
        },
    ))?;
    match response.payload {
        Some(response::Payload::UpdateExecutionTicket(v)) => Ok(v),
        _ => bail!("unexpected update execution ticket response"),
    }
}
#[cfg(windows)]
fn cancel_intent(intent_id: &str) -> Result<()> {
    let _ = call(request::Payload::CancelUpdateInstallIntent(
...
fn complete_with_retry(ticket_id: &str, exit_code: i32) -> Result<()> {
    ...
    loop {
        match call(request::Payload::CompleteUpdateInstall(
            v1::CompleteUpdateInstallRequest {
            ...
            Ok(response) => {
                return match response.payload {
                    Some(response::Payload::UpdateCompletion(v)) if v.ticket_id == ticket_id => {
                        Ok(())
                    }
                    _ => bail!("unexpected update completion response"),
                };
            }
            ...
            Err(error) => {
                return Err(error.context("maintenance service did not return after update"));
            }
        }
    }
}
...
fn confirm(intent: &v1::UpdateInstallIntentResponse, locale: &str) -> Result<bool> {
    ...
    let (title, body) = if locale == "ar" {
        (
            "AetherCore — تثبيت تحديث موثوق".to_string(),
            format!(
                "تم تنزيل تحديث AetherCore والتحقق منه محليًا.\n\nالإصدار: {}\nالحجم: {} ميغابايت\n\nسيشغّل وسيط التحديث حزمة التثبيت الموقعة التي أعدتها خدمة AetherCore فقط، وقد يعيد Windows تشغيل خدمة الصيانة أثناء الترقية. هل تريد المتابعة؟",
                release.version,
        ...
        (
            "AetherCore — Install Verified Update".to_string(),
            format!(
                "AetherCore downloaded and locally verified this update.\n\nVersion: {}\nSize: {} MB\n\nThe update broker will run only the signed installer staged by the AetherCore service. Windows may restart the maintenance service during the upgrade. Continue?",
                release.version,
                release.size_bytes / 1_048_576
            ),
        )
    ...
    let t = wide(&title);
    let b = wide(&body);
    let base = MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2 | MB_SETFOREGROUND;
    let flags = if locale == "ar" {
