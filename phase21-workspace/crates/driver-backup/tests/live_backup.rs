...
fn exports_a_bound_oem_driver_package_with_manifest() {
    let devices =
        aethercore_windows_pnp::scan_present_devices().expect("PnP inventory must succeed");
    let Some(inf) = devices
        .iter()
        ...
        eprintln!(
            "No present device with an OEM INF was found; backup probe has nothing applicable to export."
        );
        return;
    };
