...
use windows::{
    Win32::{
        ...
        Storage::FileSystem::{
            FILE_ATTRIBUTE_REPARSE_POINT, MOVEFILE_COPY_ALLOWED, MOVEFILE_WRITE_THROUGH,
            MoveFileExW,
        },
        System::{
            Com::{CLSCTX_INPROC_SERVER, CoCreateInstance},
            Registry::{
                HKEY, HKEY_LOCAL_MACHINE, HKEY_USERS, KEY_QUERY_VALUE, KEY_READ, KEY_SET_VALUE,
                KEY_WOW64_32KEY, KEY_WOW64_64KEY, REG_EXPAND_SZ, REG_MULTI_SZ, REG_SAM_FLAGS,
                REG_SZ, REG_VALUE_TYPE, RegCloseKey, RegDeleteValueW, RegEnumKeyExW, RegEnumValueW,
...
impl StartupPlatform for WindowsStartupPlatform {
    ...
    fn scan(&self) -> Result<(Vec<StartupItem>, Vec<String>)> {
        ...
        if let Err(e) = scan_scheduled_tasks(&mut items) {
            warnings.push(format!("Task Scheduler inventory unavailable: {e}"));
        }
        if let Err(e) = scan_services(&mut items, &mut warnings) {
            warnings.push(format!("Service inventory unavailable: {e}"));
        }
        items.sort_by(|a, b| {
    ...
    fn current_state(&self, action: &StartupChangeAction) -> Result<String> {
        ...
        let state = match template {
            NativeState::RegistryValue {
                ...
                data_hex,
                ..
            } => query_registry_state(&hive, &key, &value_name, &view, value_type, &data_hex)?,
            ...
            NativeState::ScheduledTask {
                ...
                xml_sha256,
                ..
            } => query_task_state(&task_path, &xml_sha256)?,
    ...
    fn apply(&self, action: &StartupChangeAction) -> Result<()> {
        ...
        match (original, applied) {
            (
                ...
                if restoring {
                    set_registry_value(&hive, &key, &value_name, &view, value_type, &data_hex)
                } else {
                    delete_registry_value(&hive, &key, &value_name, &view)
                }
            }
            (
                NativeState::StartupFile { path, .. },
                NativeState::StartupFile { backup_path, .. },
            ) => {
                if restoring {
            ...
            (
                NativeState::ScheduledTask { task_path, .. },
                NativeState::ScheduledTask { enabled, .. },
            ) => set_task_enabled(&task_path, if restoring { true } else { enabled }),
            (
                NativeState::Service {
                    service_name,
                    start_type,
                ...
                NativeState::Service {
                    start_type: disabled_type,
                    delayed_auto: disabled_delayed,
                    ..
                },
            ) => {
                if restoring {
                    set_service_start(&service_name, start_type, delayed_auto)
                } else {
                    set_service_start(&service_name, disabled_type, disabled_delayed)
                }
            }
...
fn scan_registry_startup(out: &mut Vec<StartupItem>, warnings: &mut Vec<String>) -> Result<()> {
    for (key, kind) in [(RUN_KEY, "RegistryRun"), (RUNONCE_KEY, "RegistryRunOnce")] {
        for (view, label) in [(KEY_WOW64_64KEY, "64"), (KEY_WOW64_32KEY, "32")] {
            if let Err(e) = enum_registry_values(
                HKEY_LOCAL_MACHINE,
                "HKLM",
                key,
                view,
                ...
                warnings.push(format!("HKLM {key} ({label}-bit) unavailable: {e}"));
            }
        }
    }
            ...
            for (view, label) in [(KEY_WOW64_64KEY, "64"), (KEY_WOW64_32KEY, "32")] {
                if let Err(e) = enum_registry_values(
                    ...
                    warnings.push(format!(
                        "Loaded user startup hive {sid} ({label}-bit) unavailable: {e}"
                    ));
                }
            }
        }
...
fn enum_registry_values(
    ...
    let logical_key = if let Some(sid) = hive_label.strip_prefix("HKU\\") {
        key.strip_prefix(&format!("{}\\", sid)).unwrap_or(key)
    } else {
        key
    };
    let h = open_key(root, key, KEY_QUERY_VALUE | view)?;
    let mut index = 0u32;
    loop {
        ...
        let value_name = String::from_utf16_lossy(&name);
        let supported = ty == REG_SZ.0 || ty == REG_EXPAND_SZ.0;
        let command = if supported {
            decode_utf16_bytes(&data)
        } else {
            format!("Registry value type {ty}")
        };
        let systemish = is_security_or_system_command(&command);
        let protected = systemish || !supported;
        let reason = if !supported {
            "Only REG_SZ/REG_EXPAND_SZ startup values are reversible in Phase 5."
        } else if systemish {
            "Windows/security startup targets are protected."
        } else {
            ""
        };
        let state = NativeState::RegistryValue {
            hive: hive_label.into(),
...
fn scan_startup_folders(out: &mut Vec<StartupItem>, warnings: &mut Vec<String>) -> Result<()> {
    ...
    let users = PathBuf::from(format!(r"{}\Users", system_drive));
...
fn scan_folder(
    ...
    for e in entries.flatten() {
        ...
        {
            warnings.push(format!(
                "Skipped reparse/symlink startup entry: {}",
                p.display()
            ));
            continue;
        }
        ...
        let name = e.file_name().to_string_lossy().to_string();
        let systemish = is_security_or_system_command(&p.to_string_lossy());
        let oversized = meta.len() > MAX_STARTUP_FILE_EVIDENCE_BYTES;
        let protected = systemish || oversized;
        let digest = if oversized {
            String::new()
        } else {
            file_sha256(&p)?
        };
        let state = NativeState::StartupFile {
        ...
        let reason = if systemish {
            "Windows/security startup targets are protected."
        } else if oversized {
            "Startup entry exceeds the Phase 5 evidence-size budget and is therefore observation-only."
        } else {
            ""
        };
        out.push(StartupItem {
            item_id: stable_id(&format!(
                "folder|{}",
                p.to_string_lossy().to_ascii_lowercase()
            )),
            kind: "StartupFolder".into(),
            scope: scope.into(),
            display_name: name,
            publisher: if systemish {
                "Windows / security".into()
            } else {
                "Third-party / unknown".into()
            },
            ...
            evidence_detail:
                "Present in a Startup folder; no directly correlated duration evidence is asserted."
                    .into(),
...
fn scan_scheduled_tasks(out: &mut Vec<StartupItem>) -> Result<()> {
    with_task_service(|service| unsafe {
        let root = service.GetFolder(&BSTR::from("\\")).map_err(win)?;
        scan_task_folder(&root, out)
    })
}
unsafe fn scan_task_folder(folder: &ITaskFolder, out: &mut Vec<StartupItem>) -> Result<()> {
    let tasks = unsafe { folder.GetTasks(1) }.map_err(win)?;
    let count = unsafe { tasks.Count() }.map_err(win)?;
    for i in 1..=count {
        let task = unsafe { tasks.get_Item(&VARIANT::from(i)) }.map_err(win)?;
        let enabled = unsafe { task.Enabled() }.map_err(win)?.0 != 0;
        if !enabled {
            continue;
        }
        let xml = unsafe { task.Xml() }.map_err(win)?.to_string();
        if !xml.contains("<LogonTrigger") && !xml.contains("<BootTrigger") {
            continue;
        }
        let path = unsafe { task.Path() }.map_err(win)?.to_string();
        let name = unsafe { task.Name() }.map_err(win)?.to_string();
        let protected = path
            .to_ascii_lowercase()
            .starts_with(r"\microsoft\windows\")
            || is_security_or_system_command(&xml);
        let digest = task_definition_hash(&xml);
        let command =
            extract_xml_tag(&xml, "Command").unwrap_or_else(|| "Task Scheduler action".into());
        let state = NativeState::ScheduledTask {
            task_path: path.clone(),
            enabled: true,
            xml_sha256: digest,
        };
        out.push(StartupItem{item_id:stable_id(&format!("task|{}",path.to_ascii_lowercase())),kind:"ScheduledTask".into(),scope:"Scheduled task".into(),display_name:name,publisher:if protected{"Windows / security".into()}else{"Third-party / unknown".into()},command,source:path.clone(),enabled:true,manageable:!protected,protected,protection_reason:if protected{"Microsoft Windows or security-related scheduled tasks are protected.".into()}else{String::new()},impact:"Unknown".into(),confidence:"InsufficientEvidence".into(),evidence_detail:"Task has a boot/logon trigger; no undocumented duration fields are converted into a performance claim.".into(),recommendation:if protected{"Keep enabled".into()}else{"Review".into()},service_change:false,original_state:Some(state)});
    }
    let folders = unsafe { folder.GetFolders(0) }.map_err(win)?;
    let fcount = unsafe { folders.Count() }.map_err(win)?;
    for i in 1..=fcount {
        let child = unsafe { folders.get_Item(&VARIANT::from(i)) }.map_err(win)?;
        unsafe {
            scan_task_folder(&child, out)?;
        }
    }
    Ok(())
}
...
fn scan_services(out: &mut Vec<StartupItem>, warnings: &mut Vec<String>) -> Result<()> {
    ...
    let mut configs =
        HashMap::<String, (Result<u32>, u32, String, Result<bool>, u32, Vec<String>)>::new();
    for name in services {
        ...
        let deps =
            query_multi_string(HKEY_LOCAL_MACHINE, &sub, "DependOnService").unwrap_or_default();
    ...
    for (name, (start_result, ty, image, delayed_result, launch, _)) in configs {
        let start = match start_result {
            Ok(v) => v,
            Err(e) => {
                warnings.push(format!("Service {name}: Start could not be read ({e}); excluded from the manageable startup inventory rather than guessed at."));
                continue;
            }
        };
        if start != 2 {
        ...
        let delayed = match delayed_result {
            Ok(v) => v,
            Err(e) => {
                warnings.push(format!("Service {name}: DelayedAutoStart could not be read ({e}); excluded from the manageable startup inventory rather than guessed at."));
                continue;
            }
        };
        let lname = name.to_ascii_lowercase();
        let image_lower = image.to_ascii_lowercase();
        let own_process = (ty & 0x10) != 0 && (ty & 0x20) == 0;
        let essential = is_essential_service(&lname);
        let windows_binary = image_lower.contains(&system_root)
            || image_lower.contains("\\system32\\")
            || image_lower.contains("svchost.exe");
        let security = is_security_or_system_command(&format!("{name} {image}"));
        let protected_role = is_protected_service_role(&format!("{name} {image}"));
        let is_depended = depended.contains(&lname);
        let protected = launch != 0
            || essential
            || windows_binary
            || security
            || protected_role
            || is_depended
            || !own_process;
        let reason = if launch != 0 {
            "Windows launch-protected service."
        } else if essential {
            "Essential Windows service policy."
        } else if windows_binary {
            "Windows-hosted service."
        } else if security {
            "Security-related service policy."
        } else if protected_role {
            "Networking, storage, input, accessibility, or other protected service role."
        } else if is_depended {
            "Another service declares a dependency on this service."
        } else if !own_process {
            "Shared/driver service types are not managed."
        } else {
            ""
        };
        let state = NativeState::Service {
            service_name: name.clone(),
            start_type: start,
            delayed_auto: delayed,
            service_type: ty,
            binary_path: image.clone(),
            launch_protected: launch,
        };
        out.push(StartupItem{item_id:stable_id(&format!("service|{lname}")),kind:"Service".into(),scope:"Machine service".into(),display_name:name.clone(),publisher:if protected&&windows_binary{"Windows".into()}else{"Third-party / unknown".into()},command:image.clone(),source:format!("Service: {name}"),enabled:true,manageable:!protected,protected,protection_reason:reason.into(),impact:"Unknown".into(),confidence:"InsufficientEvidence".into(),evidence_detail:if delayed{"Automatic (delayed) service; exact boot impact is not inferred without direct telemetry.".into()}else{"Automatic service; exact boot impact is not inferred without direct telemetry.".into()},recommendation:if protected{"Keep enabled".into()}else{"Review".into()},service_change:true,original_state:Some(state)});
    }
    Ok(())
}

fn query_registry_state(
    hive: &str,
    key: &str,
    value_name: &str,
    view: &str,
    ...
    let (root, key_path) = parse_hive(hive, key);
    let view_flag = match view {
        "64" => KEY_WOW64_64KEY,
        "32" => KEY_WOW64_32KEY,
        _ => REG_SAM_FLAGS(0),
    };
    match query_raw_value(root, &key_path, value_name, view_flag) {
        Ok((ty, data)) => Ok(NativeState::RegistryValue {
            hive: hive.into(),
            key: key.into(),
        ...
        Err(StartupError::Platform(e)) if e == "not-found" => Ok(NativeState::RegistryValue {
            hive: hive.into(),
            key: key.into(),
            value_name: value_name.into(),
            view: view.into(),
            ...
            data_hex: expected_hex.into(),
        }),
        Err(e) => Err(e),
    }
}
fn query_startup_file_state(
    path: &str,
    size: u64,
    modified: Option<i64>,
    expected_sha256: &str,
...
fn query_task_state(path: &str, _digest: &str) -> Result<NativeState> {
    with_task_service(|service| unsafe {
        let root = service.GetFolder(&BSTR::from("\\")).map_err(win)?;
        let task = root.GetTask(&BSTR::from(path)).map_err(win)?;
        let xml = task.Xml().map_err(win)?.to_string();
        let enabled = task.Enabled().map_err(win)?.0 != 0;
        Ok(NativeState::ScheduledTask {
            task_path: path.into(),
            enabled,
            xml_sha256: task_definition_hash(&xml),
        })
    })
}
fn query_service_state(name: &str) -> Result<NativeState> {
    let sub = format!(r"{}\{}", SERVICES_KEY, name);
    Ok(NativeState::Service {
        service_name: name.into(),
        start_type: query_dword(HKEY_LOCAL_MACHINE, &sub, "Start")?,
        delayed_auto: query_dword(HKEY_LOCAL_MACHINE, &sub, "DelayedAutoStart").unwrap_or(0) != 0,
        service_type: query_dword(HKEY_LOCAL_MACHINE, &sub, "Type")?,
        binary_path: query_string(HKEY_LOCAL_MACHINE, &sub, "ImagePath").unwrap_or_default(),
        launch_protected: query_dword(HKEY_LOCAL_MACHINE, &sub, "LaunchProtected").unwrap_or(0),
    })
}

fn delete_registry_value(hive: &str, key: &str, name: &str, view: &str) -> Result<()> {
    let (root, path) = parse_hive(hive, key);
    let vf = match view {
        "64" => KEY_WOW64_64KEY,
        "32" => KEY_WOW64_32KEY,
        _ => REG_SAM_FLAGS(0),
    };
    let h = open_key(root, &path, KEY_SET_VALUE | vf)?;
    let name_w = wide(name);
    let rc = unsafe { RegDeleteValueW(h.0, PCWSTR(name_w.as_ptr())) };
    if rc != ERROR_SUCCESS {
        return Err(StartupError::Platform(format!(
            "RegDeleteValueW error {}",
            rc.0
        )));
    }
    Ok(())
...
fn set_registry_value(
    ...
    let h = open_key(root, &path, KEY_SET_VALUE | vf)?;
    let data = hex::decode(data_hex).map_err(|e| StartupError::Platform(e.to_string()))?;
    let name_w = wide(name);
    let rc = unsafe {
        RegSetValueExW(
            h.0,
...
fn move_file(from: &str, to: &str) -> Result<()> {
    ...
    if let Some(parent) = dst.parent() {
        std::fs::create_dir_all(parent).map_err(|e| StartupError::Platform(e.to_string()))?;
    }
...
fn set_service_start(name: &str, start: u32, delayed: bool) -> Result<()> {
    ...
    unsafe {
        ...
        let name_w = wide(name);
        let svc = OwnedServiceHandle::new(
            OpenServiceW(
                scm.get(),
                PCWSTR(name_w.as_ptr()),
        ...
        ChangeServiceConfig2W(
            ...
            Some((&info as *const SERVICE_DELAYED_AUTO_START_INFO).cast()),
        )
        .map_err(win)?;
        Ok(())
    }
...
fn with_task_service<T>(f: impl FnOnce(&ITaskService) -> Result<T>) -> Result<T> {
    let _com = ComApartment::mta().map_err(|hr| {
        StartupError::Platform(format!("CoInitializeEx failed: 0x{:08X}", hr.0 as u32))
    })?;
    unsafe {
        let service: ITaskService =
            CoCreateInstance(&CLSID_TASK_SCHEDULER, None, CLSCTX_INPROC_SERVER).map_err(win)?;
        let v = VARIANT::default();
        service.Connect(&v, &v, &v, &v).map_err(win)?;
        f(&service)
    }
}
...
fn open_key(root: HKEY, key: &str, access: REG_SAM_FLAGS) -> Result<Key> {
    let mut h = HKEY::default();
    let key_w = wide(key);
    let rc = unsafe { RegOpenKeyExW(root, PCWSTR(key_w.as_ptr()), None, access, &mut h) };
    if rc == ERROR_FILE_NOT_FOUND {
        return Err(StartupError::Platform("not-found".into()));
    }
    if rc != ERROR_SUCCESS {
        return Err(StartupError::Platform(format!(
            "RegOpenKeyExW error {}",
...
fn enum_subkeys(root: HKEY, key: &str) -> Result<Vec<String>> {
    let h = if key.is_empty() {
        None
    } else {
        Some(open_key(root, key, KEY_READ)?)
    };
    let handle = h.as_ref().map(|k| k.0).unwrap_or(root);
    let mut out = Vec::new();
    for i in 0..u32::MAX {
        let mut buf = vec![0u16; 1024];
        let mut len = (buf.len() - 1) as u32;
        let rc = unsafe {
            RegEnumKeyExW(
                handle,
                i,
...
fn query_raw_value(
    ...
    let rc = unsafe { RegQueryValueExW(h.0, name_p, None, Some(&mut ty), None, Some(&mut len)) };
    if rc == ERROR_FILE_NOT_FOUND {
        return Err(StartupError::Platform("not-found".into()));
    }
    if rc != ERROR_SUCCESS {
        return Err(StartupError::Platform(format!(
            "RegQueryValueExW size error {}",
            rc.0
        )));
    }
...
fn query_dword(root: HKEY, key: &str, name: &str) -> Result<u32> {
    let (_, data) = query_raw_value(root, key, name, REG_SAM_FLAGS(0))?;
    if data.len() < 4 {
        return Err(StartupError::Platform(format!("{name} is not a DWORD")));
    }
    Ok(u32::from_le_bytes([data[0], data[1], data[2], data[3]]))
}
fn query_string(root: HKEY, key: &str, name: &str) -> Result<String> {
    let (ty, data) = query_raw_value(root, key, name, REG_SAM_FLAGS(0))?;
    if ty != REG_SZ.0 && ty != REG_EXPAND_SZ.0 {
        return Err(StartupError::Platform(format!("{name} is not a string")));
    }
    Ok(decode_utf16_bytes(&data))
}
fn query_multi_string(root: HKEY, key: &str, name: &str) -> Result<Vec<String>> {
    let (ty, data) = query_raw_value(root, key, name, REG_SAM_FLAGS(0))?;
    if ty != REG_MULTI_SZ.0 {
        return Ok(vec![]);
    }
    let u = bytes_to_u16(&data);
...
fn parse_hive(hive: &str, key: &str) -> (HKEY, String) {
    if let Some(sid) = hive.strip_prefix("HKU\\") {
        if key.starts_with(&format!("{}\\", sid)) {
            (HKEY_USERS, key.into())
        } else {
            (HKEY_USERS, format!(r"{}\{}", sid, key))
        }
    } else {
        (HKEY_LOCAL_MACHINE, key.into())
    }
}

fn modified_ms(m: &std::fs::Metadata) -> Option<i64> {
    m.modified()
        .ok()
        .and_then(|t| t.duration_since(UNIX_EPOCH).ok())
        .map(|d| d.as_millis() as i64)
...
fn file_sha256(path: &Path) -> Result<String> {
    let mut f = std::fs::File::open(path).map_err(|e| StartupError::Platform(e.to_string()))?;
...
fn normalize_task_settings_enabled(xml: &str) -> String {
    let Some(settings_start) = xml.find("<Settings") else {
        return xml.to_owned();
    };
    let Some(open_rel) = xml[settings_start..].find('>') else {
        return xml.to_owned();
    };
    let body_start = settings_start + open_rel + 1;
    let Some(close_rel) = xml[body_start..].find("</Settings>") else {
        return xml.to_owned();
    };
    let body_end = body_start + close_rel;
    let body = &xml[body_start..body_end];
    let Some(enabled_start_rel) = body.find("<Enabled>") else {
        return xml.to_owned();
    };
    let enabled_start = body_start + enabled_start_rel;
    let value_start = enabled_start + "<Enabled>".len();
    let Some(enabled_close_rel) = xml[value_start..body_end].find("</Enabled>") else {
        return xml.to_owned();
    };
...
fn stable_id(v: &str) -> String {
    hex::encode(Sha256::digest(v.as_bytes()))[..32].to_string()
}
fn win(e: windows::core::Error) -> StartupError {
    StartupError::Platform(e.to_string())
}
...
fn is_security_or_system_command(v: &str) -> bool {
...
fn is_protected_service_role(v: &str) -> bool {
    let x = v.to_ascii_lowercase();
    [
        "network",
        "ethernet",
...
fn is_essential_service(n: &str) -> bool {
    matches!(
        n,
        "rpcss"
            | "dcomlaunch"
...
mod windows_policy_tests {
    ...
    fn security_paths_fail_closed() {
        assert!(is_security_or_system_command(
            r"C:\Windows\System32\foo.exe"
        ));
        assert!(is_security_or_system_command(
            r"C:\Vendor\Endpoint Protection\agent.exe"
        ));
    }
    ...
    fn live_inventory_is_read_only() {
        ...
        assert!(
            items.iter().all(
                |item| item.recommendation == "Review" || item.recommendation == "Keep enabled"
            )
        );
    }
}
