...
use aethercore_operation_engine::{
    OperationEngine, PlanState, PlanView, StartupChangeAction, now_ms,
};
...
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct StartupExecutionItem {
    pub item_id: String,
    pub display_name: String,
    pub kind: String,
    pub stage: String,
...
impl StartupPlatform for WindowsStartupPlatform {
    ...
    fn current_state(&self, _: &StartupChangeAction) -> Result<String> {
        Err(StartupError::UnsupportedPlatform)
    }
...
pub struct StartupManager {
    ...
    db: Arc<Database>,
    platform: Arc<dyn StartupPlatform>,
    snapshot: Arc<RwLock<StartupSnapshot>>,
...
impl StartupManager {
    pub fn new(engine: Arc<OperationEngine>, db: Arc<Database>, data_root: PathBuf) -> Self {
        Self::with_platform_and_telemetry(
            ...
            db,
            data_root,
            Arc::new(WindowsStartupPlatform),
    ...
    pub fn with_platform_and_telemetry(
        ...
        Self {
            ...
            db,
            platform,
            snapshot: Arc::new(RwLock::new(StartupSnapshot::default())),
    ...
    pub fn passive_scan_with_fence(
        ...
        let epoch = {
            let current = self.snapshot.read().map_err(|_| StartupError::Busy)?;
            if current.state == StartupScanState::Scanning {
        ...
        let (items, warnings) = self.platform.scan()?;
        if token.is_cancelled() {
            return Err(StartupError::Cancelled);
        }
        let ready = StartupSnapshot {
            scan_id,
            state: StartupScanState::Ready,
        ...
        let committed = commit_fence.try_commit_checked(|| {
            ...
            if current.state == StartupScanState::Scanning || token.is_cancelled() {
                return None;
            }
    ...
    pub fn start_scan_with_lease(
        ...
        if !lease.matches(ReadWorkload::StartupDiscovery) {
    ...
    fn start_scan_inner(
        ...
        let initial = {
            ...
            let epoch = current
                ...
                .saturating_add(1)
                .max(started.max(0) as u64);
            let initial = StartupSnapshot {
                scan_id: scan_id.clone(),
                state: StartupScanState::Scanning,
                inventory_epoch: epoch,
                started_unix_ms: started,
                ..Default::default()
            };
        ...
        if let Err(error) = thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let next = match platform.scan() {
                    ...
                    Err(e) => StartupSnapshot {
                        scan_id,
                        state: StartupScanState::Failed,
                        inventory_epoch: epoch,
                        started_unix_ms: started,
                        completed_unix_ms: completed,
            ...
            })
        {
            ...
            if let Ok(mut current) = self.snapshot.write() {
                current.state = StartupScanState::Failed;
    ...
    fn snapshot(&self) -> StartupSnapshot {
        self.snapshot.read().map(|v| v.clone()).unwrap_or_default()
    }
    pub fn snapshot_for_owner(&self, owner_principal_key: &str) -> Result<StartupSnapshot> {
        let owner = self.snapshot_owner.read().map_err(|_| StartupError::Busy)?;
        if owner.as_str() != owner_principal_key {
            return Err(StartupError::OwnershipMismatch);
        }
        Ok(self.snapshot())
    }
    ...
    pub fn create_plan(
        ...
        if snap.state != StartupScanState::Ready {
            return Err(StartupError::ScanNotReady);
        }
        if snap.scan_id != scan_id || snap.inventory_epoch != epoch {
            return Err(StartupError::StaleScan);
        }
        let mut seen = HashSet::new();
        let mut actions = Vec::new();
        for d in decisions {
            if !seen.insert(&d.item_id) {
                return Err(StartupError::UnknownItem(format!(
                    "duplicate {}",
                    d.item_id
                )));
            }
            if d.decision != RecommendationDecision::Disable {
                continue;
            }
            let item = snap
                .items
                .iter()
                .find(|i| i.item_id == d.item_id)
                .ok_or_else(|| StartupError::UnknownItem(d.item_id.clone()))?;
            if item.protected {
                return Err(StartupError::Protected(item.display_name.clone()));
            }
            if !item.manageable {
                return Err(StartupError::NotManageable(item.display_name.clone()));
            }
            if item.service_change && !confirm_service_changes {
                return Err(StartupError::ServiceConfirmationRequired);
            }
            let original = item
                .original_state
                .clone()
                .ok_or_else(|| StartupError::NotManageable(item.display_name.clone()))?;
            let change_id = Uuid::new_v4().to_string();
            let applied = disabled_state(&original, &self.data_root, &change_id)?;
            actions.push(StartupChangeAction {
                change_id: change_id.clone(),
                item_id: item.item_id.clone(),
                direction: "Disable".into(),
                startup_kind: item.kind.clone(),
                display_name: item.display_name.clone(),
                source_locator: item.source.clone(),
                original_state_json: serde_json::to_string(&original)?,
                applied_state_json: serde_json::to_string(&applied)?,
                service_change: item.service_change,
            });
            if actions.len() > MAX_ACTIONS_PER_PLAN {
                return Err(StartupError::TooManyActions);
            }
        }
        if actions.is_empty() {
            return Err(StartupError::PassiveDefault);
        }
        self.engine
            .create_startup_plan(owner_principal_key, epoch, scan_id, actions)
            .map_err(Into::into)
    }
    ...
    pub fn create_restore_plan(
        ...
        let source = self
            .db
            .get_startup_change_for_owner(change_id, owner_principal_key)?
            .ok_or_else(|| StartupError::UnknownItem(change_id.into()))?;
        if !matches!(source.state.as_str(), "Applied" | "AppliedRecovered") {
            return Err(StartupError::Drift(format!(
                "change {} is not restorable",
                change_id
            )));
        }
        if source.kind == "Service" && !confirm_service_changes {
            return Err(StartupError::ServiceConfirmationRequired);
        }
        let action = StartupChangeAction {
            change_id: Uuid::new_v4().to_string(),
            item_id: source.item_id.clone(),
            direction: "Restore".into(),
            startup_kind: source.kind.clone(),
            display_name: source.display_name.clone(),
            source_locator: String::new(),
            original_state_json: source.original_json.clone(),
            applied_state_json: source.applied_json.clone(),
            service_change: source.kind == "Service",
        };
        self.engine
            .create_startup_plan(
                owner_principal_key,
                0,
                &format!("restore:{change_id}"),
                vec![action],
            )
            .map_err(Into::into)
    }
    ...
    pub fn start_with_lease(
        ...
        plan_id: &str,
        lease: MutationLease,
    ) -> Result<StartupExecutionStatus> {
        if !lease.matches(MutationWorkload::Startup, plan_id, owner_principal_key) {
            return Err(StartupError::Platform(
    ...
    fn start_inner(
        ...
        if plan.state != PlanState::AwaitingAuthorization {
            return Err(StartupError::AuthorizationRequired);
        }
        let mut running = self
            .running
            .lock()
            .map_err(|_| StartupError::AlreadyRunning)?;
        if running.is_some() {
        ...
        *running = Some(plan_id.into());
        ...
        if let Err(error) = self.engine.consume_authorization_and_begin(
            plan_id,
            owner_principal_key,
            "one-shot consent consumed; startup operation entered preflight",
        ) {
        ...
        if let Err(error) = self
            ...
            .upsert_maintenance_execution(&MaintenanceExecutionRecord {
                ...
                domain: DOMAIN.into(),
                stage: "Queued".into(),
                detail: "Authorized startup plan queued".into(),
                started_unix_ms: now,
                updated_unix_ms: now,
                ..Default::default()
            })
        {
            let _ = self.engine.transition(
                ...
                PlanState::Preflight,
                PlanState::Failed,
                "startup execution journal initialization failed after consent",
            );
            if let Ok(mut running) = self.running.lock() {
                *running = None;
            }
        ...
        let spawn = thread::Builder::new()
            .name("aether-startup-worker".into())
            .spawn(move || {
                let _mutation_lease = mutation_lease;
                let result = execute_plan_with_telemetry(
                    &engine,
                    &db,
                    platform.as_ref(),
                    &owner,
                ...
                if let Err(e) = result {
                    let _ = fail_execution(&engine, &db, &id, &e.to_string());
                }
        ...
        self.status(owner_principal_key, Some(plan_id))?
            .ok_or(StartupError::AlreadyRunning)
    }
    ...
    pub fn status(
        ...
        let action_meta = self
            ...
            .unwrap_or_default()
            .into_iter()
            .map(|a| (a.item_id.clone(), (a.display_name, a.startup_kind)))
            .collect::<HashMap<_, _>>();
        let items = self
            .db
            .maintenance_items(&rec.plan_id)?
            .into_iter()
            .map(|i| {
                let (display_name, kind) = action_meta
                    .get(&i.item_id)
                    .cloned()
                    .unwrap_or_else(|| (i.kind.clone(), i.kind.clone()));
                StartupExecutionItem {
                    item_id: i.item_id,
                    display_name,
                    kind,
                    stage: i.stage,
        ...
        let live = self
            .telemetry
            .get_for_owner(owner_principal_key, &rec.plan_id)
            .filter(|value| value.emitted_unix_ms >= rec.updated_unix_ms);
        Ok(Some(StartupExecutionStatus {
            plan_id: rec.plan_id,
            plan_state: plan.state.as_str().into(),
            stage: live.as_ref().map(|v| v.stage.clone()).unwrap_or(rec.stage),
            progress_known: live
                .as_ref()
                .map(|v| v.progress_known)
                .unwrap_or(rec.progress_known),
            overall_percent: live
                .as_ref()
                .map(|v| v.overall_percent)
                .unwrap_or(rec.overall_percent),
            current_item_id: live
                .as_ref()
                .map(|v| v.current_item_id.clone())
                .unwrap_or(rec.current_item_id),
            detail: live
                .as_ref()
                .map(|v| v.detail.clone())
                .unwrap_or(rec.detail),
            mutation_started: rec.mutation_started,
            recovery_required: rec.recovery_required,
            failure_message: rec.failure_message,
            started_unix_ms: rec.started_unix_ms,
            updated_unix_ms: live
                .as_ref()
                .map(|v| v.emitted_unix_ms)
                .unwrap_or(rec.updated_unix_ms),
            completed_unix_ms: rec.completed_unix_ms.unwrap_or(0),
            items,
        }))
    }
    ...
    pub fn history(
        ...
        owner_principal_key: &str,
        limit: usize,
    ) -> Result<Vec<StartupHistoryEntry>> {
        Ok(self
            .db
            .startup_changes_for_owner(owner_principal_key, limit)?
            .into_iter()
            .map(|r| {
                let direction = r.direction.clone();
                let restorable = matches!(r.state.as_str(), "Applied" | "AppliedRecovered")
                    && direction == "Disable";
                StartupHistoryEntry {
                    change_id: r.change_id,
                    origin_change_id: r.origin_change_id,
                    plan_id: r.plan_id,
                    item_id: r.item_id,
                    kind: r.kind,
                    ...
                    restored_unix_ms: r.restored_unix_ms,
                    restorable,
                }
            })
            .collect())
    ...
    pub fn recover_incomplete(&self) -> Result<()> {
        ...
        for mut rec in self.db.startup_changes_in_states(&["Prepared"])? {
            let action = StartupChangeAction {
                ...
                startup_kind: rec.kind.clone(),
                display_name: rec.display_name.clone(),
                source_locator: String::new(),
                original_state_json: rec.original_json.clone(),
                applied_state_json: rec.applied_json.clone(),
                service_change: rec.kind == "Service",
            };
            match self.platform.current_state(&action) {
                Ok(current) if current == expected_before(&action) => {
                    rec.state = "NoChange".into();
                    rec.detail =
                        "Restart recovery observed original state; no mutation replayed.".into();
                }
                Ok(current) if current == target_after(&action) => {
                    rec.state = "AppliedRecovered".into();
                    rec.detail="Restart recovery observed the intended changed state; no mutation replayed.".into();
                }
                Ok(_) => {
                    rec.state = "RecoveryRequired".into();
                    rec.detail =
                        "Restart recovery found ambiguous startup state; manual review required."
                            .into();
                }
                Err(e) => {
                    rec.state = "RecoveryRequired".into();
                    rec.detail = format!("Restart recovery could not inspect target: {e}");
                }
            }
            rec.updated_unix_ms = now_ms();
            self.db.upsert_startup_change(&rec)?;
        }
        for plan in self.engine.recoverable_plans()? {
            if self.engine.startup_actions(&plan.id).is_err() {
                continue;
            }
            if let Some(mut execution) = self.db.get_maintenance_execution(&plan.id)? {
                if execution.domain != DOMAIN {
                    continue;
                }
...
fn execute_plan_with_telemetry(
    engine: &OperationEngine,
    db: &Database,
    platform: &dyn StartupPlatform,
    owner_principal_key: &str,
    ...
    publish_progress(
        ...
        0,
        "",
        "Validating exact current startup state",
    );
    update_exec(
        db,
        plan_id,
    ...
    for action in &actions {
        let current = platform.current_state(action)?;
        let expected = expected_before(action);
        if current != expected {
    ...
    for action in &actions {
        let now = now_ms();
        let origin = if action.direction == "Restore" {
            origin_from_restore_scan(engine, plan_id).unwrap_or_else(|| action.change_id.clone())
        } else {
            action.change_id.clone()
        };
        db.upsert_startup_change(&StartupChangeRecord {
            change_id: action.change_id.clone(),
            origin_change_id: origin,
            plan_id: plan_id.into(),
            item_id: action.item_id.clone(),
            kind: action.startup_kind.clone(),
            display_name: action.display_name.clone(),
            direction: action.direction.clone(),
            original_json: action.original_state_json.clone(),
            applied_json: action.applied_state_json.clone(),
            state: "Prepared".into(),
            detail: "Exact original and target states durably recorded before mutation.".into(),
            created_unix_ms: now,
            updated_unix_ms: now,
    ...
    engine.transition(
        ...
        PlanState::Preflight,
        PlanState::Protected,
        "startup rollback evidence persisted",
    )?;
    engine.transition(
        plan_id,
        PlanState::Protected,
        PlanState::Executing,
        "startup mutation barrier",
    )?;
    publish_progress(
        telemetry,
        owner_principal_key,
        ...
        0,
        "",
        "Applying reviewed startup changes",
    );
    update_exec(
        db,
        plan_id,
    ...
    for (index, action) in actions.iter().enumerate() {
        ...
        publish_progress(
            ...
            pct,
            &action.item_id,
            &format!("{}: {}", action.direction, action.display_name),
        );
        db.upsert_maintenance_item(&MaintenanceItemRecord {
            ...
            bytes_affected: 0,
            detail: format!("{} {}", action.direction, action.startup_kind),
        ...
        if let Some(mut execution) = db.get_maintenance_execution(plan_id)? {
            execution.current_item_id = action.item_id.clone();
            execution.updated_unix_ms = now_ms();
            db.upsert_maintenance_execution(&execution)?;
        }
        platform.apply(action)?;
        let after = platform.current_state(action)?;
        let target = target_after(action);
        if after != target {
            return Err(StartupError::Drift(format!(
        ...
        let mut rec = db
            .get_startup_change(&action.change_id)?
            .ok_or_else(|| StartupError::UnknownItem(action.change_id.clone()))?;
        rec.state = if action.direction == "Restore" {
            "Restored".into()
        } else {
            "Applied".into()
        };
        rec.detail = "Native post-change verification matched the immutable target state.".into();
        rec.updated_unix_ms = now_ms();
        ...
        db.upsert_maintenance_item(&MaintenanceItemRecord {
            ...
            result_code: "Verified".into(),
            bytes_affected: 0,
            detail: "Change verified and reversible evidence retained.".into(),
            updated_unix_ms: now_ms(),
        })?;
    }
    engine.transition(
        ...
        PlanState::Executing,
        PlanState::Verifying,
        "startup post-change verification",
    )?;
    publish_progress(
        telemetry,
        owner_principal_key,
        ...
        95,
        "",
        "Verifying exact post-change startup state",
    );
    for action in &actions {
        if platform.current_state(action)? != target_after(action) {
            return Err(StartupError::Drift(action.display_name.clone()));
        }
    }
    let now = now_ms();
...
fn fail_execution(
    ...
    let record = staged_exec(
        ...
        "Failed",
        0,
        "Startup change did not complete cleanly",
        mutation,
        mutation,
        message,
    )?;
...
fn staged_exec(
    ...
    Ok(MaintenanceExecutionRecord {
        ...
        progress_known: true,
        overall_percent: pct,
        current_item_id: old.current_item_id,
        detail: detail.into(),
        mutation_started: mutation || old.mutation_started,
        recovery_required: recovery || old.recovery_required,
        failure_message: failure.into(),
        started_unix_ms: if old.started_unix_ms == 0 {
            now
        } else {
            old.started_unix_ms
        },
...
fn disabled_state(
    original: &NativeState,
    data_root: &Path,
    change_id: &str,
) -> Result<NativeState> {
    Ok(match original {
        NativeState::RegistryValue {
            hive,
            key,
            value_name,
            view,
...
fn expected_before(a: &StartupChangeAction) -> String {
    if a.direction == "Restore" {
        a.applied_state_json.clone()
    } else {
        a.original_state_json.clone()
    }
}
fn target_after(a: &StartupChangeAction) -> String {
    if a.direction == "Restore" {
        a.original_state_json.clone()
    } else {
        a.applied_state_json.clone()
...
fn origin_from_restore_scan(engine: &OperationEngine, plan_id: &str) -> Option<String> {
    engine
        .get_plan(plan_id)
        .ok()?
        .scan_id
...
fn summarize(items: &[StartupItem]) -> StartupSummary {
    let mut s = StartupSummary::default();
    for i in items {
        s.total += 1;
        match i.kind.as_str() {
            "RegistryRun" | "RegistryRunOnce" => s.registry += 1,
            "StartupFolder" => s.startup_folders += 1,
            "ScheduledTask" => s.scheduled_tasks += 1,
...
mod tests {
    ...
    fn approve(engine: &OperationEngine, plan: &PlanView) {
        let intent = engine.begin_consent_intent(&plan.id, OWNER).unwrap();
        engine
    ...
    impl StartupPlatform for MockPlatform {
        fn scan(&self) -> Result<(Vec<StartupItem>, Vec<String>)> {
            Ok((self.items.clone(), vec![]))
        }
        fn current_state(&self, a: &StartupChangeAction) -> Result<String> {
            Ok(self
                .states
                .lock()
                .unwrap()
                .get(&a.item_id)
                .cloned()
                .unwrap_or_else(|| expected_before(a)))
        }
        fn apply(&self, a: &StartupChangeAction) -> Result<()> {
    ...
    fn item(id: &str, protected: bool, service: bool) -> StartupItem {
        StartupItem {
            ...
            command: "agent.exe".into(),
            source: "source".into(),
            enabled: true,
            manageable: !protected,
            protected,
            protection_reason: if protected {
                "protected".into()
            } else {
                String::new()
            },
            impact: "Unknown".into(),
            confidence: "InsufficientEvidence".into(),
            evidence_detail: "No directly correlated boot-duration evidence.".into(),
            recommendation: "Review".into(),
            service_change: service,
            original_state: Some(if service {
                NativeState::Service {
                    service_name: id.into(),
                    start_type: 2,
                    delayed_auto: true,
                    service_type: 16,
                    binary_path: r"C:\Vendor\agent.exe".into(),
                    launch_protected: 0,
                }
            } else {
                NativeState::RegistryValue {
                    ...
                    exists: true,
                    value_type: 1,
                    data_hex: "41000000".into(),
                }
            }),
        }
    }
    fn manager(items: Vec<StartupItem>) -> (StartupManager, Arc<MockPlatform>, PathBuf) {
        let path = std::env::temp_dir().join(format!("aether-startup-{}.db", Uuid::new_v4()));
        ...
        (
            StartupManager::with_platform(engine, db, std::env::temp_dir(), platform.clone()),
            platform,
            path,
        )
    }
    fn ready(m: &StartupManager) {
        let now = now_ms();
        *m.snapshot_owner.write().unwrap() = OWNER.into();
        *m.snapshot.write().unwrap() = StartupSnapshot {
            scan_id: "s".into(),
            state: StartupScanState::Ready,
            inventory_epoch: 1,
            started_unix_ms: now,
            completed_unix_ms: now,
            summary: summarize(&m.platform.scan().unwrap().0),
            items: m.platform.scan().unwrap().0,
            warnings: vec![],
            error_message: String::new(),
        };
    }
    ...
    #[test]
    fn passive_default_means_zero_mutations() {
        let (m, p, path) = manager(vec![item("a", false, false)]);
        ready(&m);
        assert!(matches!(
            m.create_plan(
                OWNER,
                ...
                &[StartupDecision {
                    item_id: "a".into(),
                    decision: RecommendationDecision::Unreviewed
                }],
                false
            ),
            Err(StartupError::PassiveDefault)
        ));
    ...
    #[test]
    fn keep_enabled_is_not_an_action() {
        let (m, _, path) = manager(vec![item("a", false, false), item("b", false, false)]);
        ready(&m);
        let plan = m
            .create_plan(
                OWNER,
                ...
                &[
                    StartupDecision {
                        item_id: "a".into(),
                        decision: RecommendationDecision::KeepEnabled,
                    },
                    StartupDecision {
                        item_id: "b".into(),
    ...
    #[test]
    fn protected_target_cannot_be_selected() {
        let (m, _, path) = manager(vec![item("security", true, false)]);
        ready(&m);
        assert!(matches!(
            m.create_plan(
                OWNER,
                ...
                1,
                &[StartupDecision {
                    item_id: "security".into(),
                    decision: RecommendationDecision::Disable
                }],
                false
            ),
            Err(StartupError::Protected(_))
        ));
        drop(m);
        let _ = std::fs::remove_file(path);
    }
    #[test]
    fn service_requires_second_confirmation() {
        let (m, _, path) = manager(vec![item("svc", false, true)]);
        ready(&m);
        assert!(matches!(
            m.create_plan(
                OWNER,
            ...
            Err(StartupError::ServiceConfirmationRequired)
        ));
        assert!(
            m.create_plan(
                OWNER,
    ...
    #[test]
    fn service_restore_requires_second_confirmation() {
        let (m, _, path) = manager(vec![item("svc", false, true)]);
        ready(&m);
        let plan = m
            .create_plan(
        ...
        m.db.upsert_startup_change(&StartupChangeRecord {
            change_id: action.change_id.clone(),
            origin_change_id: action.change_id.clone(),
        ...
        .unwrap();
        assert!(matches!(
            m.create_restore_plan(OWNER, &action.change_id, false),
            Err(StartupError::ServiceConfirmationRequired)
        ));
        assert!(
            m.create_restore_plan(OWNER, &action.change_id, true)
                .is_ok()
        );
        drop(m);
        let _ = std::fs::remove_file(path);
    ...
    fn restart_recovery_observes_applied_state_without_replay() {
        ...
        p.states
            .lock()
            .unwrap()
            .insert(action.item_id.clone(), action.applied_state_json.clone());
        let now = now_ms();
        m.db.upsert_startup_change(&StartupChangeRecord {
            change_id: action.change_id.clone(),
            origin_change_id: action.change_id.clone(),
    ...
    fn snapshot_is_principal_bound() {
        ...
        assert!(matches!(
            m.snapshot_for_owner(
                "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
            ),
            Err(StartupError::OwnershipMismatch)
        ));
        drop(m);
        let _ = std::fs::remove_file(path);
    }
    ...
    fn service_disable_target_is_manual_without_stop_semantics() {
        let original = NativeState::Service {
            service_name: "vendor".into(),
            start_type: 2,
            delayed_auto: true,
            service_type: 16,
            binary_path: r"C:\Vendor\agent.exe".into(),
    ...
    fn disable_then_restore_preserves_auditable_change_chain() {
        ...
        let disable = m
            ...
            .unwrap();
        let disable_action = m
            .engine
            .startup_actions(&disable.id)
            .unwrap()
            .into_iter()
            .next()
            .unwrap();
        ...
        m.engine
            .consume_authorization_and_begin(&disable.id, OWNER, "startup_test_authorized")
            .unwrap();
        execute_plan(&m.engine, &m.db, p.as_ref(), &disable.id).unwrap();
        assert_eq!(
            m.db.get_startup_change(&disable_action.change_id)
                .unwrap()
                .unwrap()
                .state,
            "Applied"
        ...
        let restore = m
            .create_restore_plan(OWNER, &disable_action.change_id, false)
            .unwrap();
        let restore_action = m
            .engine
            .startup_actions(&restore.id)
            .unwrap()
            .into_iter()
            .next()
            .unwrap();
        ...
        m.engine
            .consume_authorization_and_begin(&restore.id, OWNER, "startup_restore_test_authorized")
            .unwrap();
        ...
        let source =
            m.db.get_startup_change(&disable_action.change_id)
                ...
                .unwrap();
        let restore_record =
            m.db.get_startup_change(&restore_action.change_id)
                .unwrap()
                .unwrap();
