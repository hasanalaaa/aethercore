...
use aethercore_collector_runtime::{
    ...
    DEFAULT_COLLECTOR_TIMEOUT, FaultKind, IsolationGate, STORAGE_IOCTL_TIMEOUT, WMI_NEXT_SLICE,
...
use windows::{
    Win32::{
        ...
        System::{
            ...
            Variant::VARIANT,
            Wmi::{
                IWbemLocator, IWbemServices, WBEM_E_ACCESS_DENIED, WBEM_FLAG_FORWARD_ONLY,
...
fn push_fault_bounded(faults: &mut Vec<CollectorFaultRecord>, fault: CollectorFaultRecord) {
    ...
    if faults.len() < MAX_PROVIDER_FAULTS {
        faults.push(fault);
    }
}
...
fn init_com() -> Result<ComApartment> {
    ...
    ComApartment::mta().map_err(|hr| {
        TelemetryError::Windows(format!("CoInitializeEx failed: 0x{:08X}", hr.0 as u32))
    })
}
...
pub fn collect_with_cancellation(parent: CancellationToken) -> Result<HardwareTelemetrySnapshot> {
    ...
    let storage = match run_isolated_gated_with_token(
        ...
        parent.child(),
        |control| {
            collect_storage(&control).map_err(|error| {
                CollectorFault::new(
                    "hardware-telemetry",
                    "storage",
                    fault_kind(&error),
...
fn collect_storage(
    ...
    let (reliability, reliability_warning) = match reliability_result {
        ...
        Err(error) => {
            push_fault_bounded(
                &mut provider_faults,
                fault_record("wmi.reliability", &error),
            );
            (
                HashMap::new(),
                Some(format!(
                    "MSFT_StorageReliabilityCounter was unavailable: {error}"
                )),
            )
        }
    };
    ...
    for device in &mut devices {
        ...
        if let Some(note) = &reliability_warning {
            device.source_notes.push(note.clone());
        }
        if let Some(reliability) = reliability.get(&device.device_id) {
            device.reliability = reliability.clone();
            device
                .source_notes
                .push("MSFT_StorageReliabilityCounter".into());
        }
            ...
            if device.bus_type.eq_ignore_ascii_case("NVMe") {
                match query_nvme_health_bounded(index, control.cancellation().child()) {
                    Ok(nvme) => {
                        ...
                        device
                            .source_notes
                            .push("NVMe SMART/Health log via IOCTL_STORAGE_QUERY_PROPERTY".into());
                    }
                    Err(error) => {
                        push_fault_bounded(
                            &mut provider_faults,
                            fault_record("nvme-health-ioctl", &error),
                        );
                        device.source_notes.push(
                            "Direct NVMe SMART/Health log was not available for this device."
                                .into(),
                        );
                    }
                }
...
fn query_physical_disks(
    services: &IWbemServices,
    control: &CollectorControl,
) -> Result<Vec<StorageDeviceTelemetry>> {
    let objects = query(
        services,
        "SELECT DeviceId,FriendlyName,FirmwareVersion,SerialNumber,BusType,MediaType,Size,HealthStatus,OperationalStatus FROM MSFT_PhysicalDisk",
        control,
    )?;
    let mut out = Vec::new();
    for o in objects {
        ...
        let mut d = StorageDeviceTelemetry {
            device_id,
            friendly_name: prop_string(&o, "FriendlyName")
        ...
        if d.device_id.is_empty() {
            d.source_notes
                .push("Physical disk DeviceId was not reported.".into());
        }
        if size.is_none() {
            d.source_notes.push("Physical disk Size was not reported by WMI; size_bytes is not a confirmed measurement.".into());
        }
        out.push(d);
    }
    Ok(out)
...
fn query_reliability(
    services: &IWbemServices,
    control: &CollectorControl,
) -> Result<HashMap<String, StorageReliability>> {
    let objects = query(
        services,
        "SELECT DeviceId,Temperature,TemperatureMax,Wear,PowerOnHours,ReadErrorsTotal,ReadErrorsUncorrected,WriteErrorsTotal,WriteErrorsUncorrected,ReadLatencyMax,WriteLatencyMax,FlushLatencyMax FROM MSFT_StorageReliabilityCounter",
        control,
    )?;
    let mut out = HashMap::new();
    for o in objects {
        let Some(device_id) = prop_string(&o, "DeviceId") else {
            continue;
        };
        out.insert(
            device_id,
    ...
    unsafe {
        ...
        loop {
            ...
            if status.0 == WBEM_S_TIMEDOUT.0 {
                if let Some(v) = take_wmi_object(&mut values, returned)? {
                    out.push(v);
                }
                if out.len() > 256 {
                    return Err(TelemetryError::MalformedResponse(
                        "WMI provider returned more than the bounded 256-object storage inventory"
                            .into(),
                    ));
                }
                continue;
            ...
            if status.is_err() {
                let detail = format!(
                    "IEnumWbemClassObject::Next failed with HRESULT 0x{:08X}",
                    status.0 as u32
                );
                return Err(
                    if status.0 == E_ACCESSDENIED.0 || status.0 == WBEM_E_ACCESS_DENIED.0 {
            ...
            if status.0 == WBEM_S_FALSE.0 {
                if returned != 0 || values[0].is_some() {
                    return Err(TelemetryError::MalformedResponse(
                        "WMI enumerator returned terminal WBEM_S_FALSE with a non-empty result"
                            .into(),
                    ));
                }
                break;
            ...
            if returned == 0 {
                return Err(TelemetryError::MalformedResponse(
                    "WMI enumerator returned zero objects without WBEM_S_TIMEDOUT or WBEM_S_FALSE"
                        .into(),
                ));
            }
        }
...
fn take_wmi_object(
    ...
    match (returned, values[0].take()) {
        ...
        _ => Err(TelemetryError::MalformedResponse(
            "WMI enumerator returned an invalid object count".into(),
        )),
    }
}
...
fn prop_string(o: &windows::Win32::System::Wmi::IWbemClassObject, n: &str) -> Option<String> {
    BSTR::try_from(&get_variant(o, n)?)
        .ok()
        .map(|v| v.to_string())
}
fn prop_u8(o: &windows::Win32::System::Wmi::IWbemClassObject, n: &str) -> Option<u8> {
    u16::try_from(&get_variant(o, n)?)
        .ok()
        .and_then(|v| u8::try_from(v).ok())
}
fn prop_u16(o: &windows::Win32::System::Wmi::IWbemClassObject, n: &str) -> Option<u16> {
    u16::try_from(&get_variant(o, n)?).ok().or_else(|| {
        u32::try_from(&get_variant(o, n)?)
            .ok()
...
fn prop_u64(o: &windows::Win32::System::Wmi::IWbemClassObject, n: &str) -> Option<u64> {
    u64::try_from(&get_variant(o, n)?)
        .ok()
        .or_else(|| u32::try_from(&get_variant(o, n)?).ok().map(u64::from))
}

fn bus_name(v: Option<u16>) -> String {
    match v {
        Some(3) => "ATA",
        Some(11) => "SATA",
        Some(17) => "NVMe",
        ...
        Some(9) => "iSCSI",
        Some(10) => "SAS",
        Some(n) => return format!("BusType {n}"),
        None => "Unknown",
    }
    .into()
}
fn media_name(v: Option<u16>) -> String {
    match v {
        Some(3) => "HDD",
        Some(4) => "SSD",
        Some(5) => "SCM",
        Some(n) => return format!("MediaType {n}"),
        None => "Unknown",
    }
    .into()
}
fn health_name(v: Option<u16>) -> String {
    match v {
        Some(0) => "Healthy",
        Some(1) => "Warning",
        Some(2) => "Unhealthy",
        Some(5) => "Unknown",
        Some(n) => return format!("HealthStatus {n}"),
        None => "Unknown",
    }
    .into()
}
...
fn isolated_fault_to_telemetry(fault: CollectorFault) -> TelemetryError {
    match fault.kind {
        ...
        FaultKind::ProviderFailure | FaultKind::Io | FaultKind::Internal => {
            TelemetryError::Windows(fault.detail)
        }
    }
}
...
fn query_ata_smart_attributes(index: u32) -> Result<Vec<AtaSmartAttribute>> {
    ...
    let input = SENDCMDINPARAMS {
        ...
        bDriveNumber: u8::try_from(index).map_err(|_| {
            TelemetryError::MalformedResponse(
                "physical-drive index did not fit ATA SMART bDriveNumber".into(),
            )
        })?,
        bReserved: [0; 3],
        dwReserved: [0; 4],
    ...
    let returned = usize::try_from(returned).map_err(|_| {
        TelemetryError::MalformedResponse("ATA SMART byte count did not fit usize".into())
    })?;
...
fn query_nvme_health(index: u32) -> Result<NvmeHealthValues> {
    ...
    .map_err(win)?;
    let returned = usize::try_from(returned).map_err(|_| {
        TelemetryError::MalformedResponse("NVMe IOCTL byte count did not fit usize".into())
    })?;
    if returned > out_len {
        return Err(TelemetryError::MalformedResponse(
            "NVMe driver reported more bytes than the supplied output buffer".into(),
        ));
    }
    if returned < size_of::<STORAGE_PROTOCOL_DATA_DESCRIPTOR>() {
        return Err(TelemetryError::MalformedResponse(
            "NVMe protocol descriptor was not returned".into(),
        ));
    }
    let descriptor =
        unsafe { std::ptr::read_unaligned(output_ptr as *const STORAGE_PROTOCOL_DATA_DESCRIPTOR) };
    let descriptor_min = size_of::<STORAGE_PROTOCOL_DATA_DESCRIPTOR>() as u32;
    if descriptor.Version < descriptor_min
        || descriptor.Size < descriptor_min
        || usize::try_from(descriptor.Size)
            .ok()
            .is_none_or(|size| size > returned)
    {
        return Err(TelemetryError::MalformedResponse(
            "NVMe protocol descriptor version/size was invalid".into(),
        ));
    }
    let protocol = &descriptor.ProtocolSpecificData;
    if protocol.ProtocolType != ProtocolTypeNvme || protocol.DataType != 2 {
        return Err(TelemetryError::MalformedResponse(
            "NVMe driver returned an unexpected protocol/data type".into(),
        ));
    }
    let protocol_offset = offset_of!(STORAGE_PROTOCOL_DATA_DESCRIPTOR, ProtocolSpecificData);
    let window = checked_protocol_window(
        returned,
        protocol_offset,
        protocol.ProtocolDataOffset,
        protocol.ProtocolDataLength,
        size_of::<STORAGE_PROTOCOL_SPECIFIC_DATA>(),
        NVME_HEALTH_LOG_BYTES,
    )?;
