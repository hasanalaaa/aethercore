...
use aethercore_collector_runtime::{
    CancellationToken, CollectorControl, CollectorFault, CollectorFaultRecord, CommitFence,
...
impl Backend for WindowsBackend {
    fn hardware(
        ...
        aethercore_hardware_telemetry::collect_with_cancellation(control.cancellation()).map_err(
            |error| {
    ...
    fn crashes(
        ...
        aethercore_crash_diagnostics::collect_with_cancellation(control.cancellation()).map_err(
            |error| {
                let kind = match &error {
...
struct Inner {
    ...
    hardware_gate: IsolationGate,
    crash_gate: IsolationGate,
}

impl DiagnosticEngine {
    pub fn new(db: Arc<Database>) -> Self {
        Self::with_backend(db, Arc::new(WindowsBackend))
    }
    pub fn with_backend(db: Arc<Database>, backend: Arc<dyn Backend>) -> Self {
        Self {
            inner: Arc::new(Inner {
                backend,
                db,
                snapshot: Mutex::new(DiagnosticsSnapshot::default()),
                owner_principal_key: Mutex::new(String::new()),
                hardware_gate: IsolationGate::default(),
                crash_gate: IsolationGate::default(),
            }),
        }
    }
    ...
    pub fn start_scan_with_lease(
        ...
        if !lease.matches(ReadWorkload::Diagnostics) {
    ...
    fn start_scan_inner(
        ...
        if let Err(error) = thread::Builder::new()
            .name("aether-diagnostic-scan".into())
            .spawn(move || {
    ...
    pub fn passive_hardware_refresh(
        ...
        let gate = self.inner.hardware_gate.clone();
        let child = token.child();
        let hardware = run_isolated_gated_with_token(
            &gate,
            "diagnostic-engine",
            "passive-hardware",
            Duration::from_secs(10),
            child,
            move |control| backend.hardware(control),
        )
        .map_err(|fault| {
            if matches!(fault.kind, FaultKind::Cancelled) {
                DiagnosticError::Cancelled
            } else {
                DiagnosticError::Provider(fault.to_string())
            }
        })?;
        ...
        let mut next = {
            ...
            let current = self
                ...
                .lock()
                .unwrap_or_else(|p| p.into_inner());
            if owner.as_str() == owner_principal_key && !current.state.running() {
                current.clone()
            } else {
        ...
        next.storage = hardware.storage.clone();
        next.memory = hardware.memory.clone();
        next.provider_faults
            .retain(|fault| !is_hardware_fault_provider(&fault.provider));
        next.provider_faults.extend(
            hardware
                .provider_faults
                .iter()
                .map(ProviderFaultRecord::from),
        );
    ...
    pub fn passive_event_log_refresh(
        &self,
        owner_principal_key: &str,
        token: CancellationToken,
        commit_fence: CommitFence,
        ...
        let backend = self.inner.backend.clone();
        let gate = self.inner.crash_gate.clone();
        let child = token.child();
        let crash = run_isolated_gated_with_token(
            &gate,
            "diagnostic-engine",
            "passive-eventlog",
            Duration::from_secs(10),
            child,
            move |control| backend.crashes(control),
        )
        .map_err(|fault| {
            if matches!(fault.kind, FaultKind::Cancelled) {
                DiagnosticError::Cancelled
        ...
        next.provider_faults
            .retain(|fault| is_hardware_fault_provider(&fault.provider));
        next.provider_faults
            .extend(crash.provider_faults.iter().map(ProviderFaultRecord::from));
    ...
    pub fn snapshot_for_owner(&self, owner_principal_key: &str) -> Result<DiagnosticsSnapshot> {
        let current = self
            .inner
            .owner_principal_key
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        if current.as_str() != owner_principal_key {
            return Err(DiagnosticError::OwnershipMismatch);
        }
        let snapshot = self
            .inner
            .snapshot
    ...
    pub fn history(
        ...
        owner_principal_key: &str,
        limit: usize,
    ) -> Result<Vec<DiagnosticHistoryEntry>> {
        self.inner
            .db
            .diagnostic_snapshots_for_owner(owner_principal_key, limit)
            .map_err(|e| DiagnosticError::Persistence(e.to_string()))
            .map(|rows| {
                rows.into_iter()
                    .map(|r| {
                        let card_count =
                            serde_json::from_str::<DiagnosticsSnapshot>(&r.snapshot_json)
                                .ok()
                                .map(|s| s.cards.len() as u32);
                        DiagnosticHistoryEntry {
                            scan_id: r.snapshot_id,
                            state: r.state,
                            collected_unix_ms: r.collected_unix_ms,
...
fn is_hardware_fault_provider(provider: &str) -> bool {
    provider.contains("hardware")
        || provider.contains("storage")
        || provider.contains("wmi")
        || provider.contains("nvme")
        || provider.contains("smart")
}
...
fn passive_state(
    ...
    if !hardware_available && !event_available {
        return if snapshot.provider_faults.is_empty() {
            ScanState::Partial
        } else {
            ScanState::Failed
        };
    ...
    if snapshot.provider_faults.is_empty() && snapshot.warnings.is_empty() {
        ScanState::Ready
    } else {
        ScanState::Partial
    }
...
fn mark_scan_runtime_failure(
    ...
    snapshot
        .provider_faults
        .push(ProviderFaultRecord::from(&fault));
    snapshot
        .warnings
        .push("Diagnostic scan stopped because an internal worker boundary failed.".into());
}
    ...
    match worker {
        Ok(handle) => handle.join().unwrap_or_else(|_| {
            Err(CollectorFault::new(
                ...
                operation,
                FaultKind::Internal,
                "provider supervisor thread terminated unexpectedly",
            ))
        }),
        Err(error) => Err(CollectorFault::new(
            ...
            operation,
            FaultKind::Internal,
            format!("failed to spawn provider supervisor thread: {error}"),
        )),
    }
}
...
fn run(inner: Arc<Inner>, owner_principal_key: String) {
    ...
    let hardware_worker = thread::Builder::new()
        .name("aether-diagnostic-hardware".into())
        .spawn(move || {
            run_isolated_gated(
                &hardware_gate,
                "diagnostic-engine",
                "hardware-provider",
                PROVIDER_WATCHDOG,
                move |control| hardware_backend.hardware(control),
            )
        });
    let crash_worker = thread::Builder::new()
        .name("aether-diagnostic-crash".into())
        .spawn(move || {
            run_isolated_gated(
                &crash_gate,
                "diagnostic-engine",
                "crash-provider",
                PROVIDER_WATCHDOG,
                move |control| crash_backend.crashes(control),
            )
        });
    ...
    let hardware = match hardware_result {
        Ok(v) => Some(v),
        Err(fault) => {
            warnings.push(format!("Hardware telemetry: {fault}"));
            provider_faults.push(ProviderFaultRecord::from(&fault));
            None
        }
    };
    let crash = match crash_result {
        Ok(v) => Some(v),
        Err(fault) => {
            warnings.push(format!("Crash diagnostics: {fault}"));
            provider_faults.push(ProviderFaultRecord::from(&fault));
            None
        }
    };
    ...
    let crashes = crash
        ...
        .unwrap_or_default();
    let cards = build_cards_with_availability(
        &storage,
        memory.as_ref(),
        &events,
        &crashes,
    ...
    let persistence_result = serde_json::to_string(&snapshot)
        .map_err(|error| {
            (
                FaultKind::Internal,
                format!("diagnostic snapshot serialization failed: {error}"),
            )
        })
        .and_then(|json| {
            inner
                .db
                .save_diagnostic_snapshot(&DiagnosticSnapshotRecord {
                    snapshot_id: scan_id,
                    owner_principal_key,
                ...
                .map_err(|error| {
                    (
                        FaultKind::Io,
                        format!("diagnostic snapshot persistence failed: {error}"),
                    )
                })
        });
...
fn build_cards_with_availability(
    ...
    if let Some(c) = crashes.first() {
        let nearby_whea = events
            ...
            .filter(|e| {
                e.provider
                    .eq_ignore_ascii_case("Microsoft-Windows-WHEA-Logger")
            })
            ...
            .filter(|e| {
                c.recorded_unix_ms
                    .is_some_and(|crash_ms| (e.recorded_unix_ms - crash_ms).abs() <= 10 * 60 * 1000)
            })
            .collect::<Vec<_>>();
...
mod tests {
    ...
    fn start_scan_leased(engine: &DiagnosticEngine, owner: &str) -> Result<DiagnosticsSnapshot> {
        let budget = ReadBudgetManager::new(4);
        let lease = budget
            .try_acquire(ReadWorkload::Diagnostics)
            .expect("read budget lease");
        engine.start_scan_with_lease(owner, lease)
    }
    struct Mock {
    ...
    struct CrashUnavailableMock;
    impl Backend for CrashUnavailableMock {
        fn hardware(
            &self,
            _control: CollectorControl,
        ) -> std::result::Result<HardwareTelemetrySnapshot, CollectorFault> {
            Ok(HardwareTelemetrySnapshot::default())
        }
        fn crashes(
            &self,
            _control: CollectorControl,
    ...
    fn db() -> (Arc<Database>, tempfile::TempDir) {
        ...
        let p = dir
            .path()
            .join(format!("aethercore-diag-{}.db", Uuid::new_v4()));
        (Arc::new(Database::open(&p).unwrap()), dir)
    }
    #[test]
    fn no_whea_never_becomes_ram_healthy() {
        let cards = build_cards(&[], &MemoryTelemetry::default(), &[], &[]);
        let c = cards
            .iter()
            .find(|c| c.card_id == "memory:no-logged-errors")
            .unwrap();
        assert!(c.summary.contains("does not prove RAM is fault-free"));
    }
    #[test]
    fn critical_storage_card_uses_backup_first_guidance() {
        let mut d = StorageDeviceTelemetry {
            device_id: "0".into(),
            friendly_name: "Disk".into(),
            severity: "ActionRequired".into(),
            summary: "errors".into(),
            reasons: vec!["uncorrected".into()],
            ..Default::default()
        };
        let cards = build_cards(&[d.clone()], &MemoryTelemetry::default(), &[], &[]);
        assert!(cards[0].actions[0].contains("Back up"));
        d.severity = "Normal".into();
        assert!(
            !build_cards(&[d], &MemoryTelemetry::default(), &[], &[])
                .iter()
                .any(|c| c.domain == "Storage")
        );
    }
    #[test]
    fn scan_persists_history() {
        let (db, _tmp) = db();
        ...
        assert!(!e.history(OWNER, 10).unwrap().is_empty());
        drop(e);
        drop(db);
    }
    #[test]
    fn partial_collector_failure_is_honest_and_persisted() {
        let (db, _tmp) = db();
        let e = DiagnosticEngine::with_backend(db.clone(), Arc::new(PartialMock));
        start_scan_leased(&e, OWNER).unwrap();
        for _ in 0..100 {
            if !e.snapshot().state.running() {
                break;
        ...
        assert!(s.warnings.iter().any(|w| w.contains("Hardware telemetry")));
        assert!(!e.history(OWNER, 10).unwrap().is_empty());
        drop(e);
        drop(db);
    }
    ...
    // ("...in the last 0 days").
    #[test]
    fn crash_provider_failure_defaults_the_window_not_to_zero() {
        let (db, _tmp) = db();
        let e = DiagnosticEngine::with_backend(db.clone(), Arc::new(CrashUnavailableMock));
        start_scan_leased(&e, OWNER).unwrap();
        for _ in 0..100 {
            if !e.snapshot().state.running() {
                break;
        ...
        assert_eq!(
            s.event_window_days,
            aethercore_crash_diagnostics::DEFAULT_EVENT_WINDOW_DAYS,
            "a failed crash provider must not report a 0-day event window"
        );
        drop(e);
        drop(db);
    }
    ...
    #[test]
    fn a_real_zero_card_scan_is_distinguishable_from_an_unparseable_one() {
        let (db, _tmp) = db();
        let e = DiagnosticEngine::with_backend(
            db.clone(),
            Arc::new(Mock {
        ...
        db.save_diagnostic_snapshot(&aethercore_persistence::DiagnosticSnapshotRecord {
            snapshot_id: "real-zero".into(),
            owner_principal_key: OWNER.into(),
            state: "Ready".into(),
            collected_unix_ms: 2,
            warning_count: 0,
            snapshot_json: serde_json::to_string(&empty_but_real).unwrap(),
        })
        .unwrap();
        db.save_diagnostic_snapshot(&aethercore_persistence::DiagnosticSnapshotRecord {
            snapshot_id: "unparseable".into(),
            owner_principal_key: OWNER.into(),
            state: "Ready".into(),
            collected_unix_ms: 1,
            warning_count: 0,
            snapshot_json: "{ this is not valid json".into(),
        })
        .unwrap();
        let entries = e.history(OWNER, 10).unwrap();
        let real = entries
            .iter()
            .find(|x| x.scan_id == "real-zero")
            .expect("the real-zero row");
        let broken = entries
            .iter()
            .find(|x| x.scan_id == "unparseable")
            .expect("the unparseable row");
        assert_eq!(
            real.card_count,
            Some(0),
            "a scan that genuinely produced zero cards must report a determined zero"
        );
        assert_eq!(
            broken.card_count, None,
            "an unparseable stored snapshot must not report zero cards as if it had been measured"
        );
        drop(e);
        drop(db);
    }
    #[test]
    fn diagnostic_snapshot_is_principal_bound() {
        let (db, _tmp) = db();
        let e = DiagnosticEngine::with_backend(
            db.clone(),
            Arc::new(Mock {
        ...
        assert!(matches!(
            e.snapshot_for_owner(other),
            Err(DiagnosticError::OwnershipMismatch)
        ));
        drop(e);
        drop(db);
    }
    #[test]
    fn unavailable_event_source_never_becomes_no_logged_errors() {
        let cards = build_cards_with_availability(
            &[],
            Some(&MemoryTelemetry::default()),
            &[],
            &[],
            false,
            0,
        );
        assert!(cards.iter().any(|c| c.card_id == "memory:whea-unavailable"));
        assert!(!cards.iter().any(|c| c.card_id == "memory:no-logged-errors"));
    }
    ...
    fn unavailable_memory_collector_never_becomes_zero_percent_pressure_evidence() {
        ...
        let no_errors = cards
            .iter()
            .find(|c| c.card_id == "memory:no-logged-errors")
            .expect("event-log conclusion should still exist");
        assert!(
            no_errors.evidence.is_empty(),
            "unavailable memory telemetry must not create synthetic pressure evidence"
        );
        assert!(
    ...
    fn nested_provider_faults_are_preserved_in_the_diagnostic_snapshot() {
        ...
        assert!(
            s.provider_faults
                .iter()
                .any(|fault| fault.operation == "nvme-health-ioctl")
        );
        assert!(
            s.provider_faults
                .iter()
                .any(|fault| fault.operation == "eventlog.render")
        );
        drop(e);
        drop(db);
    }
    ...
    fn provider_supervisor_panic_is_classified() {
        let worker = thread::Builder::new()
            .name("fault-injection-provider-supervisor".into())
            .spawn(|| -> std::result::Result<(), CollectorFault> {
                panic!("fault injection");
            });
    ...
    fn scan_runtime_failure_marks_collecting_snapshot_failed() {
        ...
        assert!(
            snapshot
                .provider_faults
                .iter()
                .any(|fault| fault.operation == "scan.run" && fault.kind == "Internal")
        );
        drop(e);
        drop(db);
    }
