...
use aethercore_driver_authority::{
    AcquisitionCapability, ApplicabilityState, AuthorityType, CoverageState,
    DeviceAuthorityCoverage, DeviceIdentity, DriverAuthorityEngine, DriverCandidateV2,
    DriverManagementAuthority, DriverOverride, DriverOverrideKind, InstallationCapability,
    MachineProfile, ManagementAuthorityAvailability, MatchKind, OfficialUtilityProvider,
    PackageTrustState, RecommendationState, UpdateAvailabilityEvidence, WindowsUpdateProvider,
...
impl DriverHub {
    ...
    pub fn resolve_install_selection(
        ...
        for device in &snapshot.devices {
            for candidate in &device.candidates {
                ...
                if !candidate.selectable
                    ...
                    || !matches!(
                        candidate.recommendation_state.as_str(),
                        "Recommended" | "Optional"
                    )
                {
                    return Err(HubError::CandidateNotSelectable(
        ...
        for id in candidate_ids {
            result.push(
                found
                    .remove(id.as_str())
                    .ok_or_else(|| HubError::CandidateNotFound(id.clone()))?,
            );
        }
        Ok(result)
    }
    ...
    pub fn set_candidate_policy(
        ...
        let database = self.inner.database.as_ref().ok_or_else(|| {
            HubError::Persistence("driver authority preference store unavailable".into())
        })?;
        let snapshot = self.snapshot_for_owner(owner_principal_key)?;
        if snapshot.state != ScanState::Ready
            || snapshot.scan_id != scan_id
            || snapshot.inventory_epoch != inventory_epoch
        {
            return Err(HubError::StaleSnapshot);
        }
        ...
        for device in &snapshot.devices {
            ...
            {
                matched = Some((device, candidate));
                break;
            }
        }
        let (device, candidate) =
            matched.ok_or_else(|| HubError::CandidateNotFound(candidate_id.into()))?;
        ...
        let (kind, behavior, version, expires) = match policy {
            ...
            "RemindLater" => (
                DriverOverrideKind::RemindLater,
                "RemindLater",
                String::new(),
                Some(default_defer_until(now)),
            ),
            "IgnoreOptional" if candidate.recommendation_state == "Optional" => (
        ...
        let override_id = format!(
            "{}:{}:{}:{}",
            privacy_key, behavior, version, candidate.authority_provider_id
        );
        database
            .upsert_driver_authority_override(&DriverAuthorityOverrideRecord {
                owner_principal_key: owner_principal_key.into(),
                override_id,
                device_privacy_key: privacy_key,
                behavior: behavior.into(),
                candidate_version: version,
                provider_id: candidate.authority_provider_id.clone(),
        ...
        if current.scan_id == scan_id && current.inventory_epoch == inventory_epoch {
            for device in &mut current.devices {
                for candidate in &mut device.candidates {
                    if candidate.candidate_id == candidate_id {
                        ...
                        candidate.recommendation_reasons.push(
                            match policy {
                                "RemindLater" => "USER_DEFERRED",
                                "IgnoreOptional" => "USER_IGNORED_OPTIONAL",
                                _ => "USER_IGNORED_EXACT_VERSION",
                            }
                            .into(),
                        );
                    }
            ...
            current.summary = summarize(
                &current.devices,
                current.summary.update_offer_count as usize,
            );
        }
        Ok(current.clone())
    }
    ...
    fn load_overrides(&self, owner: &str) -> (Vec<DriverOverride>, Option<String>) {
        let Some(database) = self.inner.database.as_ref() else {
            return (Vec::new(), None);
        };
        match database.driver_authority_overrides_for_owner(owner, Utc::now().timestamp_millis()) {
            Ok(rows) => (
                rows.into_iter()
                    .filter_map(|r| {
                        let kind = match r.behavior.as_str() {
                            "IgnoreExactVersion" => DriverOverrideKind::IgnoreExactVersion,
                            "RemindLater" => DriverOverrideKind::RemindLater,
                            "IgnoreOptional" => DriverOverrideKind::IgnoreOptional,
                            _ => return None,
                        };
                        Some(DriverOverride {
                            device_privacy_key: r.device_privacy_key,
                            kind,
                            candidate_version: r.candidate_version,
                            authority_provider_id: r.provider_id,
            ...
            Err(error) => (
                Vec::new(),
                Some(format!("Saved driver overrides unavailable: {error}")),
            ),
        }
    }
    ...
    fn persist_authority_snapshot(&self, owner: &str, snapshot: &DriverHubSnapshot) {
        ...
        if let Ok(snapshot_json) = serde_json::to_string(snapshot) {
            let _ = database.upsert_driver_authority_scan(&DriverAuthorityScanRecord {
                owner_principal_key: owner.into(),
                scan_id: snapshot.scan_id.clone(),
                inventory_epoch: snapshot.inventory_epoch,
                authority_coverage: snapshot.authority_coverage.clone(),
                snapshot_json,
                created_unix_ms: snapshot.completed_unix_ms,
            });
        }
    }
    ...
    pub fn passive_scan_with_fence(
        ...
        let (scan_id, epoch, started) = {
            ...
            (
                ...
                current.inventory_epoch.saturating_add(1),
        ...
        let committed = commit_fence.try_commit_checked(|| {
            ...
            let mut current = self
                ...
                .lock()
                .unwrap_or_else(|p| p.into_inner());
            if current.state.is_running() || token.is_cancelled() {
                return None;
            }
...
fn match_inventory_with_overrides(
    ...
    for (index, device) in devices.iter().enumerate() {
        for id in &device.hardware_ids {
            ...
            if !key.is_empty() {
                hardware_map.entry(key).or_default().push(index);
            }
        }
        for id in &device.compatible_ids {
            let key = normalize_pnp_id(id);
            if !key.is_empty() {
                compatible_map.entry(key).or_default().push(index);
            }
        }
    }
    ...
    if let Err(error) = &registry_result {
        warnings.push(format!(
            "Driver provider coverage registry invalid; authority completeness is unknown: {error}"
        ));
    }
    let gpu_policies: Vec<Option<GpuManagement>> =
        devices.iter().map(gpu_policy_for_device).collect();
    ...
    for offer in &discovery.offers {
        ...
        if let Some(indexes) = indexes {
            for index in indexes {
                ...
                normalized[*index].push(WindowsUpdateProvider::from_offer(
                    &identity, offer, match_kind,
                ));
            }
        } else {
            unmatched.push(UnmatchedOffer {
                ...
                hardware_id: offer.hardware_id.clone(),
    ...
    for (index, device) in devices.into_iter().enumerate() {
        ...
        if let Some(gpu_info) = &gpu {
            let guidance = OfficialUtilityProvider {
        ...
        if let Some(registry) = registry {
            for provider_id in coverage_detail
                .manual_authorities
                .iter()
                .chain(coverage_detail.unsupported_authorities.iter())
            {
                if management.iter().any(|m| &m.provider_id == provider_id) {
                    continue;
                }
                if let Some(guidance) =
                    management_authority_for_registry_provider(provider_id, registry)
                {
                    management.push(management_info(guidance));
                }
            }
        ...
        let mut device_candidates: Vec<DriverCandidate> = decision
            .candidates
            .into_iter()
            .map(driver_candidate_from_v2)
            .collect();
        device_candidates.sort_by(|a, b| {
            recommendation_priority(a)
                .cmp(&recommendation_priority(b))
            ...
            if device.status.missing_driver && recommended_candidate_id.is_empty() {
                ...
                match coverage_detail.completeness {
                    CoverageState::CompleteForRequiredAuthorities
                        if coverage_detail.permits_strong_up_to_date() =>
                    {
                        "UpToDate"
                    }
                    CoverageState::ManualAuthorityRequired | CoverageState::Partial => {
                    ...
                    CoverageState::Unknown | CoverageState::CompleteForRequiredAuthorities => {
                        "UpdateStatusUnknown"
                    }
                }
            }
            .to_string();
        let device_state = if device.status.missing_driver {
            "MissingDriver"
        } else if device.status.has_problem {
            "Problem"
        } else if device_candidates.iter().any(|c| c.firmware_managed) {
            "FirmwareManualReview"
        } else if !recommended_candidate_id.is_empty() {
            "UpdateAvailable"
        } else {
            "Healthy"
        ...
        result_devices.push(DriverDevice {
            ...
            raw_status: device.status.raw_status,
            problem_code: device.status.problem_code,
            has_problem: device.status.has_problem,
            missing_driver: device.status.missing_driver,
...
fn windows_update_scan_state(discovery: &DiscoveryResult) -> CoverageState {
    let offline = discovery
        .warnings
        .iter()
        .any(|warning| warning.starts_with("Windows Update discovery offline:"));
    let unavailable = discovery
        .warnings
        .iter()
        .any(|warning| warning.starts_with("Windows Update discovery unavailable:"));
    if offline {
    ...
    for device in devices {
        out.extend(
            device
                .manual_authorities
                .iter()
                .map(|id| format!("{id}:ManualAuthorityRequired")),
        );
        out.extend(
            device
                .unavailable_authorities
                .iter()
                .filter(|id| id.as_str() != "microsoft.windows-update")
                .map(|id| format!("{id}:Unavailable")),
        );
        out.extend(
            device
                .unsupported_authorities
                .iter()
                .map(|id| format!("{id}:UnsupportedAutomation")),
        );
    }
    out
}
...
fn management_availability_label(value: ManagementAuthorityAvailability) -> &'static str {
    match value {
        ManagementAuthorityAvailability::Available => "Available",
        ManagementAuthorityAvailability::Installed => "Installed",
        ManagementAuthorityAvailability::NotInstalled => "NotInstalled",
        ManagementAuthorityAvailability::ProviderUnavailable => "ProviderUnavailable",
    }
}
fn update_availability_label(value: UpdateAvailabilityEvidence) -> &'static str {
    match value {
        UpdateAvailabilityEvidence::UnknownUntilVendorCheck => "UnknownUntilVendorCheck",
        UpdateAvailabilityEvidence::UpdateAvailable => "UpdateAvailable",
        UpdateAvailabilityEvidence::NoUpdateReported => "NoUpdateReported",
        UpdateAvailabilityEvidence::ProviderUnavailable => "ProviderUnavailable",
    }
}
...
fn recommendation_priority(candidate: &DriverCandidate) -> u8 {
    match candidate.recommendation_state.as_str() {
        ...
        "ManualOfficial" => 3,
        "FirmwareProtected" => 4,
        "CurrentDriverPreferred" => 5,
        _ => 6,
    }
}

fn match_priority(candidate: &DriverCandidate) -> u8 {
    match candidate.match_quality.as_str() {
        "Hardware ID" => 0,
        "Compatible ID" => 1,
        _ => 2,
    }
...
fn summarize(devices: &[DriverDevice], offer_count: usize) -> DriverHubSummary {
    ...
    {
        unique_selectable_updates.insert((
            candidate.candidate_id.clone(),
            candidate.update_id.clone(),
            candidate.revision,
        ));
    }
    DriverHubSummary {
        device_count: devices.len().try_into().unwrap_or(u32::MAX),
        missing_driver_count: devices
            .iter()
            .filter(|d| d.missing_driver)
            .count()
            .try_into()
            .unwrap_or(u32::MAX),
        problem_device_count: devices
            .iter()
        ...
        matched_device_count: devices
            .iter()
            .filter(|d| !d.candidates.is_empty())
            .count()
            .try_into()
            .unwrap_or(u32::MAX),
        matched_candidate_count: devices
            .iter()
            .map(|d| d.candidates.len())
            .sum::<usize>()
            .try_into()
            .unwrap_or(u32::MAX),
        selectable_update_count: unique_selectable_updates
            .len()
            .try_into()
            .unwrap_or(u32::MAX),
        vendor_managed_gpu_count: devices
            .iter()
            .filter(|d| d.gpu.is_some())
            .count()
            .try_into()
            .unwrap_or(u32::MAX),
        vendor_managed_display_count: devices
            .iter()
            .filter(|d| d.display_managed)
            .count()
            .try_into()
            .unwrap_or(u32::MAX),
        recommended_update_count: devices
            .iter()
            .filter(|d| {
                !d.recommended_candidate_id.is_empty()
                    && d.candidates.iter().any(|c| c.recommended && c.selectable)
            })
            .count()
            .try_into()
            .unwrap_or(u32::MAX),
        optional_update_count: devices
            .iter()
            .flat_map(|d| &d.candidates)
            .filter(|c| c.recommendation_state == "Optional")
            .count()
            .try_into()
            .unwrap_or(u32::MAX),
        vendor_managed_update_count: devices
            .iter()
            .flat_map(|d| &d.management_authorities)
            .filter(|m| {
                m.update_availability == "UpdateAvailable"
                    && !m.update_evidence_candidate_id.is_empty()
            })
            .count()
            .try_into()
            .unwrap_or(u32::MAX),
        status_unknown_count: devices
            .iter()
            .filter(|d| {
                matches!(
                    d.update_status.as_str(),
                    "NoUpdateFoundFromCheckedSources"
                        | "UpdateStatusUnknownOffline"
                        | "ProviderUnavailable"
                        | "UpdateStatusUnknown"
                )
            })
            ...
            .try_into()
            .unwrap_or(u32::MAX),
        management_authority_count: devices
            .iter()
            .map(|d| d.management_authorities.len())
            .sum::<usize>()
            .try_into()
            .unwrap_or(u32::MAX),
    }
...
fn driver_candidate_from_v2(candidate: DriverCandidateV2) -> DriverCandidate {
    ...
    let selectable = candidate.executable()
        && candidate.installation_mode == InstallationCapability::WindowsManaged;
    DriverCandidate {
        candidate_id: candidate.candidate_id,
        update_id: candidate.update_id,
        revision: candidate.revision,
        title: candidate.title,
        provider: candidate.provenance.package_publisher.clone(),
        manufacturer: candidate.provenance.package_publisher.clone(),
        model: String::new(),
        driver_class: candidate.driver_class,
        matched_hardware_id: candidate.matched_id,
        match_quality: match candidate.match_kind {
            MatchKind::HardwareId => "Hardware ID",
            MatchKind::CompatibleId => "Compatible ID",
            MatchKind::VendorFamily => "Vendor family",
            MatchKind::DeviceClass => "Device class",
        ...
        target_version_source: target_version_source.into(),
        selectable,
        selected_by_default: selectable
            && candidate.recommendation_state == RecommendationState::Recommended,
        vendor_managed: candidate.vendor_managed,
        firmware_managed: candidate.firmware,
        selection_policy: if candidate.firmware {
            "FirmwareManualReview"
        } else if candidate.vendor_managed {
            "OfficialVendorUtility"
        } else if selectable {
        ...
        .into(),
        authority_type: authority_type_label(candidate.authority.authority_type).into(),
        authority_provider_id: candidate.authority.provider_id,
        authority_name: candidate.authority.display_name,
        official_source: candidate.provenance.official_source,
        applicability: applicability_label(candidate.applicability).into(),
        trust_state: trust_state_label(candidate.trust_state).into(),
...
fn authority_type_label(v: AuthorityType) -> &'static str {
    match v {
        AuthorityType::WindowsUpdate => "WindowsUpdate",
        AuthorityType::Oem => "Oem",
        AuthorityType::ComponentVendor => "ComponentVendor",
        AuthorityType::VendorUtility => "VendorUtility",
        AuthorityType::ManualOfficial => "ManualOfficial",
    }
}
fn applicability_label(v: ApplicabilityState) -> &'static str {
    match v {
        ApplicabilityState::Exact => "Exact",
        ApplicabilityState::Compatible => "Compatible",
        ApplicabilityState::Contextual => "Contextual",
        ApplicabilityState::Incompatible => "Incompatible",
        ApplicabilityState::Unknown => "Unknown",
    }
}
fn trust_state_label(v: PackageTrustState) -> &'static str {
    match v {
        PackageTrustState::WindowsManaged => "WindowsManaged",
        PackageTrustState::TrustedSignature => "TrustedSignature",
        PackageTrustState::TrustedSignatureAndDigest => "TrustedSignatureAndDigest",
        PackageTrustState::UnexpectedPublisher => "UnexpectedPublisher",
        PackageTrustState::InvalidSignature => "InvalidSignature",
        PackageTrustState::Unsigned => "Unsigned",
        PackageTrustState::Unverifiable => "Unverifiable",
        PackageTrustState::TestSigned => "TestSigned",
        PackageTrustState::NotApplicable => "NotApplicable",
    }
}
fn recommendation_state_label(v: RecommendationState) -> &'static str {
    match v {
        RecommendationState::Recommended => "Recommended",
        RecommendationState::Alternative => "Alternative",
        RecommendationState::Optional => "Optional",
        RecommendationState::NotRecommended => "NotRecommended",
        RecommendationState::ManualOfficial => "ManualOfficial",
        RecommendationState::FirmwareProtected => "FirmwareProtected",
        RecommendationState::CurrentDriverPreferred => "CurrentDriverPreferred",
    }
}
fn installation_capability_label(v: InstallationCapability) -> &'static str {
    match v {
        InstallationCapability::WindowsManaged => "WindowsManaged",
        InstallationCapability::DirectTrusted => "DirectTrusted",
        InstallationCapability::OfficialUtility => "OfficialUtility",
        InstallationCapability::ManualOfficial => "ManualOfficial",
        InstallationCapability::Unsupported => "Unsupported",
    }
}
fn acquisition_capability_label(v: AcquisitionCapability) -> &'static str {
    match v {
        AcquisitionCapability::WindowsManaged => "WindowsManaged",
        AcquisitionCapability::DirectTrusted => "DirectTrusted",
        AcquisitionCapability::OfficialUtility => "OfficialUtility",
        AcquisitionCapability::ManualOfficial => "ManualOfficial",
        AcquisitionCapability::Unsupported => "Unsupported",
    }
}
fn coverage_label(v: CoverageState) -> &'static str {
    match v {
        CoverageState::CompleteForRequiredAuthorities => "CompleteForRequiredAuthorities",
        CoverageState::Partial => "Partial",
        CoverageState::Offline => "Offline",
        CoverageState::ProviderUnavailable => "ProviderUnavailable",
        CoverageState::ManualAuthorityRequired => "ManualAuthorityRequired",
...
fn gpu_policy_for_device(device: &DeviceRecord) -> Option<GpuManagement> {
    ...
    Some(GpuManagement {
        ...
        management_status: if app_installed {
            "Installed"
        } else {
            "NotInstalled"
        }
...
mod tests {
    ...
    fn firmware_offers_are_visible_but_never_selectable() {
        ...
        let d = device(
            "UEFI\\RES_{12345678-1234-1234-1234-123456789ABC}",
            "Firmware",
        );
        let snapshot = match_inventory(
            "scan".into(),
    ...
    fn d18_04_known_nvidia_display_keeps_management_separate_from_wua_package() {
        ...
        assert_eq!(
            device.management_authorities[0].provider_id,
            "component.nvidia"
        );
        assert_eq!(
            device.management_authorities[0].update_availability,
    ...
    fn d18_1_gpu_01_amd_and_intel_follow_same_no_update_truth_rule() {
        for hardware_id in ["PCI\\VEN_1002&DEV_73BF", "PCI\\VEN_8086&DEV_46A6"] {
            let snapshot = match_inventory(
                ...
                1,
                1,
                vec![device(hardware_id, "Display")],
                DiscoveryResult {
                    offers: vec![],
                    warnings: vec![],
                },
            ...
            assert_eq!(
                d.management_authorities[0].update_availability,
                "UnknownUntilVendorCheck"
            );
        }
    }
    ...
    fn wua_partial_warning_marks_partial_coverage() {
        ...
        assert_eq!(
            ...
            vec!["microsoft.windows-update:Partial"]
        );
    ...
    fn driver_snapshot_is_principal_bound() {
        ...
        for _ in 0..100 {
            if hub.snapshot_for_owner(OWNER).unwrap().state == ScanState::Ready {
                ...
                assert!(matches!(
                    hub.snapshot_for_owner(other),
                    Err(HubError::OwnershipMismatch)
                ));
                return;
            }
            std::thread::sleep(std::time::Duration::from_millis(5));
    ...
    fn offline_scan_keeps_inventory_and_marks_update_truth_unknown() {
        ...
        for _ in 0..100 {
            ...
            if snapshot.state == ScanState::Ready {
                ...
                assert_eq!(
                    snapshot.devices[0].update_status,
                    "UpdateStatusUnknownOffline"
                );
                assert_eq!(
                    snapshot.provider_status,
                    vec!["microsoft.windows-update:Offline"]
                );
                assert!(
                    snapshot
                        .warnings
                        .iter()
                        .any(|warning| warning.contains("discovery offline"))
                );
                return;
            }
            std::thread::sleep(std::time::Duration::from_millis(5));
    ...
    impl DiscoveryBackend for UpdateFailureBackend {
        ...
        fn updates(&self) -> std::result::Result<DiscoveryResult, DiscoveryFailure> {
            Err(DiscoveryFailure::Unavailable(
                "simulated WUA policy failure".into(),
            ))
        }
    }
