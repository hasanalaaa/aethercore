...
impl ReadWorkload {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::RepairAssessment => "RepairAssessment",
            Self::CleanupDiscovery => "CleanupDiscovery",
            Self::StartupDiscovery => "StartupDiscovery",
...
struct State {
    total: usize,
    by_kind: HashMap<ReadWorkload, usize>,
}
#[derive(Clone)]
pub struct ReadBudgetManager {
    max_total: usize,
    state: Arc<Mutex<State>>,
}
pub struct ReadBudgetLease {
    manager: ReadBudgetManager,
    kind: ReadWorkload,
}
impl ReadBudgetLease {
    pub fn matches(&self, kind: ReadWorkload) -> bool {
        self.kind == kind
    }
}
#[derive(Debug, Error)]
pub enum ReadBudgetError {
    #[error("read-only resource budget exhausted")]
    Exhausted,
}
...
impl ReadBudgetManager {
    ...
    pub fn try_acquire(&self, kind: ReadWorkload) -> Result<ReadBudgetLease, ReadBudgetError> {
        ...
        if s.total >= self.max_total || s.by_kind.get(&kind).copied().unwrap_or(0) >= 1 {
            return Err(ReadBudgetError::Exhausted);
        }
        s.total += 1;
        *s.by_kind.entry(kind).or_default() += 1;
        Ok(ReadBudgetLease {
            manager: self.clone(),
            kind,
        })
...
impl Drop for ReadBudgetLease {
    fn drop(&mut self) {
        let mut s = self.manager.state.lock().unwrap_or_else(|p| p.into_inner());
        s.total = s.total.saturating_sub(1);
        if let Some(v) = s.by_kind.get_mut(&self.kind) {
