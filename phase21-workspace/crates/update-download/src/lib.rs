...
impl HttpsTransport {
    ...
    pub fn download_to_file(
        ...
        if response
            .content_length()
            .is_some_and(|n| n != expected_size || n > max_bytes)
        {
        ...
        let mut cleanup = TempDownload::new(temporary.clone());
        ...
        loop {
            ...
            if total > max_bytes || total > expected_size {
                return Err(DownloadError::SizeMismatch);
            }
...
fn ensure_https(url: &str) -> Result<(), DownloadError> {
    ...
    {
        return Err(DownloadError::InvalidHttpsUrl);
    }
    let rest = &url[8..];
...
mod tests {
    use super::*;
    #[test]
    fn downloader_rejects_non_https_and_ambiguous_authorities() {
        for value in [
            ...
            assert!(ensure_https(value).is_err(), "{value}");
        }
