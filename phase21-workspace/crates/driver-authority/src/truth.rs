...
use super::{
    AcquisitionCapability, AuthorityType, CoverageState, DeviceIdentity, GpuVendor, InstallationCapability,
    MachineProfile, ProviderAuthorityClass, ProviderImplementationStatus, ProviderRegistry,
};

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[serde(rename_all = "camelCase")]
pub enum ManagementAuthorityAvailability { Available, Installed, NotInstalled, ProviderUnavailable }

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[serde(rename_all = "camelCase")]
pub enum UpdateAvailabilityEvidence { UnknownUntilVendorCheck, UpdateAvailable, NoUpdateReported, ProviderUnavailable }
...
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[serde(rename_all = "camelCase")]
pub enum AuthorityEvaluationState { Evaluated, PartiallyEvaluated, Offline, Unavailable, Unsupported, ManualRequired, Unknown }
...
impl DeviceAuthorityCoverage {
    pub fn from_evaluations(required: &[AuthorityRequirement], evaluations: &[AuthorityEvaluation]) -> Self {
        let mut out = Self {
        ...
        for req in required {
            let eval = evaluations.iter().find(|e| e.provider_id == req.provider_id);
            match eval.map(|e| e.state).unwrap_or(AuthorityEvaluationState::Unknown) {
                AuthorityEvaluationState::Evaluated => out.evaluated_authorities.push(req.provider_id.clone()),
                AuthorityEvaluationState::PartiallyEvaluated => { out.evaluated_authorities.push(req.provider_id.clone()); saw_partial = true; }
                AuthorityEvaluationState::Offline => { out.unavailable_authorities.push(req.provider_id.clone()); saw_offline = true; }
                AuthorityEvaluationState::Unavailable => { out.unavailable_authorities.push(req.provider_id.clone()); saw_unavailable = true; }
                AuthorityEvaluationState::Unsupported => { out.unsupported_authorities.push(req.provider_id.clone()); saw_partial = true; }
                AuthorityEvaluationState::ManualRequired => { out.manual_authorities.push(req.provider_id.clone()); saw_manual = true; }
                AuthorityEvaluationState::Unknown => { out.unavailable_authorities.push(req.provider_id.clone()); saw_unknown = true; }
            }
        }
    ...
    pub fn permits_strong_up_to_date(&self) -> bool {
        self.completeness == CoverageState::CompleteForRequiredAuthorities
            ...
            && self.required_authorities.iter().all(|id| self.evaluated_authorities.contains(id))
    }
}

pub fn required_authorities(device: &DeviceIdentity, machine: &MachineProfile, registry: &ProviderRegistry) -> Vec<AuthorityRequirement> {
    let mut required = Vec::new();
    if registry.windows_update().is_some() {
        required.push(AuthorityRequirement { provider_id: "microsoft.windows-update".into(), authority_type: AuthorityType::WindowsUpdate, reason: "platform driver authority".into() });
    }
    if let Some(oem) = registry.oem_provider_for_machine(machine) {
        required.push(AuthorityRequirement { provider_id: oem.provider_id.clone(), authority_type: AuthorityType::Oem, reason: "machine OEM authority applies".into() });
    }
    for component in registry.component_providers_for_device(device) {
        required.push(AuthorityRequirement { provider_id: component.provider_id.clone(), authority_type: AuthorityType::ComponentVendor, reason: "component vendor authority applies".into() });
    }
...
pub fn evaluate_required_authorities(required: &[AuthorityRequirement], registry: &ProviderRegistry, windows_update_state: CoverageState) -> Vec<AuthorityEvaluation> {
...
pub fn management_authority_for_gpu(vendor: GpuVendor, app_installed: bool, official_url: &str, app_name: &str) -> DriverManagementAuthority {
    let provider_id = match vendor { GpuVendor::Nvidia => "component.nvidia", GpuVendor::Amd => "component.amd", GpuVendor::Intel => "component.intel" };
    DriverManagementAuthority {
        ...
        availability: if app_installed { ManagementAuthorityAvailability::Installed } else { ManagementAuthorityAvailability::NotInstalled },
...
pub fn management_authority_for_registry_provider(provider_id: &str, registry: &ProviderRegistry) -> Option<DriverManagementAuthority> {
    let provider = registry.by_id(provider_id)?;
    if provider.authority_class == ProviderAuthorityClass::WindowsUpdate { return None; }
    ...
    let (acquisition, installation) = match provider.implementation_status {
        ProviderImplementationStatus::OfficialUtility => (AcquisitionCapability::OfficialUtility, InstallationCapability::OfficialUtility),
        ProviderImplementationStatus::ManualOfficial => (AcquisitionCapability::ManualOfficial, InstallationCapability::ManualOfficial),
        ProviderImplementationStatus::Implemented => (AcquisitionCapability::Unsupported, InstallationCapability::Unsupported),
        ProviderImplementationStatus::UnsupportedAutomation => (AcquisitionCapability::Unsupported, InstallationCapability::Unsupported),
    };
    Some(DriverManagementAuthority {
        provider_id: provider.provider_id.clone(), authority_type, display_name: provider.provider_family.clone(),
        official_authority: provider.official_authority.clone(), official_url: String::new(),
        availability: ManagementAuthorityAvailability::Available,
        update_availability: UpdateAvailabilityEvidence::UnknownUntilVendorCheck,
        acquisition_capability: acquisition, installation_capability: installation, update_evidence_candidate_id: String::new(),
    })
}
...
mod tests {
    use super::*;
    use crate::{builtin_provider_registry, MachineKind};

    #[test]
    fn utility_presence_never_proves_update_availability() {
        let authority = management_authority_for_gpu(GpuVendor::Nvidia, true, "https://www.nvidia.com/", "NVIDIA App");
        assert_eq!(authority.availability, ManagementAuthorityAvailability::Installed);
        assert_eq!(authority.update_availability, UpdateAvailabilityEvidence::UnknownUntilVendorCheck);
    ...
    fn oem_and_gpu_require_device_aware_authority_coverage() {
        ...
        let device = DeviceIdentity { class_name:"Display".into(), vendor_id:"10DE".into(), ..Default::default() };
        let machine = MachineProfile { manufacturer:"Dell Inc.".into(), machine_kind:MachineKind::Oem, ..Default::default() };
        let required = required_authorities(&device, &machine, &registry);
        assert!(required.iter().any(|r| r.provider_id == "microsoft.windows-update"));
        assert!(required.iter().any(|r| r.provider_id == "oem.dell"));
        assert!(required.iter().any(|r| r.provider_id == "component.nvidia"));
        let eval = evaluate_required_authorities(&required, &registry, CoverageState::CompleteForRequiredAuthorities);
        let coverage = DeviceAuthorityCoverage::from_evaluations(&required, &eval);
        assert_eq!(coverage.completeness, CoverageState::ManualAuthorityRequired);
