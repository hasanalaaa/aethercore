...
Ledger rows: `DBT-P63-012`; new `DBT-P75-024`…`027` (end of this file).
...
- `cargo clippy -p <all seven crates> --all-targets --locked -- -D warnings` — clean. The
  earlier findings in untouched code are gone since `mac-clippy` (#26) merged; the lane was replayed
  onto `main` at `bcd7211`.
- Red-before re-measured after the replay: with `main`'s `cleaner/src/lib.rs` and this lane's tests,
  3 cleaner tests fail (`left: [("Completed", "Verifying", false)]`).
- `cargo test` over the seven crates: 22 suites, 96 passed, 0 failed.
...
1. `DBT-P75-024` — `startup-manager` had the same transition-then-journal ordering (`fail_execution`, the Completed
   path) — not in the ledger. Fixed here; proof: the startup-manager test above. Proposed: CLOSED.
2. `DBT-P75-025` — The stream watchers decided terminality from a second engine read and could stop after streaming
   ...
   verification = code.
3. `DBT-P75-026` — Runtime failure after mutation in `cleaner` / `system-repair` wrote no recovery record. Fixed
   here; proof: the two recovery-record assertions above. Proposed: CLOSED.
4. `DBT-P75-027` — Crate-level SQL probes need a `rusqlite` dev-dependency in `cleaner` and `system-repair`
