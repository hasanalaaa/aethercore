...
| `DBT-P75-020` | `2da7c0c` (query moved back into the collector for the phase-6 static gate) | Kernel-Power events of every id filled the 128-event cap, though only id 41 is classified | the XPath takes Kernel-Power `EventID=41` only | windows-2025 throwaway run `36186229655`: the old query returned 9 events, all Kernel-Power 109/172/577 and none 41; the Event Log accepts the new query (`NoMatchingEventsFound`) |
