...
* **The red run before the fix was NOT obtained.** A test-only branch plus a
  `workflow_dispatch` was prepared; pushing that extra branch was refused by this
  session's permission policy. Anyone with push rights can reproduce it: branch from
  `origin/main`, add only `tests/keys_generate_windows_rng.rs`, regenerate the seal,
