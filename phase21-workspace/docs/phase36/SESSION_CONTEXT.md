...
| 3 `DBT-P41-001` on a clean machine | **IN PROGRESS — implemented, record written, VM work next** | §48.3 — `Launch` condition in `Product.wxs` + `vc_redist` chained in `Bundle.wxs`; static gate 346/21 `NEW_FAILURES=[]`; destructive record below, committed before the first destructive step |
