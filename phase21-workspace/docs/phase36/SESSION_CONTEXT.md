...
| 2 the icon, closed | **DONE — `DBT-P36-004` CLOSED** | §48.2 — the mark ships as deliberate artwork; 17 icons regenerated byte-identically from `65f4417…`, full set wired into `tauri.conf.json` (3 entries -> 17), `provisional` retired from `SOURCE.json`, static gate **346/21, `NEW_FAILURES=[]`**. `icon.ico` is byte-identical to the file P47 traced into the MSI `Icon` table (`4a49d865…`, 12,928 B); the MSI re-verification is folded into §48.3's VM build so the VM is touched once. New `DBT-P48-001` raised for the in-app mark |
| 3 `DBT-P41-001` on a clean machine | **IN PROGRESS — built once, ICE03 found and fixed, rebuilding** | §48.3 — `Launch` condition + `vc_redist` chained; build exit 0, payload PASS, 16 File rows, 4 LaunchCondition rows, but `wix msi validate` emitted **1** line (ICE03 string overflow, 360 chars into a 255-char column). Message shortened to 245; rebuild next |
