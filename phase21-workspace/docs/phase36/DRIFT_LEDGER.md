...
| DBT-P36-004 | Icon set was `COMPILE_ONLY_PLACEHOLDER=True`, AI-generated, not the product mark. | **Closed 2026-09-05 (Phase 48, item 2).** `design/icon/aethercore-mark.svg` ships as the product's mark: the geometric AE ligature, monochrome `#5C7CFA` (`--role-interactive`) on `#030508`. It is deliberate artwork, not a placeholder. It is **monochrome by design** -- plum is the denied-by-policy role, and spending it on an icon would cost it its meaning. The owner may replace it at any time by dropping a new SVG through `tools/icon-pipeline/build-icons.mjs`, which regenerates all seventeen files in one command; `apps/desktop/icons/SOURCE.json` records which SVG made the committed set, by path and sha256. The `provisional` flag that marked the set as a stand-in is removed rather than flipped to `false`, because a generator cannot know an editorial decision. See SESSION_CONTEXT.md §48.2. |
