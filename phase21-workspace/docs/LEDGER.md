...
| `DBT-P59-002` | **`p30-fulltree-spot-hash-ok` cannot fail: the audit regenerates the ledger from the tree it then compares against** | OPEN — the instrument-lies class, distinct from the reader class | `scripts/phase30-adversarial-audit.py:215-243`. It loads the committed ledger and compares the tree to it (`:216-226`), **discards that result** (`spot_ok` and `diverged` are reset at `:232-233`), runs `scripts/_build_p30_patch.py` which rewrites `PHASE_30_EXPECTED_FULL_SHA256.json` from the current tree (`:228-230`), then compares the tree to that (`:234-242`) and reports the verdict. The seven ledger entries with no file in the tree — `.DS_Store`, `.github/dependabot.yml`, `.github/workflows/ci.yml`, `fuzz.yml`, `release.yml` and two more — simply drop out of the regenerated ledger. The `if p.is_file()` skip at `:222`/`:238` is fail-open too, but repairing it changes nothing while the expected value is minted from the measured value. This regeneration is also the tracked-file write P58 recorded: running this audit rewrites four files under `PHASE_30_BINARY_SAFE_PATCH/` and `SBOM.cdx.json`. Not fixed P59: the first loop is dead code and the second needs a fixed expectation, which is a decision about what the P30 seal means | any |
| `DBT-P59-003` | **`MANIFEST.sha256` has not described this source tree for a long time, so `OMEGA-RB-003` is permanently raised and `omega-evidence.py` can never reach a clean verdict** | OPEN — recorded P59, not fixed | Measured P59 with the same rules `scripts/regenerate-source-manifest.py` uses (exclude `.git`, `target`, `node_modules`, `out`, `__pycache__`, `.pyc`/`.pyo`): the manifest lists **551** files, the tree holds **1,457**. **909 files in the tree are unlisted** (`Cargo.lock`, `pnpm-lock.yaml`, `SBOM.cdx.json`, `EXTENSIBILITY.md`, …), **200 listed files have a different hash**, and **3 listed files are gone** — `.github/dependabot.yml`, `.github/workflows/ci.yml` and `.github/workflows/release.yml`, which P58 moved to the repository root without regenerating. Regenerating it is a 909-file commit and a decision about what the delivered-source seal is meant to cover, which is why P59 recorded it rather than doing it | any |
