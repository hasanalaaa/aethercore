...
| `DBT-P49-002` | the consumer installer's UI: "AetherCore Setup Setup", stock WiX logo, no version, no license, progress stuck on "Initializing..." | **source fixed; screenshots owed** | `docs/phase55/INSTALLER-UI.md`. Name, LogoFile and `MsiPackage/@DisplayName` corrected in `Bundle.wxs`; logo hash-checked against `SOURCE.json` at build time. **Licence deliberately still absent — the repository has no licence text; owner action, not invented** | this machine (Item 4.D) |
