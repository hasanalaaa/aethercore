...
use aethercore_windows_foundation::{BackgroundThreadMode, ComApartment, OwnedHandle, OwnedServiceHandle, ThreadImpersonation};
use chrono::Utc;
use windows::{
    ...
    Win32::{
        ...
        Networking::NetworkListManager::{
            ...
            RemoteDesktop::{
                ...
                WTSQueryUserToken, WTSINFOEXW, WTSActive, WTSSessionInfoEx,
            },
            Services::{
        ...
        UI::Shell::{
            ...
            QUNS_NOT_PRESENT, QUNS_PRESENTATION_MODE, QUNS_RUNNING_D3D_FULL_SCREEN,
        },
    },
};

use crate::{NetworkCost, PresentationState, ServicingState, SystemState, SystemStateProbe, ThermalPressure};
...
#[derive(Clone, Copy)]
struct SlowSignals { sampled: Instant, network: NetworkCost, servicing: ServicingState, thermal: ThermalPressure }
...
impl Default for WindowsSystemStateProbe {
    fn default() -> Self {
        Self { slow: Arc::new(Mutex::new(None)), slow_refreshing: Arc::new(AtomicBool::new(false)) }
    }
}
...
impl WindowsSystemStateProbe {
    ...
    fn slow_signals(&self, refresh: bool) -> SlowSignals {
        ...
        if let Some(value) = *cache {
            if !refresh || value.sampled.elapsed() < Duration::from_secs(2) { return value; }
        } else if !refresh {
            return SlowSignals { sampled: Instant::now(), network: NetworkCost::Unknown, servicing: ServicingState::Unknown, thermal: ThermalPressure::Unknown };
        }
    ...
    fn refresh_slow_nonblocking_impl(&self) {
        if self.slow_refreshing.compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire).is_err() {
            return;
        }
        let slow = self.slow.clone();
        let refreshing = self.slow_refreshing.clone();
        let spawn = thread::Builder::new().name("aether-idle-slow-signals".into()).spawn(move || {
            let value = catch_unwind(AssertUnwindSafe(|| run_in_background_mode(|| Ok(collect_slow_signals()))))
                .ok()
                .and_then(Result::ok);
...
impl WindowsSystemStateProbe {
    fn sample_impl(&self, refresh_slow: bool) -> Result<SystemState, String> {
        unsafe {
            ...
            if session_id == NO_CONSOLE_SESSION { return Err("no active console session".into()); }

            let mut raw_token = HANDLE::default();
            WTSQueryUserToken(session_id, &mut raw_token).map_err(|e| format!("WTSQueryUserToken: {e}"))?;
            let token = OwnedHandle::new(raw_token);
            let result = (|| {
                let principal = inspect_session_token(token.get(), session_id).map_err(|e| e.to_string())?;
                ...
                GetSystemPowerStatus(&mut power).map_err(|e| format!("GetSystemPowerStatus: {e}"))?;
                let slow = self.slow_signals(refresh_slow);
                let session_unlocked = active && unlocked && !matches!(presentation, PresentationState::LockedOrAbsent);
                Ok(SystemState {
...
impl SystemStateProbe for WindowsSystemStateProbe {
    fn sample(&self) -> Result<SystemState, String> { self.sample_impl(true) }
    fn sample_fast(&self) -> Result<SystemState, String> { self.sample_impl(false) }
    fn refresh_slow_nonblocking(&self) { self.refresh_slow_nonblocking_impl(); }
}
...
fn query_idle(session_id: u32) -> Result<(Duration, bool, bool), String> {
    unsafe {
        ...
        if raw.is_null() { return Err("WTSSessionInfoEx returned a null buffer".into()); }
        let result = (|| {
            if bytes < size_of::<WTSINFOEXW>() as u32 { return Err("WTSSessionInfoEx returned a truncated buffer".into()); }
            let info = &*(raw.0.cast::<WTSINFOEXW>());
            if info.Level != 1 { return Err("WTSSessionInfoEx returned an unsupported level".into()); }
            let level = info.Data.WTSInfoExLevel1;
            if level.SessionId != session_id { return Err("WTSSessionInfoEx session id mismatch".into()); }
            ...
            Ok((Duration::from_secs((ticks / FILETIME_TICKS_PER_SECOND) as u64), level.SessionState == WTSActive, unlocked))
        })();
    ...
    unsafe {
        ...
        (|| {
            ...
            let services: IWbemServices = locator
                .ConnectServer(&BSTR::from("ROOT\\WMI"), &BSTR::new(), &BSTR::new(), &BSTR::new(), 0, &BSTR::new(), None)
                .map_err(|e| format!("thermal ConnectServer: {e}"))?;
            CoSetProxyBlanket(
                &services, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, PCWSTR::null(),
            ...
            loop {
                if Instant::now() >= deadline { return Err("thermal WMI enumeration deadline exceeded".into()); }
                ...
                if status.0 == WBEM_S_FALSE.0 {
                    if returned != 0 || values[0].is_some() { return Err("thermal WMI returned terminal WBEM_S_FALSE with data".into()); }
                    break;
                }
                if status.is_err() { return Err(format!("thermal WMI Next failed: 0x{:08X}", status.0 as u32)); }
                if returned > 1 { return Err("thermal WMI returned more objects than the supplied slice".into()); }
                match (returned, values[0].take()) {
                    (0, None) if status.0 == WBEM_S_TIMEDOUT.0 => continue,
                    (0, None) => return Err("thermal WMI returned zero objects without timeout/end status".into()),
                    (1, Some(object)) => {
                        zones += 1;
                        if zones > MAX_THERMAL_ZONES { return Err("thermal WMI exceeded bounded zone count".into()); }
                    ...
                    (0, Some(_)) => return Err("thermal WMI populated an object while reporting zero returned".into()),
                    (1, None) => return Err("thermal WMI reported one object without supplying it".into()),
...
fn max_thermal_pressure(a: ThermalPressure, b: ThermalPressure) -> ThermalPressure {
    fn rank(v: ThermalPressure) -> u8 { match v { ThermalPressure::Unknown=>0, ThermalPressure::Normal=>1, ThermalPressure::Elevated=>2, ThermalPressure::Critical=>3 } }
    if rank(b) > rank(a) { b } else { a }
}

fn classify_thermal_zone(current_tenths_kelvin: u32, critical_tenths_kelvin: u32) -> ThermalPressure {
    ...
    if margin <= 50 { ThermalPressure::Critical }
    else if margin <= 150 { ThermalPressure::Elevated }
...
fn thermal_prop_u32(object: &IWbemClassObject, name: &str) -> Option<u32> {
    unsafe {
        ...
        object.Get(PCWSTR(wide.as_ptr()), 0, &mut value, None, None).ok()?;
        u32::try_from(&value).ok().or_else(|| u16::try_from(&value).ok().map(u32::from))
    }
}
...
fn probe_network_cost() -> Result<NetworkCost, String> {
    unsafe {
        ...
        (|| {
            let manager: INetworkCostManager = CoCreateInstance(&NETWORK_LIST_MANAGER_CLSID, None, CLSCTX_ALL)
                .map_err(|e| format!("NetworkListManager: {e}"))?;
            let mut cost = 0u32;
            manager.GetCost(&mut cost, ptr::null()).map_err(|e| format!("INetworkCostManager::GetCost: {e}"))?;
            ...
            Ok(if cost & metered_mask != 0 { NetworkCost::Metered } else { NetworkCost::Unmetered })
        })()
    }
}
...
fn probe_servicing_state() -> Result<ServicingState, String> {
    unsafe {
        ...
        for name in ["TrustedInstaller", "UsoSvc", "WaaSMedicSvc"] {
            ...
            let service = match OpenServiceW(scm.get(), PCWSTR(name_w.as_ptr()), SERVICE_QUERY_STATUS) {
            ...
            if QueryServiceStatusEx(service.get(), SC_STATUS_PROCESS_INFO, Some(bytes), &mut needed).is_err() {
                uncertain = true;
                continue;
            }
            observed += 1;
        ...
        Ok(if observed == 0 || uncertain { ServicingState::Unknown } else { ServicingState::Idle })
    }
}
...
mod tests {
    use super::*;
    #[test] fn thermal_zone_uses_trip_point_margin_without_invented_absolute_thresholds() {
        assert_eq!(classify_thermal_zone(3_000, 3_300), ThermalPressure::Normal);
        assert_eq!(classify_thermal_zone(3_200, 3_300), ThermalPressure::Elevated);
        assert_eq!(classify_thermal_zone(3_260, 3_300), ThermalPressure::Critical);
        assert_eq!(classify_thermal_zone(0, 0), ThermalPressure::Unknown);
    }
    #[test] fn worst_thermal_zone_wins() {
        assert_eq!(max_thermal_pressure(ThermalPressure::Normal, ThermalPressure::Elevated), ThermalPressure::Elevated);
        assert_eq!(max_thermal_pressure(ThermalPressure::Critical, ThermalPressure::Normal), ThermalPressure::Critical);
    }
