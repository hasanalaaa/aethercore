...
use std::{path::{Path, PathBuf}, sync::{Arc, atomic::{AtomicBool, Ordering}}, thread};
...
use aethercore_operation_kernel::{MutationLease, MutationWorkload, ProgressTelemetry, ProgressTelemetryStore};
use aethercore_operation_engine::{DriverEvidence, DriverInstallAction, OperationEngine, PlanState, PlanView};
...
pub trait InstallPlatform: Send + Sync + 'static {
    ...
    fn verify_devices(&self, instance_ids: &[String]) -> std::result::Result<Vec<DeviceVerification>, String>;
    ...
    fn backup_driver(&self, inf: &str, destination: &Path) -> std::result::Result<BackupEvidence, String>;
    fn execute_wua(
        &self,
...
impl InstallPlatform for WindowsInstallPlatform {
    fn boot_marker_ms(&self) -> std::result::Result<i64, String> { platform::boot_marker_ms() }
    fn verify_devices(&self, ids: &[String]) -> std::result::Result<Vec<DeviceVerification>, String> { aethercore_windows_pnp::verify_device_instances(ids).map_err(|e| e.to_string()) }
    fn begin_restore(&self, plan_id: &str) -> std::result::Result<RestorePointEvidence, String> { aethercore_restore_point::begin_driver_install(plan_id).map_err(|e| e.to_string()) }
    fn end_restore(&self, seq: i64, desc: &str) -> std::result::Result<(), String> { aethercore_restore_point::end_driver_install(seq, desc).map_err(|e| e.to_string()) }
    fn cancel_restore(&self, seq: i64, desc: &str) -> std::result::Result<(), String> { aethercore_restore_point::cancel_driver_install(seq, desc).map_err(|e| e.to_string()) }
    fn backup_driver(&self, inf: &str, destination: &Path) -> std::result::Result<BackupEvidence, String> { aethercore_driver_backup::export_driver_package(inf, destination).map_err(|e| e.to_string()) }
    fn execute_wua(&self, ids: &[UpdateIdentity], progress: &mut dyn FnMut(WuaProgress), before: &mut dyn FnMut() -> std::result::Result<(), String>) -> std::result::Result<WuaExecutionResult, String> {
        aethercore_windows_update::execute_driver_updates(ids, |p| progress(p), || before()).map_err(|e| e.to_string())
    }
}
...
impl DriverInstallCoordinator {
    pub fn new(engine: Arc<OperationEngine>, hub: Arc<DriverHub>, db: Arc<Database>, product_data_root: PathBuf) -> Self {
        Self::with_platform_and_telemetry(engine, hub, db, product_data_root, Arc::new(WindowsInstallPlatform), ProgressTelemetryStore::new())
    }

    pub fn with_platform(engine: Arc<OperationEngine>, hub: Arc<DriverHub>, db: Arc<Database>, product_data_root: PathBuf, platform: Arc<dyn InstallPlatform>) -> Self {
        Self::with_platform_and_telemetry(engine, hub, db, product_data_root, platform, ProgressTelemetryStore::new())
    }

    pub fn with_platform_and_telemetry(engine: Arc<OperationEngine>, hub: Arc<DriverHub>, db: Arc<Database>, product_data_root: PathBuf, platform: Arc<dyn InstallPlatform>, telemetry: ProgressTelemetryStore) -> Self {
        Self { engine, hub, db, platform, product_data_root, running: Arc::new(AtomicBool::new(false)), telemetry }
    }

    pub fn create_plan(&self, owner_principal_key: &str, scan_id: &str, inventory_epoch: u64, candidate_ids: &[String]) -> Result<PlanView> {
        let selection = self.hub.resolve_install_selection(owner_principal_key, scan_id, inventory_epoch, candidate_ids)?;
        let actions = selection.into_iter().map(action_from_selection).collect();
        Ok(self.engine.create_driver_install_plan(owner_principal_key, inventory_epoch, scan_id, actions)?)
    }
    ...
    pub fn start_with_lease(
        ...
        if !lease.matches(MutationWorkload::DriverInstall, plan_id, owner_principal_key) {
    ...
    fn start_inner(
        ...
        if actions.is_empty() { return Err(InstallError::InvalidPlan); }

        if self.running.swap(true, Ordering::SeqCst) { return Err(InstallError::Busy); }
        ...
        self.status(owner_principal_key, Some(plan_id))?.ok_or(InstallError::InvalidPlan)
    }

    pub fn status(&self, owner_principal_key: &str, plan_id: Option<&str>) -> Result<Option<InstallStatus>> {
        ...
        let plan = self.engine.get_plan_for_owner(&execution.plan_id, owner_principal_key)?;
        let items = self.db.install_items(&execution.plan_id)?.into_iter().map(item_status).collect();
        let live = self
            .telemetry
    ...
    pub fn recovery_history(&self, owner_principal_key: &str, limit: usize) -> Result<Vec<RecoveryEntry>> {
        Ok(self.db.recovery_records_for_owner(owner_principal_key, limit)?.into_iter().map(|r| RecoveryEntry {
            seq:r.seq, plan_id:r.plan_id, severity:r.severity, kind:r.kind, summary:r.summary, detail:r.detail,
            restore_point_sequence:r.restore_point_sequence.unwrap_or(0), backup_root:r.backup_root, created_unix_ms:r.created_unix_ms,
        }).collect())
    }
    ...
    pub fn recover_incomplete(&self) -> Result<()> {
        ...
        for plan in self.engine.recoverable_plans()? {
            let Some(mut execution) = self.db.get_execution(&plan.id)? else { continue; };
            match plan.state {
                PlanState::RebootPending if execution.reboot_boot_marker_ms != 0 && boot_changed(execution.reboot_boot_marker_ms, current_boot) => {
                    self.engine.transition(&plan.id, PlanState::RebootPending, PlanState::Resuming, "Windows reboot detected; resuming verification only")?;
                    self.engine.transition(&plan.id, PlanState::Resuming, PlanState::Verifying, "re-interrogating devices after reboot")?;
                ...
                PlanState::Executing | PlanState::Verifying => {
                    ...
                    execution.stage = "RecoveryRequired".into(); execution.updated_unix_ms = now_ms();
                    self.db.upsert_execution(&execution)?;
                    let _ = self.engine.transition(&plan.id, plan.state, PlanState::Failed, "interrupted mutation was not replayed");
                ...
                PlanState::Preflight | PlanState::Protected => {
                    if let Some(seq) = execution.restore_point_sequence {
                        if !execution.mutation_started { let _ = self.platform.cancel_restore(seq, &aethercore_restore_point::description_for_plan(&plan.id)); }
                    }
                    execution.recovery_required = false; execution.failure_message = "Installation stopped before driver mutation.".into(); execution.stage = "FailedSafe".into(); execution.updated_unix_ms=now_ms(); execution.completed_unix_ms=Some(now_ms());
                    self.db.upsert_execution(&execution)?;
                    let _ = self.engine.transition(&plan.id, plan.state, PlanState::Failed, "service restart before driver mutation");
                }
                _ => {}
            }
        }
    ...
    fn initialize_records(&self, plan_id: &str, actions: &[DriverInstallAction], now: i64) -> Result<()> {
        let execution = ExecutionRecord { plan_id:plan_id.into(), stage:"Preflight".into(), detail:"Revalidating selected devices before any change.".into(), started_unix_ms:now, updated_unix_ms:now, ..Default::default() };
        self.db.upsert_execution(&execution)?;
        for action in actions {
            self.db.upsert_install_item(&InstallItemRecord { plan_id:plan_id.into(), candidate_id:action.candidate_id.clone(), update_id:action.update_id.clone(), revision:action.revision, instance_id:action.instance_id.clone(), title:action.update_title.clone(), stage:"Queued".into(), before_driver_json:serde_json::to_string(&action.current_driver).unwrap_or_default(), before_problem_code:action.current_problem_code, updated_unix_ms:now, ..Default::default() })?;
        }
        self.db.add_checkpoint(plan_id,"","preflight-started","{}",now)?;
        Ok(())
    }

    fn run_plan(&self, owner_principal_key: &str, plan_id: &str, actions: Vec<DriverInstallAction>) -> Result<()> {
        self.preflight(plan_id, &actions)?;
        let mut execution = self.db.get_execution(plan_id)?.ok_or(InstallError::InvalidPlan)?;
        ...
        self.db.upsert_execution(&execution)?;
        self.db.add_checkpoint(plan_id, "", "preflight-complete", "{}", now_ms())?;
        ...
        let mut before = || -> std::result::Result<(), String> {
            if before_called { return Err("install mutation barrier invoked more than once".into()); }
            before_called = true;

            let restore = self.platform.begin_restore(plan_id).map_err(|e| format!("restore point: {e}"))?;
            ...
            let mut r = self.db.get_execution(plan_id).map_err(|e| e.to_string())?.ok_or("execution record missing")?;
            ...
            self.backup_all(plan_id, &actions, &backup_root).map_err(|e| e.to_string())?;
            ...
            self.db.add_checkpoint(plan_id, "", "protected", "{}", now_ms()).map_err(|e| e.to_string())?;
            ...
            r.updated_unix_ms = now_ms();
            self.db.upsert_execution(&r).map_err(|e| e.to_string())?;
            self.db.add_checkpoint(plan_id, "", "mutation-started", "{}", now_ms()).map_err(|e| e.to_string())?;
            self.engine.transition(
                plan_id,
                PlanState::Protected,
                PlanState::Executing,
        ...
        let mut on_progress = |p: WuaProgress| { let _ = self.record_progress(owner_principal_key, plan_id, &actions, &identities, p); };
        let wua = platform.execute_wua(&identities, &mut on_progress, &mut before).map_err(InstallError::Execution);
        ...
        match wua {
            Ok(result) => {
                if !execution.mutation_started {
                    return Err(InstallError::Execution("WUA returned without crossing the mutation barrier".into()));
                }
                self.apply_wua_results(plan_id, &actions, &result)?;
                self.engine.transition(plan_id, PlanState::Executing, PlanState::Verifying, "WUA execution finished; re-interrogating devnodes")?;
                ...
                if let Some(restore) = restore_evidence.as_ref() {
                    ...
                    return Err(InstallError::Protection("restore-point evidence was lost after the mutation barrier".into()));
                }
            ...
            Err(error) => {
                ...
                if execution.mutation_started {
                    ...
                    self.db.upsert_execution(&execution)?;
                    let _ = self.engine.transition(plan_id, PlanState::Executing, PlanState::Failed, "WUA failed after driver mutation began");
                    self.add_recovery(
                        &execution,
                        "warning",
                        "install-failure",
                    ...
                    if current == PlanState::Protected {
                        let _ = self.engine.transition(plan_id, PlanState::Protected, PlanState::Failed, "WUA failed before mutation");
                    } else if current == PlanState::Preflight {
                        let _ = self.engine.transition(plan_id, PlanState::Preflight, PlanState::Failed, "WUA failed before protection/mutation");
                    }
                    execution.failure_message = error.to_string();
    ...
    fn close_restore_transaction(&self, plan_id: &str, execution: &mut ExecutionRecord, restore: &RestorePointEvidence, cancel: bool) -> Result<()> {
        let outcome = if cancel {
            self.platform.cancel_restore(restore.sequence_number, &restore.description)
        } else {
            self.platform.end_restore(restore.sequence_number, &restore.description)
        };
        match outcome {
            Ok(()) => {
                self.db.add_checkpoint(
                    plan_id,
                    "",
                    if cancel { "restore-point-cancelled" } else { "restore-point-ended" },
    ...
    fn preflight(&self, _plan_id: &str, actions: &[DriverInstallAction]) -> Result<()> {
        let ids = actions.iter().map(|a| a.instance_id.clone()).collect::<Vec<_>>();
        let fresh = self.platform.verify_devices(&ids).map_err(InstallError::Preflight)?;
        ...
        for action in actions {
            if action.driver_class.eq_ignore_ascii_case("firmware") {
                return Err(InstallError::Preflight("firmware remains protected from generic driver mutation".into()));
            }
            if action.installation_mode != "WindowsManaged"
                || action.authority_type != "WindowsUpdate"
                || action.trust_state != "WindowsManaged"
                || !matches!(action.recommendation_state.as_str(), "Recommended" | "Optional")
            {
                return Err(InstallError::Preflight(format!("sealed driver authority is not executable: {} / {}", action.authority_type, action.installation_mode)));
            }
            if action.provenance_digest != action_provenance_digest(action) {
                return Err(InstallError::Preflight(format!("sealed driver provenance changed: {}", action.candidate_id)));
            }
            let v = fresh
                .iter()
                .find(|v| v.instance_id.eq_ignore_ascii_case(&action.instance_id))
                .ok_or_else(|| InstallError::Preflight(format!("device disappeared: {}", action.instance_id)))?;
            if !v.present {
                return Err(InstallError::Preflight(format!("device is no longer present: {}", action.instance_id)));
            }
            if v.class_name.eq_ignore_ascii_case("firmware") {
                return Err(InstallError::Preflight("device class changed to protected firmware".into()));
            }
            if !action.driver_class.is_empty() && !v.class_name.eq_ignore_ascii_case(&action.driver_class) {
                return Err(InstallError::Preflight(format!("device class changed since scan: {}", action.instance_id)));
            }
            let wanted = normalize_pnp_id(&action.matched_hardware_id);
            let id_still_bound = v.hardware_ids.iter().chain(v.compatible_ids.iter())
                .any(|id| normalize_pnp_id(id) == wanted);
            if wanted.is_empty() || !id_still_bound {
                return Err(InstallError::Preflight(format!("matched hardware identity changed since scan: {}", action.instance_id)));
            }
            if !drivers_equivalent(&v.driver, &action.current_driver) {
                return Err(InstallError::Preflight(format!("bound driver changed since scan: {}", action.instance_id)));
            }
            let current_problem = if v.status.has_problem { v.status.problem_code } else { 0 };
            if current_problem != action.current_problem_code {
                return Err(InstallError::Preflight(format!("device problem state changed since scan: {}", action.instance_id)));
            }
        }
        Ok(())
    }

    fn backup_all(&self, plan_id:&str, actions:&[DriverInstallAction], root:&Path) -> Result<()> {
        for action in actions {
            let mut item=self.db.install_items(plan_id)?.into_iter().find(|i|i.candidate_id==action.candidate_id).ok_or(InstallError::InvalidPlan)?;
            let Some(driver)=&action.current_driver else {
                item.stage="BackupNotApplicable".into(); item.detail="No currently bound driver package exists to export.".into(); item.updated_unix_ms=now_ms(); self.db.upsert_install_item(&item)?;
                self.db.add_checkpoint(plan_id,&action.candidate_id,"driver-backup-not-applicable",r#"{"reason":"no-bound-driver"}"#,now_ms())?;
                continue;
            };
            if !aethercore_driver_backup::validate_oem_inf_name(&driver.inf_path) {
                item.stage="BackupNotApplicable".into(); item.detail="The bound driver is not an OEM Driver Store INF, so PnPUtil export is not applicable.".into(); item.updated_unix_ms=now_ms(); self.db.upsert_install_item(&item)?;
                self.db.add_checkpoint(plan_id,&action.candidate_id,"driver-backup-not-applicable",r#"{"reason":"non-oem-inf"}"#,now_ms())?;
                continue;
            }
            let destination=candidate_backup_root(root, &action.candidate_id).map_err(|e| InstallError::Protection(e.to_string()))?;
            let evidence=self.platform.backup_driver(&driver.inf_path,&destination).map_err(InstallError::Protection)?;
            item.backup_path=evidence.backup_directory.clone(); item.stage="BackedUp".into(); item.detail=format!("Exported {} files before mutation.", evidence.file_count); item.updated_unix_ms=now_ms(); self.db.upsert_install_item(&item)?;
            self.db.add_checkpoint(plan_id,&action.candidate_id,"driver-backup-verified",&serde_json::to_string(&evidence).unwrap_or_default(),now_ms())?;
        }
        Ok(())
    }

    fn record_progress(&self, owner_principal_key:&str, plan_id:&str, actions:&[DriverInstallAction], identities:&[UpdateIdentity], p:WuaProgress) -> Result<()> {
        let mut execution=self.db.get_execution(plan_id)?.ok_or(InstallError::InvalidPlan)?;
        let now = now_ms();
        ...
        let candidate = identities.get(p.current_update_index as usize)
        ...
        if !candidate.is_empty() { execution.current_candidate_id=candidate; }
        if let Some(identity)=identities.get(p.current_update_index as usize) {
            let matched=actions.iter().filter(|a| same_update(a, identity)).collect::<Vec<_>>();
            let items=self.db.install_items(plan_id)?;
            for action in matched {
                let mut item=items.iter().find(|i|i.candidate_id==action.candidate_id).cloned().ok_or(InstallError::InvalidPlan)?;
    ...
    fn apply_wua_results(&self, plan_id:&str, actions:&[DriverInstallAction], result:&WuaExecutionResult) -> Result<()> {
        let items=self.db.install_items(plan_id)?;
        for action in actions {
            let identity=UpdateIdentity{update_id:action.update_id.clone(),revision:action.revision};
            let w=result.updates.iter().find(|w| w.identity==identity).ok_or_else(||InstallError::Execution(format!("WUA did not return a per-update result for {} revision {}", action.update_id, action.revision)))?;
            let mut item=items.iter().find(|i|i.candidate_id==action.candidate_id).cloned().ok_or(InstallError::InvalidPlan)?;
            item.result_code=w.result_code.clone(); item.hresult=w.hresult; item.reboot_required=w.reboot_required; item.stage="Installed".into(); item.progress_known=true; item.progress_percent=100; item.updated_unix_ms=now_ms(); self.db.upsert_install_item(&item)?;
        }
        Ok(())
    }

    fn finish_verification(&self, plan_id:&str, actions:&[DriverInstallAction], execution:&mut ExecutionRecord, after_reboot:bool) -> Result<()> {
        let ids=actions.iter().map(|a|a.instance_id.clone()).collect::<Vec<_>>();
        let fresh=self.platform.verify_devices(&ids).map_err(InstallError::Verification)?;
        let mut all_healthy=true;
        for action in actions {
            let v=fresh.iter().find(|v|v.instance_id.eq_ignore_ascii_case(&action.instance_id));
            let mut item=self.db.install_items(plan_id)?.into_iter().find(|i|i.candidate_id==action.candidate_id).ok_or(InstallError::InvalidPlan)?;
            if let Some(v)=v { item.after_driver_json=serde_json::to_string(&v.driver).unwrap_or_default(); item.after_problem_code=v.status.problem_code; item.verified=v.present && !v.status.has_problem; item.detail=if item.verified { "PnP reports the device present without a problem code." } else { "PnP reports a device problem after installation." }.into(); }
            else { item.verified=false; item.detail="Device is not present after installation.".into(); }
            item.stage="Verified".into(); item.updated_unix_ms=now_ms(); all_healthy &= item.verified && result_code_success(&item.result_code); self.db.upsert_install_item(&item)?;
        }
        if execution.reboot_required && !after_reboot {
            execution.stage="RebootPending".into(); execution.detail="Windows requires a restart before final verification can complete.".into(); execution.reboot_boot_marker_ms=self.platform.boot_marker_ms().unwrap_or_default(); execution.updated_unix_ms=now_ms(); self.db.upsert_execution(execution)?;
            self.engine.transition(plan_id, PlanState::Verifying, PlanState::RebootPending, "WUA or a selected update requires reboot")?;
            return Ok(());
        }
        execution.progress_known=true; execution.overall_percent=100; execution.updated_unix_ms=now_ms(); execution.completed_unix_ms=Some(now_ms()); execution.reboot_required=false;
        if all_healthy { execution.stage="Completed".into(); execution.detail="All selected devices passed post-install PnP verification.".into(); self.db.upsert_execution(execution)?; self.engine.transition(plan_id, PlanState::Verifying, PlanState::Completed, "all selected devices verified healthy")?; }
        else { execution.stage="FailedVerification".into(); execution.recovery_required=true; execution.failure_message="One or more devices did not pass post-install verification.".into(); self.db.upsert_execution(execution)?; self.engine.transition(plan_id, PlanState::Verifying, PlanState::Failed, "post-install device verification failed")?; self.add_recovery(execution,"warning","verification-failure","A device needs recovery review","The installation result or PnP health check failed. The restore point and driver backup evidence were preserved.")?; }
        Ok(())
    }
        ...
        if let Some(mut r)=self.db.get_execution(plan_id)? {
            if r.completed_unix_ms.is_none() {
                ...
                let close_error = match (r.restore_point_sequence, r.mutation_started) {
                    (Some(seq), false) => self.platform.cancel_restore(seq, &restore_description).err(),
                ...
                r.stage=if r.mutation_started { "FailedAfterMutation" } else { "FailedSafe" }.into();
                ...
                if let Some(close_error) = close_error {
                    r.detail.push_str(&format!(" Restore transaction close also failed: {close_error}"));
                }
                r.updated_unix_ms=now_ms(); r.completed_unix_ms=Some(now_ms());
                self.db.upsert_execution(&r)?;
                if r.recovery_required {
                    self.add_recovery(&r, "warning", "safe-failure", "Driver installation needs recovery review", &r.detail)?;
                }
            }
        }
        if let Some(state) = state {
            if matches!(state, PlanState::Preflight|PlanState::Protected|PlanState::Executing|PlanState::Verifying|PlanState::Resuming) {
                let _=self.engine.transition(plan_id,state,PlanState::Failed,"driver install worker failed safely");
            }
        }
        Ok(())
    }

    fn add_recovery(&self, r:&ExecutionRecord, severity:&str, kind:&str, summary:&str, detail:&str) -> Result<()> { self.db.add_recovery_record(&RecoveryRecord { plan_id:r.plan_id.clone(), severity:severity.into(), kind:kind.into(), summary:summary.into(), detail:detail.into(), restore_point_sequence:r.restore_point_sequence, backup_root:r.backup_root.clone(), created_unix_ms:now_ms(), ..Default::default() })?; Ok(()) }
}
...
fn unique_identities(actions:&[DriverInstallAction])->Vec<UpdateIdentity>{
    ...
    for action in actions {
        let identity=UpdateIdentity{update_id:action.update_id.clone(),revision:action.revision};
        if seen.insert(identity.clone()) { out.push(identity); }
    }
    out
}
fn same_update(action:&DriverInstallAction, identity:&UpdateIdentity)->bool { action.revision==identity.revision && action.update_id.eq_ignore_ascii_case(&identity.update_id) }
...
fn action_from_selection(v: InstallSelection) -> DriverInstallAction {
    let mut action = DriverInstallAction {
        ...
        current_driver:v.current_driver.map(|d| DriverEvidence { provider:d.provider, version:d.version, inf_path:d.inf_path, date:d.date }),
...
fn action_provenance_digest(action:&DriverInstallAction)->String {
    let material=format!("{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}",action.candidate_id,action.update_id,action.revision,normalize_pnp_id(&action.instance_id),normalize_pnp_id(&action.matched_hardware_id),action.target_version,action.authority_type,action.authority_provider_id,action.official_source,action.installation_mode,action.trust_state);
    hex::encode(Sha256::digest(material.as_bytes()))
}
fn norm(value:&str)->String { value.trim().to_ascii_lowercase() }
fn drivers_equivalent(actual:&Option<InstalledDriver>, expected:&Option<DriverEvidence>)->bool {
    match (actual, expected) {
        (None, None) => true,
        (Some(a), Some(e)) => norm(&a.provider)==norm(&e.provider)
...
mod tests {
    ...
    struct FakePlatform { mutation_count: Mutex<u32>, reboot: bool }
    impl InstallPlatform for FakePlatform {
        fn boot_marker_ms(&self)->std::result::Result<i64,String>{Ok(1000)}
        fn verify_devices(&self,ids:&[String])->std::result::Result<Vec<DeviceVerification>,String>{Ok(ids.iter().map(|id|DeviceVerification{instance_id:id.clone(),present:true,class_name:"Net".into(),hardware_ids:vec!["PCI\\VEN_FAKE&DEV_0001".into()],compatible_ids:Vec::new(),status:DeviceStatus::default(),driver:Some(InstalledDriver{provider:"Vendor".into(),version:"2.0".into(),inf_path:"oem2.inf".into(),date:"".into()})}).collect())}
    ...
    #[test] fn boot_marker_requires_real_boot_change(){assert!(!boot_changed(1_000,20_000));assert!(boot_changed(1_000,40_001));}
    #[test] fn only_success_result_codes_verify(){assert!(result_code_success("orcSucceeded"));assert!(!result_code_success("orcSucceededWithErrors"));assert!(!result_code_success("orcFailed"));}
    #[test] fn fake_platform_never_mutates_before_barrier(){let p=FakePlatform{mutation_count:Mutex::new(0),reboot:false}; let ids=vec![UpdateIdentity{update_id:"u".into(),revision:1}]; let mut progress=|_:WuaProgress|{}; let mut barrier=||Ok(()); let _=p.execute_wua(&ids,&mut progress,&mut barrier).unwrap(); assert_eq!(*p.mutation_count.lock().unwrap(),1);}

    #[test]
    fn same_wua_offer_is_executed_once_for_multiple_devnodes() {
        let mk = |candidate: &str, instance: &str| DriverInstallAction {
            candidate_id: candidate.into(), update_id: "shared-update".into(), revision: 4,
            instance_id: instance.into(), display_name: instance.into(),
            matched_hardware_id: "PCI\\VEN_FAKE&DEV_0001".into(), match_quality: "Hardware ID".into(),
            current_driver: None, current_problem_code: 0, update_title: "Vendor - Net".into(),
            provider: "Vendor".into(), driver_class: "Net".into(), min_download_bytes: 1, max_download_bytes: 2,
            target_version:"2.0".into(), authority_type:"WindowsUpdate".into(), authority_provider_id:"microsoft.windows-update".into(),
            official_source:"Windows Update".into(), recommendation_state:"Recommended".into(), installation_mode:"WindowsManaged".into(),
            trust_state:"WindowsManaged".into(), provenance_digest:String::new(),
        };
    ...
    fn preflight_driver_binding_comparison_uses_all_evidence() {
        ...
        let exact = Some(DriverEvidence { provider:"vendor".into(), version:"2.0".into(), inf_path:"OEM2.INF".into(), date:"2026-01-01".into() });
        let wrong_inf = Some(DriverEvidence { provider:"Vendor".into(), version:"2.0".into(), inf_path:"oem9.inf".into(), date:"2026-01-01".into() });
