...
pub fn driver(snapshot: &DriverHubSnapshot, now: i64) -> Vec<SystemFact> {
    let mut out = vec![SystemFact::new(
        ...
        FactPayload::InventorySummary { device_count: snapshot.summary.device_count },
        EvidenceKind::DeviceState,
        format!("devices={};authorityCoverage={}", snapshot.summary.device_count, snapshot.authority_coverage),
    )];
    if snapshot.state != DriverScanState::Ready { return out; }
    for d in &snapshot.devices {
        let resource = ResourceRef::private("pnp-device", &d.instance_id, display(&d.display_name, "Device"));
        let driver_version = d.driver.as_ref().map(|v| v.version.clone()).unwrap_or_default();
        let gpu_vendor = d.gpu.as_ref().map(|g| format!("{:?}", g.vendor)).unwrap_or_default();
        out.push(SystemFact::new(
            Domain::Hardware, "driver-hub", resource.clone(), snapshot.completed_unix_ms, Freshness::Current, Confidence::Confirmed,
            FactPayload::HardwareDevice { class_name:d.class_name.clone(), manufacturer:d.manufacturer.clone(), description:d.description.clone(), installed_driver_version:driver_version.clone(), gpu_vendor },
            EvidenceKind::DeviceState,
            format!("class={};manufacturer={};driverVersion={}", d.class_name, d.manufacturer, driver_version),
        ));
        out.push(SystemFact::new(
        ...
        for c in &d.candidates {
            let finding_worthy = c.candidate_id == d.recommended_candidate_id
                || matches!(c.recommendation_state.as_str(), "Recommended" | "Optional" | "FirmwareProtected");
            if !finding_worthy { continue; }
            out.push(SystemFact::new(
...
pub fn repair(snapshot:&RepairAssessment)->Vec<SystemFact>{
    if snapshot.state!=RepairAssessmentState::Ready{return Vec::new()}
    snapshot.checks.iter().map(|c|{
        let confidence = if c.stage.eq_ignore_ascii_case("Unknown") || matches!(c.result_code.as_str(), "ProbeUnavailable" | "SystemFilesUnknown" | "WinReStateUnverified" | "WinReStateUnknown" | "RestoreStateUnverified" | "RestoreStateUnknown") { Confidence::Unknown } else { Confidence::Confirmed };
        SystemFact::new(Domain::Windows,"system-repair",ResourceRef::global("windows-check",&c.id,display(&c.title,"Windows integrity check")),snapshot.completed_unix_ms,Freshness::Current,confidence,FactPayload::WindowsIntegrity{check_id:c.id.clone(),result_code:c.result_code.clone(),exit_code:c.exit_code,detail:c.detail.clone()},EvidenceKind::ServicingResult,format!("{};exitCode={}",c.result_code,c.exit_code))
    }).collect()
}

pub fn diagnostics(snapshot: &DiagnosticsSnapshot) -> Vec<SystemFact> {
    let mut out = Vec::new();
    if !matches!(snapshot.state, DiagnosticScanState::Ready | DiagnosticScanState::Partial) {
        return out;
    }
    for d in &snapshot.storage {
        let resource = ResourceRef::private("storage-device", &d.device_id, display(&d.friendly_name, "Storage device"));
        let r = &d.reliability;
        out.push(SystemFact::new(
            Domain::Storage,
            "diagnostic-engine",
    ...
    if let Some(m) = &snapshot.memory {
        out.push(SystemFact::new(
            ...
            EvidenceKind::DeviceState,
            format!("load={}%;availableBytes={};totalBytes={};pressure={}", m.memory_load_percent, m.available_physical_bytes, m.total_physical_bytes, m.pressure_label),
        ));
    }
    for e in &snapshot.events {
        out.push(SystemFact::new(
            Domain::Hardware,
            "crash-diagnostics",
            ResourceRef::global("hardware-event", &format!("{}-{}-{}", e.provider, e.event_id, e.recorded_unix_ms), "Hardware error evidence"),
            e.recorded_unix_ms,
            Freshness::Recent,
            Confidence::High,
            FactPayload::HardwareEvent { category: e.category.clone(), provider: e.provider.clone(), event_id: e.event_id },
            EvidenceKind::HardwareEvent,
            format!("provider={};eventId={};category={}", e.provider, e.event_id, e.category),
        ));
    }
...
pub fn startup(snapshot:&StartupSnapshot)->Vec<SystemFact>{if snapshot.state!=StartupScanState::Ready{return Vec::new()}vec![SystemFact::new(Domain::Startup,"startup-manager",ResourceRef::global("startup","startup-footprint","Startup footprint"),snapshot.completed_unix_ms,Freshness::Current,Confidence::Confirmed,FactPayload::StartupFootprint{high_impact_count:snapshot.summary.high_impact,manageable_count:snapshot.summary.manageable,total_count:snapshot.summary.total},EvidenceKind::StartupRegistration,format!("total={};highImpact={};manageable={}",snapshot.summary.total,snapshot.summary.high_impact,snapshot.summary.manageable))]}
...
fn nonzero(value:Option<&str>)->bool{value.map(str::trim).filter(|v|!v.is_empty()).is_some_and(|v|v.parse::<u128>().map(|n|n>0).unwrap_or_else(|_|v.trim_start_matches('0').trim_start_matches('x').chars().any(|c|c!='0')))}
...
pub fn driver_change(item: &InstallItemRecord) -> SystemFact {
    let raw_identity = format!("{}|{}|{}", item.instance_id, item.candidate_id, item.updated_unix_ms);
    let before = serde_json::from_str::<InstalledDriverEvidence>(&item.before_driver_json).unwrap_or(InstalledDriverEvidence { version:String::new(), provider:String::new() });
    let after = serde_json::from_str::<InstalledDriverEvidence>(&item.after_driver_json).unwrap_or(InstalledDriverEvidence { version:String::new(), provider:String::new() });
    let version = after.version.clone();
    SystemFact::new(
        Domain::Drivers,
        "driver-install-journal",
        ResourceRef::private("driver-change", &raw_identity, display(&item.title, "Driver change")),
        item.updated_unix_ms,
        Freshness::Recent,
        Confidence::Confirmed,
        FactPayload::DriverChange { installed_unix_ms:item.updated_unix_ms, previous_version:before.version.clone(), version:version.clone(), authority_type:String::new(), authority_provider_id:String::new(), provider:after.provider.clone(), reboot_required:item.reboot_required, verified:item.verified, rollback_available:!item.backup_path.is_empty() },
        EvidenceKind::DriverVersion,
        format!("verified={};previousVersion={};version={};provider={};reboot={};rollbackAvailable={}", item.verified, before.version, version, after.provider, item.reboot_required, !item.backup_path.is_empty()),
    )
}
