...
use windows::{
    Win32::{
        ...
        System::{
            ...
            UpdateAgent::{
                IDownloadCompletedCallback, IDownloadCompletedCallbackArgs, IDownloadCompletedCallback_Impl,
                IDownloadJob, IDownloadProgressChangedCallback, IDownloadProgressChangedCallbackArgs,
                IDownloadProgressChangedCallback_Impl, IInstallationCompletedCallback,
                IInstallationCompletedCallbackArgs, IInstallationCompletedCallback_Impl,
                IInstallationJob, IInstallationProgressChangedCallback,
                IInstallationProgressChangedCallbackArgs, IInstallationProgressChangedCallback_Impl,
                IUpdate, IUpdateInstaller2, IUpdateSession, IWindowsDriverUpdate, UpdateSession,
                orcAborted, orcFailed, orcInProgress, orcNotStarted, orcSucceeded,
...
use crate::{ExecutionStage, Result, UpdateError, UpdateIdentity, WuaExecutionResult, WuaProgress, WuaUpdateResult};
...
pub fn ensure_servicing_available() -> Result<()> {
    let _com = ComApartment::mta().map_err(|hr| UpdateError::Wua(format!(
    ...
    unsafe {
        ...
        session.SetClientApplicationID(&BSTR::from("AetherCore System Repair Preflight")).map_err(wua_err)?;
        let installer = session.CreateUpdateInstaller().map_err(wua_err)?;
        installer.SetClientApplicationID(&BSTR::from("AetherCore System Repair Preflight")).map_err(wua_err)?;
        if installer.IsBusy().map_err(wua_err)?.as_bool() { return Err(UpdateError::Busy); }
        if installer.RebootRequiredBeforeInstallation().map_err(wua_err)?.as_bool() { return Err(UpdateError::RebootPending); }
    }
    Ok(())
}
...
{
    if identities.is_empty() { return Err(UpdateError::OfferNoLongerApplicable("empty selection".into())); }
    ...
    unsafe {
        ...
        let installer = session.CreateUpdateInstaller().map_err(wua_err)?;
        installer.SetClientApplicationID(&BSTR::from("AetherCore Safe Driver Installation")).map_err(wua_err)?;
        installer.SetUpdates(&selected).map_err(wua_err)?;
        installer.SetIsForced(VARIANT_BOOL(0)).map_err(wua_err)?;
        installer.SetAllowSourcePrompts(VARIANT_BOOL(0)).map_err(wua_err)?;
        ...
        installer2.SetForceQuiet(VARIANT_BOOL(-1)).map_err(wua_err)?;
        if installer.IsBusy().map_err(wua_err)?.as_bool() { return Err(UpdateError::Busy); }
        if installer.RebootRequiredBeforeInstallation().map_err(wua_err)?.as_bool() { return Err(UpdateError::RebootPending); }

        let downloader = session.CreateUpdateDownloader().map_err(wua_err)?;
        downloader.SetClientApplicationID(&BSTR::from("AetherCore Safe Driver Installation")).map_err(wua_err)?;
        ...
        let download_job = downloader.BeginDownload(&dp, &dc, &state).map_err(wua_err)?;
        ...
        if !is_success(download_code) {
            return Err(UpdateError::Wua(format!("driver download failed: {download_code:?}; HRESULT={:?}", download_result.HResult().map_err(wua_err)?)));
        }
        ...
        for index in 0..count {
            ...
            if !is_success(code) { return Err(UpdateError::Wua(format!("download failed for selected update {index}: {code:?}"))); }
        }
...
unsafe fn revalidate_selection(session: &IUpdateSession, identities: &[UpdateIdentity]) -> Result<windows::Win32::System::UpdateAgent::IUpdateCollection> {
    let searcher = unsafe { session.CreateUpdateSearcher().map_err(wua_err)? };
    unsafe { searcher.SetOnline(VARIANT_BOOL(-1)).map_err(wua_err)?; }
    let search = unsafe { searcher.Search(&BSTR::from(SEARCH_CRITERIA)).map_err(wua_err)? };
    ...
    if code != orcSucceeded { return Err(UpdateError::Wua(format!("WUA pre-install revalidation was not complete: {code:?}"))); }
    let all = unsafe { search.Updates().map_err(wua_err)? };
    let selected = unsafe { all.Copy().map_err(wua_err)? };
    unsafe { selected.Clear().map_err(wua_err)?; }
    ...
    for i in 0..count {
        ...
        if update.cast::<IWindowsDriverUpdate>().is_err() { continue; }
        let identity = unsafe { update.Identity().map_err(wua_err)? };
        let key = (unsafe { identity.UpdateID().map_err(wua_err)?.to_string() }, unsafe { identity.RevisionNumber().map_err(wua_err)? });
        if identities.iter().any(|want| want.update_id == key.0 && want.revision == key.1) { found.insert(key, update); }
    }

    for identity in identities {
        let key = (identity.update_id.clone(), identity.revision);
        let update = found.remove(&key).ok_or_else(|| UpdateError::OfferNoLongerApplicable(format!("{} rev {}", identity.update_id, identity.revision)))?;
        if unsafe { update.IsInstalled().map_err(wua_err)?.as_bool() } {
            return Err(UpdateError::OfferNoLongerApplicable(format!("{} is already installed", identity.update_id)));
        }
        ...
        if class.eq_ignore_ascii_case("firmware") {
            return Err(UpdateError::OfferNoLongerApplicable("firmware is protected from generic driver execution".into()));
        }
        unsafe { selected.Add(&update).map_err(wua_err)?; }
    }
    Ok(selected)
}
...
unsafe fn wait_download<P: FnMut(WuaProgress)>(job: &IDownloadJob, progress: &mut P) -> Result<()> {
    ...
    loop {
        if started.elapsed() > DOWNLOAD_TIMEOUT { unsafe { let _ = job.RequestAbort(); let _ = job.CleanUp(); } return Err(UpdateError::Timeout("download".into())); }
...
unsafe fn wait_install<P: FnMut(WuaProgress)>(job: &IInstallationJob, progress: &mut P) -> Result<()> {
    let started = Instant::now();
    loop {
        if started.elapsed() > INSTALL_TIMEOUT { unsafe { let _ = job.RequestAbort(); let _ = job.CleanUp(); } return Err(UpdateError::Timeout("installation".into())); }
        let p = unsafe { job.GetProgress().map_err(wua_err)? };
        progress(WuaProgress {
...
fn clamp_percent(value: i32) -> u32 { value.clamp(0, 100) as u32 }
fn is_success(code: windows::Win32::System::UpdateAgent::OperationResultCode) -> bool { code == orcSucceeded }
fn result_code_name(code: windows::Win32::System::UpdateAgent::OperationResultCode) -> &'static str {
    if code == orcNotStarted { "orcNotStarted" }
    else if code == orcInProgress { "orcInProgress" }
    else if code == orcSucceeded { "orcSucceeded" }
    else if code == orcSucceededWithErrors { "orcSucceededWithErrors" }
    else if code == orcFailed { "orcFailed" }
    else if code == orcAborted { "orcAborted" }
    else { "orcUnknown" }
}

fn decimal_to_u64(value: &DECIMAL) -> std::result::Result<u64, ()> {
    #[link(name = "oleaut32")]
    unsafe extern "system" { fn VarUI8FromDec(pdecin: *const DECIMAL, pout: *mut u64) -> windows::core::HRESULT; }
    let mut out = 0u64;
    let hr = unsafe { VarUI8FromDec(value, &mut out) };
    if hr.is_err() { Err(()) } else { Ok(out) }
}

fn wua_err(error: windows::core::Error) -> UpdateError { UpdateError::Wua(error.to_string()) }
