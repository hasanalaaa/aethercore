#!/usr/bin/env python3
"""Build the Phase 23 binary-safe reconstruction package.

Compares the delivered Phase 23 tree against the sealed Phase 20 source tree and emits:
...
Usage: python3 build_phase23_patch.py <phase20_tree> <delivered_tree> [out_dir]
"""
...
OUT = Path(sys.argv[3]).resolve() if len(sys.argv) > 3 else DELIVERED / "PHASE_23_BINARY_SAFE_PATCH"
...
def main() -> int:
    ...
    manifest = {
        "schema": "aethercore.phase23.manifest.v1",
        "base": "AetherCore-Phase22-Master-Delivery",
...
"""
    (OUT / "apply_phase23_patch.sh").write_text(apply_script, encoding="utf-8")
    (OUT / "apply_phase23_patch.sh").chmod(0o755)
    (OUT / "verify_phase23.py").write_text(verify_script, encoding="utf-8")
