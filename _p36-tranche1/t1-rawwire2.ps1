...
# remove the earlier duplicate probe module attempt at end of file if present, then build
...
cargo build --locked --release -p aethercore-ipc --example ipc_rawwire_probe *> "$inc\tranche1-rawwire-build2.log"
if ($LASTEXITCODE -ne 0) { Write-Output 'BUILD FAILED'; Get-Content "$inc\tranche1-rawwire-build2.log" -Tail 20; exit 1 }
Write-Output 'BUILD OK'
Copy-Item "$inc\payload\libomp140.aarch64.dll" 'target\release\examples\' -Force
Write-Output '=== RAW-WIRE probe run ==='
