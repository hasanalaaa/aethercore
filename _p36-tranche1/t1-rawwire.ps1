...
cargo build --locked --release -p aethercore-ipc --example ipc_rawwire_probe *> "$inc\tranche1-rawwire-build.log"
if ($LASTEXITCODE -ne 0) { Write-Output 'BUILD FAILED'; Get-Content "$inc\tranche1-rawwire-build.log" -Tail 15; exit 1 }
Copy-Item "$inc\payload\libomp140.aarch64.dll" 'target\release\examples\' -Force
Write-Output '=== RAW-WIRE probe (hello -> dup-hello breadcrumb -> request -> bounded response read) ==='
& 'target\release\examples\ipc_rawwire_probe.exe' 2>&1 | ForEach-Object { Write-Output $_ }
Write-Output '=== service log tail (look for duplicate-hello breadcrumb) ==='
