#!/usr/bin/env python3
"""Build the Phase 22 binary-safe reconstruction package.

Compares the delivered Phase 22 tree against the sealed Phase 20 source tree and emits:
...
Usage: python3 build_phase22_patch.py <phase20_tree> <delivered_tree> [out_dir]
"""
...
OUT = Path(sys.argv[3]).resolve() if len(sys.argv) > 3 else DELIVERED / "PHASE_22_BINARY_SAFE_PATCH"
...
def all_files(root: Path) -> dict[str, Path]:
    return {
        ...
        # Embedded delivery packaging from the base archive is not source; the P22
        # package supersedes it (its verify script re-verifies the whole tree).
...
def main() -> int:
    ...
    manifest = {
        "schema": "aethercore.phase22.manifest.v1",
        "base": "AetherCore-Phase21-Master-Delivery",
...
# Verify a reconstructed Phase 22 tree against MANIFEST.json.
...
"""
    (OUT / "apply_phase22_patch.sh").write_text(apply_script, encoding="utf-8")
    (OUT / "apply_phase22_patch.sh").chmod(0o755)
    (OUT / "verify_phase22.py").write_text(verify_script, encoding="utf-8")
