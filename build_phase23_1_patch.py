#!/usr/bin/env python3
"""Build the Phase 23.1 binary-safe reconstruction package.

Compares the delivered Phase 23.1 tree against the sealed Phase 20 source tree and emits:
...
Usage: python3 build_phase23_1_patch.py <phase20_tree> <delivered_tree> [out_dir]
"""
...
OUT = Path(sys.argv[3]).resolve() if len(sys.argv) > 3 else DELIVERED / "PHASE_23_1_BINARY_SAFE_PATCH"
...
def main() -> int:
    ...
    manifest = {
        "schema": "aethercore.phase23_1.manifest.v1",
...
# Verify a reconstructed Phase 23.1 tree against MANIFEST.json.
...
"""
    (OUT / "apply_phase23_1_patch.sh").write_text(apply_script, encoding="utf-8")
    (OUT / "apply_phase23_1_patch.sh").chmod(0o755)
    (OUT / "verify_phase23_1.py").write_text(verify_script, encoding="utf-8")
